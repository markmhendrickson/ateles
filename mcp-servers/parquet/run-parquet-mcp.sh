#!/bin/bash
# Wrapper script for Parquet MCP server
...
if [ ! -f "$SCRIPT_DIR/.git" ] && [ ! -d "$SCRIPT_DIR/.git" ] && [ ! -f "$SCRIPT_DIR/parquet_mcp_server.py" ]; then
    echo "Error: Parquet MCP server submodule not initialized." >&2
    echo "" >&2
    echo "The submodule is defined in .gitmodules but not initialized." >&2
    echo "" >&2
    echo "To initialize:" >&2
    echo "  cd $REPO_ROOT" >&2
    echo "  git submodule update --init mcp/parquet" >&2
    echo "" >&2
    echo "Or run:" >&2
...
if [ -f "$REPO_ROOT/execution/venv/bin/python3" ]; then
    exec "$REPO_ROOT/execution/venv/bin/python3" "$SCRIPT_DIR/parquet_mcp_server.py"
else
    exec python3 "$SCRIPT_DIR/parquet_mcp_server.py"
fi
