...
artifact is by definition an entry that system holds; the interoperable id is a field the adapter writes as an
...
**The objection, answered.** The open question declined this answer because one artifact would map
...
| a calendar deleted, or a primary calendar cleared | handled | an observation on every tracked artifact on it. The clearing operation deletes every event on a primary calendar at once, and the record's artifacts survive it — the observations record that the artifacts are gone |
...
| a meeting ended **and produced a recording** | not by the calendar, and not by the recording system either: the recording's arrival is a delivery its own system's adapter resolves, and a recording the record does not yet track is that adapter's fourth outcome — the adapter creates the recording artifact and a task for meeting processing enters intake with it attached (`adapters.md#no-external-event-advances-a-step-by-itself`, source 3 of `work_model.md#where-tasks-come-from-every-source-indexed`; `workflows.md#meeting-processing`). The calendar event becomes the task's second artifact through a time-matched read, not through the delivery | the calendar knows nothing about recordings; the linkage between a recording and an event is made by matching times, and it is a read with coverage, not a delivered fact |
...
The reading projection needs this distinction, so it is stated as a table. The right column is inventory:
