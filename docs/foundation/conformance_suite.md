...
| 12 | the successor declarations the run exercises (`feature`, `bug`, `payment`, …), each per `(project, type)` | named in intake's `successors`, or the closing verdict cannot select them | **derived** | each declaration; `successors` naming intake is refused as a declaration error |
