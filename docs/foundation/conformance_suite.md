...
| WM-13 | `work_model.md#intake-is-every-tasks-first-workflow`: every task is admitted with exactly one intake batch at creation | `T1`; a task created by a batch; a task from a daemon | read each task and its batches at creation, then after routing | the task is readable without its intake batch and `ADDRESSED_BY` edge; any batch of a workflow other than intake has no intake batch before it on the chain; or a task has two intake batches (X-5) | M |
| WM-14 | the same: a child may take intake's fast path and never skips intake | a child created by a routed batch | read the child and its intake batch at creation | `link` or `dedupe` open; the child is readable without its intake batch; or the batch opened after creation | M |
| WM-14a | `work_model.md#what-distinguishes-a-task-being-assembled-from-one-intake-has-not-reached` (decision 84): every task first becomes readable with its intake batch and `ADDRESSED_BY` edge; an assembling task's same unit also contains the persistent `classify` hold finding and is admitted whole or not at all; creation grants no lease, and only the declaration-resolved `pm` step owner may claim assembly; the exclusion survives lease lapse, crash, return, and transfer until a `classify` verdict exists; an ordinary intake task has the batch and edge but no assembly finding and stays claimable | one task created as still being assembled by a non-PM creator; one by several contributors; one complete task created for ordinary intake; an eligible PM holder that renews, crashes, and transfers the lease; mutants that expose the task early, delete the finding on lapse, or grant the creator a lease | read after every creation and lease boundary; evaluate ordinary claimability and the PM-only assembly claim | either creation unit is partial; a creator or contributor receives a lease without resolving as `pm`; the task enters an ordinary claim pool before a `classify` verdict, including after lapse or transfer; an eligible successor PM cannot claim assembly; the ordinary task is withheld or carries an assembly finding; any of the three mutants stays green | M — effect-shaped acceptance for decision 84; the persistent-exclusion and creator-ownership mutants prove the rule can fail |
...
| WM-21 | the same: a daemon's task enters intake like every task, with its intake batch at creation | `X(gmail)` delivers an untracked message | read the task at creation | the task is readable without its intake batch; a batch of another workflow; or a lease held by the daemon that created it | M |
...
| WM-32b | the same: `DEPENDS_ON` is not on the chain; the created task is a peer with its own intake batch admitted at creation and its own priority | as WM-32 | read the chain and the task at creation | the chain follows a `DEPENDS_ON` edge; the task is readable without its intake batch; or its priority moved with the dependency | M |
...
| WM-39 | `work_model.md#where-tasks-come-from-every-source-indexed`: every task from all nine sources gets its intake batch and `ADDRESSED_BY` edge at creation; decision 84's assembly exception adds the persistent hold to that same unit, and the creating principal holds no privilege over either shape | a full run exercising each source, once ordinarily and once where assembly is requested | read every task's provenance, first batch, hold, and lease at creation | a task whose provenance names none of the nine kinds; any task readable without its intake batch and edge; a first batch other than intake; an ordinary task carrying the assembly hold; an assembling task without the hold; or a lease granted because the credential created the task rather than because it resolves as the declared `pm` step owner | M |
