...
| 8 | the credential binding per agent: a `principal_binding` edge carrying `credential_kind: aauth_sub` with the agent's `sub` and `iss` → the `agent` (decision 101) | the AAuth `sub` binds to the agent and reaches the operator through the agent's `principal_binding` | **derived**; same type gap as step 3 | a write by the agent attributes A-for-B |
