...
| 1 | the registry: every entity type and relationship type the concept tables name (`data_model.md#concepts`, `data_model.md#relationships`), each with a version and a `reducer_config`; every relationship type also declares `acyclic` or `cycles_admitted` (decision 102) | nothing can be written until its type exists; a type registered later leaves earlier writes in `raw_fragments` forever (`data_model.md#record-conventions`); a relationship type with no valid acyclicity declaration is refused | **derived** — the first member of the closed set (decision 43, ruled in its enumeration half below): an operator act by membership, read back | every type present with its declared fields; every relationship type's acyclicity declaration present with the written value; a registration omitting it is refused; a write of an undeclared field lands in `raw_fragments` |
...
| 6 | the first agents: at least the `pm` step owner's agent and the operator-facing agent, each with an **acts-as** `principal_binding` → the operator carrying only `credential_kind: acts_as` (traversal-only and non-presentable; the edge decision 48's counting rule reads; step 8 adds each agent's *second* binding, its AAuth credential → the `agent` itself) | intake's every step is the `pm` step owner's; operator-only work and every checkpoint the operator is carried is the operator-facing agent's (`workflows.md#roles-named-in-this-document`) | **derived** as to which two, and as members of the closed set (decision 43); later writes to them are gated as step 5's are (decision 43, ruled) | each `agent` with `prompt_markdown`, `context_entity_types[]`, a version; the binding edge |
...
| 8 | the credential binding per agent: a **second** `principal_binding` edge carrying `credential_kind: aauth_sub` with the agent's `sub` and `iss` → the `agent` (decision 101). This does not replace step 6's acts-as edge and does not contradict it: the presented AAuth shape has credential values and attributes the agent; the traversal-only acts-as shape does not | the AAuth `sub` binds to the agent, which is what makes a write attributable to the agent rather than to the operator; the operator is reached by traversing the agent's step-6 acts-as binding | **derived**; same type gap as step 3 | a write by the agent attributes A-for-B |
...
as a data-model gap (`migration.md`, G17), **closed by decision 101's ruling**; the presented-credential
variant carries the credential kind, value, issuer, principal endpoint, and expiry, while the second
admitted variant is traversal-only `acts_as` carrying only its kind (decision 107). Both shapes are read
back at steps 3, 6, and 8.
...
| AU-29 | `authority_model.md#what-the-credential-binding-carries-and-what-a-check-reads-to-resolve-a-credential-to-a-principal` (decisions 107–109): `acts_as` is traversal-only, agent → operator exactly; it carries no presented value; agent → agent uses `delegation_edge`; an operator never sources acts-as | an AAuth binding to agent A; A acts-as operator O; agent B; operator P | present AAuth then read the acting interest; present `credential_kind: acts_as`; write A → B acts-as; write O → P acts-as; write A → B `delegation_edge` | the AAuth presentation does not attribute to A or the traversal does not yield O; the acts-as presentation resolves or is treated as merely missing a value; either forbidden acts-as edge lands; the delegation edge is refused | M — removing the presentation-kind exclusion or either endpoint check turns this row red |
...
| DM-31 | `data_model.md#whether-acyclicity-is-a-property-of-a-relationship-type-or-of-the-graph` (decision 102): every relationship type declares acyclic or cycles-admitted at registration; absence, malformed values, and unknown values are refused; an acyclic type is checked before every relationship write, and unknown check state denies | register one type under each valid declaration; register three types with absent, malformed, and unknown declarations; write a chain then an edge closing its cycle; make the cycle read unavailable; write a cycle on the cycles-admitted type | either valid registration is refused; any invalid registration lands; the closing edge on the acyclic type lands; the edge lands under the unavailable read; the admitted-cycle write is refused by a universal check | M — the missing-declaration and skipped-write-check mutants are planted positives |
...
| PM-13 | `planning_model.md#every-planning-record-belongs-to-an-instance-and-swarm-operations-planning-belongs-to-the-controlling-instance` (decision 99): every planning record belongs to one instance; swarm-operations planning in a multi-instance deployment is created in its controlling instance; none means refusal, never a global record | a deployment naming instances A and B with A controlling; one naming both with none controlling | create a swarm-operations plan in each deployment; inspect every planning record's instance and ascent | the first plan is outside A or belongs to no instance; the second creates any plan or chooses an instance by order; a planning hierarchy spans A and B | M |
