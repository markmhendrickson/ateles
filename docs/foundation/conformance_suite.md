...
existence when a closing verdict names a successor "and at no other moment", then names another moment:
the intake batch "opened on the task's creation". The principles settle who opens it — "every arrow into a
batch is a principal's recorded conclusion", and the daemon write contract says the tasks a poll produces are
...
| WM-27 | `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`: opened only by a closing verdict | every batch after a full run | read `FOLLOWS` and the predecessor's closing verdict | a non-intake batch with no `FOLLOWS`; a `FOLLOWS` target whose closing verdict's `successor` is not this batch's `workflow_type`; a batch opened by a daemon, an adapter, or a sweeper credential in `RP` | M |
...
| WM-31 | `work_model.md#a-batch-may-hold-on-a-condition-discovered-mid-flight`: a hold is a finding naming the condition, no verdict, the lease renewed; no held state and no field | `T-at(payment, pay)`, `X(rail)` returning `unknown`, `CLK` | the step owner holds | no finding naming the condition and what would resolve it; a verdict written; the lease not renewed; a held, paused, or waiting value on the batch or the task; the hold's duration not readable from observations on the finding; the finding not a `finding` entity `REFERS_TO` the batch with no `PART_OF` to any verdict | M (X-14 closed) |
| WM-31a | the same: a hold ends by verdict, checkpoint, or lapse, never by elapsed time into a pass; a hold owing nobody a decision is a rule-5 deferral | four holds: the condition resolves; it owes a decision; it never resolves and owes none; the holder dies | wait | the first does not end in a verdict; the second raises no checkpoint; the third raises no `rounds_exhausted` carrying the finding at the ceiling; the fourth does not lapse and read claimable; any closes by time | M (`hold_bound`; U-20 closed) |
...
| WM-35a | `work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`: one live instance, never zero and never two; the closing verdict creates the next, `FOLLOWS` task to task, the rule copied | a task with a `recurrence` rule, routed and completed; `RP` failing the creation write; `RP` failing the verdict after the creation, then a re-claim | read after the closing verdict | zero non-terminal instances, or two outside the closing interval; the new one lacks `FOLLOWS` to the completed one or its `recurrence`; the completed one's status ever leaves terminal; the new one has no intake batch; under the failed creation the verdict landed; under the failed verdict the re-claim creates a second successor (U-28 closed: create, read back, then sign) | M |
...
| AU-22 | `authority_model.md#what-the-credential-binding-carries-and-what-a-check-reads-to-resolve-a-credential-to-a-principal`, resolver-outcome table, row 1: `Permit` (resolved) — at least one live (unended, unexpired) `principal_binding` matches the presented `credential_kind` + `credential_value` (+ `credential_issuer` when required), and every match targets the same principal | a principal with one live `principal_binding` edge carrying a `credential_kind`/`credential_value`/`credential_issuer` the test presents | resolve the credential | the resolver returns anything but `Permit`; the resolved principal is not the edge's endpoint | M (decision 101; no resolver exists yet in this checkout, so this row is red until one is built — the operator's 2026-09-18 ruling on ateles#925 requires the row to exist and be able to fail, not to pass) |
| AU-23 | the same table, row 2: `Deny` (no binding) — no matching edge for the presented key; recorded as unattributed, never defaulted to the operator | a presented credential with zero `principal_binding` edges of any kind on the instance | resolve the credential | the resolver returns `Permit`; the write proceeds attributed to the operator or to any principal | M (same red-by-design status as AU-22) |
| AU-24 | the same table, row 3: `Deny` (expired-only) — matching edges exist but all are ended or `now >= expires_at`; distinct from no-binding | a principal whose only matching `principal_binding` edge has `expires_at` in the past, or is ended | resolve the credential | the resolver returns `Permit`; the outcome is recorded as no-binding rather than as expired/ended, collapsing the two into one surface | M (same red-by-design status as AU-22) |
| AU-25 | the same table, row 4: `Indeterminate` (binding source unreadable/partial) — the relationship store is unreachable, timed out, or the read is incomplete; the enforcement point treats it as `Deny` and the third value is kept in diagnostics rather than coerced to a plain `Deny` in the recorded decision | `RP` failing or hanging the read of `principal_binding` edges for a presented credential | resolve the credential | the enforcement point permits; the recorded decision is written as a plain `Deny` with no distinguishable `Indeterminate` in diagnostics (principle 7) | M (same red-by-design status as AU-22) |
| AU-26 | the same table, row 5: `Deny` (ambiguous/conflicting) — two or more live matching edges target different principals; fail closed, never pick the first edge | two live `principal_binding` edges matching one presented credential, targeting two different principals | resolve the credential | the resolver returns `Permit` on either principal instead of `Deny`; the resolver's choice depends on edge order | M (same red-by-design status as AU-22) |
| AU-27 | the same table, row 6: `Deny` (malformed presentation) — the presented credential is missing a required key part (`credential_kind` / `credential_value` / issuer when the kind requires one), or the kind is unrecognized | a presentation missing `credential_value`; a presentation naming a `credential_kind` the instance has no registry entry for | resolve the credential | the resolver returns `Permit` or `Indeterminate` instead of `Deny`; a missing part is treated as a wildcard match | M (same red-by-design status as AU-22) |
| AU-28 | `authority_model.md#grants`, as proposed by open conformance.md register row 111 (operator leaning recorded 2026-09-18 on ateles#925; status stays open while G25 / neotoma#2357 is unlanded): a capability's `relationship_types[]` beside `entity_types[]` governs edge writes on the same default-deny shape as decision 41 — absent or empty denies every relationship write; naming a type permits that type and denies every other; a wildcard over relationship types is the same fail-open shape AU-21 refuses for entity types | an agent whose grant capability names `relationship_types: ["principal_binding"]` and no other; an agent whose grant capability names `entity_types` but no `relationship_types` at all; an agent whose grant capability names `relationship_types: []` (present but empty); a grant capability carrying `relationship_types: ["*"]` | write a `principal_binding` edge; write a `delegation_edge` under the same grant; write a `principal_binding` edge under the empty-array grant; register the wildcard-relationship-type grant | the `principal_binding` write lands under the first grant but the `delegation_edge` write also lands (no per-type discrimination); the second grant's edge write of any type lands (absent `relationship_types[]` does not deny); the third grant's edge write lands (a runner that special-cases only the missing key, rather than the missing-or-empty rule, does not deny an explicit empty array); the wildcard grant is read as in force for a human, or as admitting a relationship type G25's registry does not carry | M (open register row 111; red-by-design until G25 — the relationship-type registry, `neotoma#2357`, in review — lands, since a grant cannot name a type the record has no registry for, and until a resolver enforces the dimension at all. Bounding condition 2 of the operator leaning: no `ruled` register row while G25 is unlanded — this row tests the proposed admission shape, not a closed ruling) |
