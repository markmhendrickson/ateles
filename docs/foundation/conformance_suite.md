...
Amendment history: `revisions.md#conformance_suitemd`.
## Purpose
...
built elsewhere; the decisions open when each row below was designed (31 to 36 and 42 to 56 at this
document's own last revision; 43 and 44 are argued below), whose rows are designed per candidate and held
pending until this document is revised to catch up with the register — almost all of that range has since
been ruled, per `conformance.md#the-register-of-open-design-decisions` (register-reconciliation pass,
2026-09-06) — and the authority model's open questions, held the same way; and the reading projection generated from the matrix below, whose
generator and check are `conformance.md`'s (decision 66) and whose content follows this document's, as it has
...
| `B0+pol(...)` | `B0` with the instance's `action_policy` set as stated: which classes are listed low, which high, which carry `operator_only`, and the declared lossy-mutation count |
...
the suite's first dependency is the record's own, exactly as the migration's stage 1 is, and until
...
| 8 | the credential binding per agent: a **second** `principal_binding` edge carrying `credential_kind: aauth_sub` with the agent's `sub` and `iss` → the `agent` (decision 101). This does not replace step 6's acts-as edge and does not contradict it: the two edges of this type have different endpoints on purpose | the AAuth `sub` binds to the agent, which is what makes a write attributable to the agent rather than to the operator; the operator is reached through the agent's step-6 acts-as binding | **derived**; same type gap as step 3 | a write by the agent attributes A-for-B |
| 9 | the `swarm_roster` for the declaration scope: role → agent, by role | a workflow's `owner_role` is resolved against it at claim, and — by `failure_posture.md#checkpoints-on-tasks-one-queue-one-protocol` — at declaration, so the roster must precede the first declaration | **derived** | every role the intake declaration names resolves |
...
| 11 | the intake declaration: `workflow` for `(*, intake)` with its five steps, `owner_role` values, `reads_to_enter` and `reads_to_close`, `fast_paths` (`inherits`), `successors` | the first declaration, and the one every workflow-entering task needs; the design names it the operator act | **derived** as an operator act (`work_model.md`); the `*` scope is a wording gap (below) | the declaration; a role it names resolves against step 9; a step reading a type not in step 10 raises `undeclared_dependency` at first use |
| 12 | the successor declarations the run exercises (`feature`, `bug`, `payment`, …), each per `(declaration_scope, type)` | named in intake's `successors`, or the closing verdict cannot select them | **derived** | each declaration; `successors` naming intake is refused as a declaration error |
| 13 | per adapter: the adapter `agent`, its grant and binding, the `vendor_binding` context entity, the credential bindings of the actors it will meet (`cred-owner`, `cred-op`, `cred-other`), and the adapter's action classes listed in the policy | the six admission obligations (`adapters.md#the-admission-contract`); an adapter whose class is unlisted is inert, which is the right failure direction | **derived** from the admission section | the grant; the policy classes; a delivery from `cred-none` resolves to no principal |
...
**An agent bound to a human holds two `principal_binding` edges, not one.** Steps 6 and 8 both write an
edge of this type for the same agent, with deliberately different endpoints: step 6's acts-as edge ends at
the `operator` and is the edge decision 48's counting rule reads (so two agents under one operator are one
interest); step 8's AAuth edge ends at the `agent` and is what resolves a presented `sub`, making the
agent's writes attributable to itself as A-for-B rather than to the operator. `credential_kind` tells them
apart, and a resolver takes the endpoint of the edge whose kind matches what was presented. A suite that
writes only one of the two fails: with only step 6, no presented AAuth credential resolves and attribution
collapses onto the operator; with only step 8, the counting rule has no operator endpoint to read and two
agents under one operator would count as two interests.
...
**The intake batch's opener is explicit, and differs from every successor batch's opener.**
`work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow` names both moments: a
workflow-entering task's atomic creation unit opens its intake batch without a predecessor verdict, while a
closing verdict opens a successor batch. The daemon write contract says the tasks a poll produces are "each
entering intake" (`data_model.md#write-contract`), so the creating principal's store of a workflow-entering
task **is** the act, and the intake batch and its `ADDRESSED_BY` edge are part of what creation writes. The
suite asserts both forms (every workflow-entering task has exactly one intake batch, opened at creation; an
aggregate parent is not claimable and has none; every successor batch names its predecessor verdict).
...
as a data-model gap (`migration.md`, G17), **closed by decision 101's ruling**; the derivation is an edge type carrying the credential kind, value, and issuer, the principal,
and an expiry, and it is proposed rather than assumed.

**A `declaration_scope` has no home.** `workflow` and `swarm_roster` are per declaration scope; a batch
carries `declaration_scope`; a task's row in `data_model.md#concepts` carries none and no rule says how a
task acquires one, so the intake batch opened at creation has a `declaration_scope` from nowhere. Decision
70's ruling (2026-09-06) removes one candidate answer — the scope is not the planning record at the
`project` level, so a task does not acquire it from its ascent
(`planning_model.md#project-names-a-planning-level-the-scoping-key-on-a-declaration-is-a-different-term`).
The derivation that fits the model is that `classify` writes it, as it writes `action_type`; recorded as a
gap.

**The context entities have no rows.** `priority_rubric`, `release_criteria`, `brand_voice`,
`payment_profile`, `vendor_binding`, and `deployment_configuration` are read by declared steps
...
**Intake's `*` scope.** `workflow` declares "one entity per (declaration scope, workflow type)"; the intake and
...
| U-5 | `gates_and_workflows.md#findings-conclusions-and-what-a-blocking-finding-obliges`: a conclusion carries no condition | **closed** for the mechanical half, **R** for the prose, by the testability pass (revision 37): neither `verdict` nor `finding` declares a field a condition could be written in, so none can be written as one, and what a finding obliges is a task `REFERS_TO` it or an amendment to the acceptance criteria (U-8); the rule itself now says which half is reviewed (`gates_and_workflows.md#findings-conclusions-and-what-a-blocking-finding-obliges`). GW-22 reads it | — |
| U-6 | the same section: a blocking conclusion names its evidence | **closed** by the testability pass (revision 37): the `finding` row (`data_model.md#concepts`) — `severity`, `kind`, `scope`, `evidence`, `text`; `PART_OF` the verdict that carries it, `REFERS_TO` the batch it judges; evidence required on a blocking finding, whose write is refused without it (`migration.md`, G15 closed). GW-19, GW-23, GW-24, GW-25, and DM-27 read it; ruling 13's hold finding has a home (X-14 closed); the blocker the matrix named on decision 32 is gone, and the decision, since ruled, stood on its own question | — |
...
| U-9 | the same section: a step that reads a type it did not declare is a declaration error, caught in the pull request | **D**, definitional, for the runtime half (the testability pass, revision 37): the rule is by its own words a declaration-time rule, and GW-6 tests it there; at runtime the only enforcement point that could see a read is the record, which knows the principal and not the step, and the design keeps no read log by rule (`adapters.md#what-the-record-supplies-and-what-an-adapter-therefore-never-builds`). What binds a read at runtime is the grant, the outer bound (AU-21), and a read within the grant but outside the declaration leaves no artefact by the nature of a read. **Amended by revision 71 (2026-09-08):** the **D** stands for the declaration half, but its stated reason no longer holds for the grant half — decision 94 makes a read refusable per entity type, and a refusal is an artefact, so a read *outside the grant* is now testable at the point that refuses it and AU-21 carries it. What remains definitional is only the read inside the grant and outside the declaration, which nothing refuses and which therefore still leaves nothing | — |
...
| U-16 | `telegram.md#messages`: a message "reading as a new ask" is a task; one that "asks nothing" is an observation | **closed** by the testability pass (revision 37): every uncorrelated free-text message from a bound principal is a task, and intake's closing verdict names no successor where it asks nothing (`telegram.md#messages`; `telegram.md#which-checkpoint-a-reply-answers-is-decided-by-correlation-not-by-reading-the-text`). TG-1 reads it | — |
...
| U-19 | `adapters.md#what-the-adapter-does-with-every-event`: drops are counted per window | **closed** by the testability pass (revision 37): the window is declared on `vendor_binding`, and while the record is reachable the adapter writes one observation per window on its own `agent_session` carrying the dispositions counted (`adapters.md#what-the-adapter-does-with-every-event`). FP-3 and AD-12 read it | — |
...
| U-21 | `data_model.md#record-conventions`: a verdict is pinned to the artifact state it judged, by `head` | **closed** by the testability pass (revision 37): `artifact_refs[]` carries a per-kind pinned state, the kinds listed once in `data_model.md#record-conventions` — `head`; the message itself; the message set with its coverage; the dated fact; the declaration as read; the rail state as read — and a kind admitted later states its own under linkage. DM-6 reads it | — |
| U-22 | `workflows.md#security`: the closing verdict's only permitted successor is `release` | **closed** by the testability pass (revision 37): `none_permitted` beside `successors`; a closing verdict naming none under a declaration that does not permit it is refused at the write, and `security` does not permit it (`gates_and_workflows.md#sequencing-is-data-successors-and-the-chain`; `workflows.md#security`). WF-12 reads it | — |
...
| U-24 | `work_model.md#the-claim-and-the-lease-are-one-primitive`, read against the verdict rule | nothing says whether a verdict written by a step owner whose lease has lapsed closes the step; two runners of one role can each hold "the step owner's" conclusion, and the latest stands | **closed** by the ruling of decision 44 (2026-09-06): a `signed` or blocking verdict requires a held lease by its signer at the write, and a late one is refused (`#whether-a-verdict-from-a-step-owner-whose-lease-has-lapsed-closes-the-step`); the work-model axis's cell is mechanical |
| U-25 | `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`: attaching a task part-way through is a step owner's judgement written into that step's verdict | **closed** by the testability pass (revision 37): `tasks_attached[]` on `verdict` (`data_model.md#concepts`; `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`). WM-28 reads it | — |
...
| U-27 | `telegram.md#during-a-halt-a-read-on-the-channel-is-answered-with-the-halt-and-never-with-data`: the answer goes only to the chat the adapter's binding names, read at start and not from the record | **closed** by the testability pass (revision 37): the start-time binding is a cache with the staleness bound `vendor_binding` declares, refreshed on every successful read and `Indeterminate` past it, when the adapter answers no one (`telegram.md#during-a-halt-a-read-on-the-channel-is-answered-with-the-halt-and-never-with-data`). TG-12a reads it | — |
| U-28 | `work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`: the next instance is created "as part of" the closing verdict | **closed** by the testability pass (revision 37): the creation precedes the verdict and is read back first, the bounded retrieval makes a re-claim idempotent, and the closing interval is the one moment two instances are live (`work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`). WM-35a reads it | — |
...
| U-30 | `gates_and_workflows.md#declaration-batch-projection`: a contiguous named group of steps is a stage, and each step carries `phase`; `workflows.md#whether-a-stage-names-anything-a-step-does-not` | no rule reads a stage or a step's `phase` — no gate, conclusion, fast path, condition, successor, or checkpoint keys on either — so no state changes when one is wrong, and by principle 4's own test the field is decoration; the matrix confirms what open decision 33 argues | either a reader that keys on it, stated as a rule, or the retirement the decision proposes; the matrix is neutral between them, having nothing to observe under either |
| U-31 | `work_model.md#what-a-claim-predicate-treats-as-claimable`: the predicate reads the record's live status vocabulary onto `open`, `blocked`, or terminal | **closed** by the testability pass (revision 37): the registered `task` type declares its terminal set, one spelling per meaning; a closing verdict writes from it and a value outside it is refused; the reader stays tolerant permanently (`work_model.md#what-a-claim-predicate-treats-as-claimable`; `migration.md`, G7 closed). WM-11, WM-16, and WM-20 read it | — |
...
| U-33 | `workflows.md#meeting-processing`, the entry condition; `workflows.md#research-and-analysis` `persist` | **closed** by the memo-gap pass (revision 31): `REFERS_TO` is widened to task → a record entity it concerns, and to verdict → a record entity it read (`data_model.md#relationships`; `migration.md`, G12 closed); WF-19 and WF-25 read the edge. Kept so the number is not reused | — |
| U-34 | `workflows.md#outreach` `review`: the draft is judged "for what it discloses," with no observable named — the rule the minimization-recalibration pass's own method targets, a defect its broadened-capture purpose (`data_model.md#record-conventions`) makes load-bearing rather than incidental | **closed** by the minimization-recalibration pass (2026-09-06): a `disclosure_lint` step precedes `review` and fails on a mechanical check — a third party or a fact about one that the draft states, with no `link` edge from this task to that entity, is a finding, not a judgement (`workflows.md#outreach`; `workflows.md#what-link-attaches-and-what-it-leaves-to-hydration`). What `review` still judges — facts, voice, whether the reply answers what was asked — has no mechanical form and stays **R**. WF-16 reads it | — |
...
| X-3 | `data_model.md#relationships` `SIGNED_BY` verdict → agent against `gates_and_workflows.md#declaration-batch-projection` — only the operator principal writes `waived` | **closed** by the testability pass (revision 37): `SIGNED_BY` is verdict → principal — the step owner's agent, or the operator principal on `waived` (`data_model.md#relationships`); the verdict's `agent` and `agent_version` are absent on the operator's waiver. DM-1 reads the registry it implies | — |
| X-4 | `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow` — a batch opens on a closing verdict "and at no other moment" — against the same section's intake batch "opened on the task's creation" | **closed** by the testability pass (revision 37), tightened with decision 84: a batch comes into existence at one of two moments and no other — a workflow-entering task's atomic creation unit opens its intake batch without a predecessor verdict, and a closing verdict naming a successor opens every later batch (`work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`). WM-13 and WM-27 read both; WM-35 holds the aggregate-parent exemption | — |
| X-5 | `workflows.md#bug` — a bug needing a design choice closes without a successor "and [has] the task enter intake again" — against `workflows.md#intake`'s entry condition — "no task meets it twice" — and now against ruling 38 (`gates_and_workflows.md#closed-work-is-reviewed-on-the-record-and-redone-through-intake-never-reopened`), under which further work on closed work is a **new** task through intake | **closed** by the testability pass (revision 37): the bug section is rewritten under ruling 38 — the batch closes with no successor, its closing verdict carrying the finding that names the design choice, and the design work is a new task through intake referring to this batch's artifacts (`workflows.md#bug`). WF-11 reads it | — |
...
| PR-2 | `principles.md#2-a-write-that-reports-success-has-not-necessarily-happened-read-it-back` | `B0`, `RP` | every decision-carrying write in every other row | `RP` shows a decision write (verdict, resolution, confirmation, governance write) with no retrieve of the written entity from the same credential before its next write | M |
...
| PR-7 | `principles.md#7-unknown-stays-distinct-from-a-conclusion` | `B0`, `RP` failing reads | read gate, grant, drift, reachability, CI state under a failed read | any reader returns pending, clear, empty, or the zero value instead of `unknown` | M |
...
| WM-13 | `work_model.md#intake-is-every-tasks-first-workflow`: every workflow-entering task atomically gets one intake batch and `ADDRESSED_BY` at creation; an aggregate parent gets neither and is not claimable | `T1`; a batch-created task; a daemon-created task; an aggregate parent | read at creation, then after routing | a workflow-entering task is readable without its intake batch and `ADDRESSED_BY`; an aggregate parent gets either; a non-intake batch has no intake predecessor; or a task has two intake batches (X-5) | M |
| WM-14 | the same: every workflow-entering child task may take intake's fast path and never skips intake | a child created by a routed batch | read the child and its intake batch at creation | `link` or `dedupe` open; the child is readable without its intake batch; or the batch opened after creation | M |
| WM-14a | `work_model.md#what-distinguishes-a-task-being-assembled-from-one-intake-has-not-reached` (decision 84): every workflow-entering task atomically gets its intake batch and `ADDRESSED_BY` at creation; assembly adds the persistent `classify` hold; creation grants no lease; only the declaration-resolved `pm` step owner may claim it; the exclusion survives lapse, crash, return, and transfer; an ordinary task lacks the hold and stays claimable | non-PM creator; multiple contributors; ordinary task; PM renewal, crash, and transfer; early-exposure, delete-on-lapse, and creator-auto-grant mutants | read every boundary; evaluate ordinary and PM-only claims | a partial unit; creator privilege; ordinary exposure before `classify`; successor PM refused; ordinary task withheld; or a mutant stays green | M — effect-shaped; persistent-exclusion and creator-ownership mutants prove failure |
| WM-15 | `work_model.md#what-a-claim-predicate-treats-as-claimable` | tasks in every status the record holds, one with a lapsed lease, one assigned, one under an open `repeated_lapse` checkpoint, one of a batch under an open `unclaimed_step` | each agent evaluates claimability | a terminal task reads claimable; the task under `repeated_lapse` reads claimable; the step under `unclaimed_step` reads as not claimable to its role; a lapsed lease blocks; an assigned task reads claimable to another; the retired `blocked` status anywhere (X-17 closed) | M |
...
| WM-20 | `work_model.md#a-task-is-executed-only-through-a-workflow` | every terminal task after a full run | read the chain | a terminal task with no batch whose closing verdict carries a terminal-writing conclusion; a status write to `done` with no verdict behind it | M |
| WM-21 | the same: every daemon-created workflow-entering task enters intake with its intake batch at creation | `X(gmail)` delivers an untracked message | read the task at creation | the task is readable without its intake batch; a batch of another workflow; or a lease held by the daemon that created it | M |
...
| WM-27 | `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`: an intake batch opens at workflow-entering task creation without a predecessor verdict; every successor batch is opened only by a closing verdict | every batch after a full run | read creation units, `FOLLOWS`, and predecessor closing verdicts | an intake batch absent from its task's creation unit or carrying `FOLLOWS`; a non-intake successor batch with no `FOLLOWS`; a `FOLLOWS` target whose closing verdict's `successor` is not this batch's `workflow_type`; or a batch opened by a daemon, an adapter, or a sweeper credential in `RP` | M |
| WM-28 | the same: attach part-way is a step owner's judgement in a verdict; the task inherits the verdicts | a batch at step 3; attach a task | read the edge and the verdicts | an `ADDRESSED_BY` edge created after open that no verdict's `tasks_attached[]` names; the attached task's step state differs from the batch's | M (U-25 closed) |
...
| WM-31 | `work_model.md#a-batch-may-hold-on-a-condition-discovered-mid-flight`: an ordinary active hold is a finding naming the condition, no verdict, and a renewed lease; no held state and no field. Decision 84's assembly exception admits its creator-time `classify` finding before any owner or lease, and that finding derives the persistent exclusion rather than falsely asserting an active holder | `T-at(payment, pay)`, `X(rail)` returning `unknown`, `CLK`; an assembling task's atomic creation unit before a PM claim | the step owner holds; read the assembling task before claim | no finding naming the condition and what would resolve it; a verdict written; the ordinary lease not renewed; a held, paused, or waiting value on the batch or the task; the hold's duration not readable from observations on the finding; the finding not a `finding` entity `REFERS_TO` the batch with no `PART_OF` to any verdict; the creator-time assembly finding is refused or makes the absent creator read as holder | M (X-14 closed; decision 84 assembly exception) |
| WM-31a | the same: an ordinary active hold ends by verdict, checkpoint, or lapse, never by elapsed time into a pass; a hold owing nobody a decision is a rule-5 deferral. Under the assembly exception, lapse ends only the active hold: the finding's exclusion survives lapse, ordinary claims remain refused, the PM-only `classify` claim reopens, and only its verdict clears the exclusion | four ordinary holds: the condition resolves; it owes a decision; it never resolves and owes none; the holder dies. One assembling task whose PM lease lapses | wait | the first does not end in a verdict; the second raises no checkpoint; the third raises no `rounds_exhausted` carrying the finding at the ceiling; the fourth does not lapse and read claimable; any closes by time; the assembly finding disappears, the task enters the ordinary pool, a non-PM can claim, or the exclusion clears without a `classify` verdict | M (`hold_bound`; U-20 closed; decision 84 assembly exception) |
| WM-32 | `work_model.md#a-batch-may-depend-on-a-task-it-created`: a `DEPENDS_ON` edge, never a field; the verdict is refused while it is unended and the task non-terminal; ending it is a recorded act | a batch whose step creates a task and records the dependency | write the verdict before the task is terminal; end the edge; write again | the verdict lands unended; a `blocked_by` field or list exists; the edge lacks `created_at`; the end lacks `ended_at` | M |
...
| WM-32b | the same: a created workflow-entering peer task has its own intake batch at creation and priority; `DEPENDS_ON` is not its chain | as WM-32 | read at creation | the chain follows `DEPENDS_ON`; no intake batch; or priority moved with the dependency | M |
| WM-33 | `work_model.md#artifacts-are-records-a-batch-leaves-never-its-subject` | every artifact after a full run | read `system`, `external_id`, and every `CLOSES` edge | an artifact with a null or pending `external_id`; an artifact whose thing lives in the record (a draft, an analysis, a verdict); a verdict `CLOSES` an artifact | M |
...
| WM-35 | `work_model.md#parent-and-child-tasks`: an aggregate parent is not claimable, never enters a workflow, and has no intake batch or `ADDRESSED_BY`; its children are workflow-entering tasks | an aggregate parent with three children | claim the parent; complete two children | the aggregate parent reads claimable, enters a workflow, gets an intake batch or `ADDRESSED_BY`, reads complete before the third, or carries a stored parent status | M |
| WM-35a | `work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`: one live workflow-entering instance, never zero and never two; the closing verdict creates the next workflow-entering task with its intake batch at creation, `FOLLOWS` task to task, and the rule copied | a task with a `recurrence` rule, routed and completed; `RP` failing the creation write; `RP` failing the verdict after the creation, then a re-claim | read after the closing verdict | zero non-terminal instances, or two outside the closing interval; the new one lacks `FOLLOWS` to the completed one or its `recurrence`; the completed one's status ever leaves terminal; the new task is readable without its intake batch; under the failed creation the verdict landed; under the failed verdict the re-claim creates a second successor (U-28 closed: create, read back, then sign) | M |
...
| WM-38 | `work_model.md#whether-the-step-path-is-a-mechanism-of-its-own-and-what-the-engine-is-called` | — | — | P (decision 34, ruled; WM-36's observables are per credential and do not depend on the count or the name, so no row changes under either option) |
| WM-39 | `work_model.md#where-tasks-come-from-every-source-indexed`: every workflow-entering peer task from all nine sources gets its intake batch and `ADDRESSED_BY` at creation; an aggregate parent is not claimable and gets neither; decision 84's assembly exception adds the hold, and only the declared `pm` step owner gets a lease | each source, ordinary and assembling; aggregate parent | read provenance, first batch, hold, lease | unknown provenance; missing or wrong intake; parent with intake; ordinary with hold; assembly without hold; creator-only lease | M |
...
| GW-3 | the same: step state is derived; no step entity | `T-at(feature, arch)` | census; read step state | a per-step entity or status row; state that does not change when the lease or verdict does | M |
| GW-3a | the same: `step_status` is the projection of the batch's verdicts, proved equal to them by a reconciler; neither is deleted, neither is a second source of truth; no transition event type | `T-at(feature, arch)` signed; the projection mutated to disagree | read both; run the reconciler | the reconciler reports agreement, or nothing runs it; a reader takes the projection where it disagrees with the verdicts; a transition-event type in the census | M |
| GW-4 | the same: a rejected verdict write is an error, never swallowed | a verdict missing a required field | write it | the step reads signed; or the writer proceeds with no error in `RP` | M |
...
| GW-12 | the same: only a verdict closes a required step; no principal signs for another | `T-at(feature, arch)` | the `pr_review` owner writes a verdict on `arch` | it lands | M |
| GW-13 | the same: only the operator may waive; one `waived` per step; scoped to one batch's unsigned required steps | a batch with two unsigned required steps; a second batch | the operator waives; a step owner writes `waived`; the operator waives a signed step | the step owner's lands; fewer than two `waived` verdicts each naming its step and reason; a batch-level flag; the second batch changes; the signed step's conclusion changes | M |
...
| GW-17 | the same: no step is closed by elapsed time | a required step open, `CLK` past every declared interval | wait | the step reads anything but open; a checkpoint `unclaimed_step` carries a conclusion | M |
...
| GW-19 | `gates_and_workflows.md#findings-conclusions-and-what-a-blocking-finding-obliges`: findings bind; a contradictory conclusion is refused at submission | a verdict carrying a `finding` of severity `blocking` under `signed` | write it | it lands; the step closes | M (U-6 closed) |
| GW-20 | the same: three conclusion values | a verdict with `APPROVE` as its conclusion | write it | it lands | M |
| GW-21 | the same: a conclusion is terminal, never revised | a signed step | correct the conclusion in place; write a new verdict | the correction lands; the new verdict does not stand as latest per owner per head; the earlier is not readable | M |
| GW-22 | the same: a conclusion carries no condition | a verdict and a finding each written with a `conditions` field; a blocking finding whose remedy is neither a task `REFERS_TO` it nor a criteria correction | write; read | the field reads as declared on either (it lands in `raw_fragments`); the remedy exists on the record in no readable form | M for shape; R for a condition written in prose (U-5 closed) |
| GW-23 | the same: routing a remedy never transfers the conclusion; a decision-kind finding is not routable | a blocking `finding` of kind `decision_or_attestation` | a task is created from it; an implementer's artifact arrives | a task exists `REFERS_TO` the decision-kind finding; the step reads signed on the implementer's work with no owner verdict | M (U-6 closed) |
| GW-24 | the same: a blocking conclusion names its evidence | a blocking `finding` with no `evidence`; one naming a mechanism and the result it read | write | the first lands; the second is refused | M (U-6 closed) |
...
| FP-3 | rule 2: announce entering and leaving, aggregated per window | as FP-1, then restore; three blocked claims in one window | — | `CH` shows no announcement on entering; none on leaving; three messages for one window; the window not the one `vendor_binding` declares | M (U-19 closed) |
...
| FP-5 | rule 4: a mid-task write failure leaves the prior state; the lease lapses; no conclusion posted elsewhere | a step claimed; `RP` failing writes; `X(github)` logging | the owner's verdict write fails | the task's status changed; a comment or review on `X(github)` carrying the conclusion; a re-claim replays the conclusion rather than re-deriving it | M |
...
| FP-17 | the same: escalation reorders, never signs | a checkpoint on an unclaimed step | read the step | any verdict attributed to the escalating mechanism; the step closed | M |
...
| AU-10 | the same: the denied principal does not route around | as AU-9 | — | another principal's verdict on the step; a conclusion on `X`; a call under another credential | M |
...
| DM-6 | the same: a verdict is pinned; a later head does not invalidate automatically; staleness is derived | a signed step; the head moves | read | `artifact_refs[]` lacks the pinned state the kind names — the head, the message, the message set with coverage, the dated fact, the declaration, the rail state; the verdict changes; a stored stale flag | M for every kind (U-21 closed) |
...
| DM-9 | the same: schema versions; a verdict pins the agent version | a verdict | read | `agent_version` absent | M |
...
| DM-10b | `conformance.md`, schema drift mode 1 (decision 67): the substrate refuses a write naming an undeclared field, rather than accepting it into `raw_fragments` | a write with a field the registered type does not declare | write | the store returns 200 and the field is absent from the type, present only in `raw_fragments` | M — expected-failing (xfail) on every Neotoma checkout that returns 200 today, per principle 4; the row stays red until the store itself refuses the write at the API, and DM-10's client-side read-back check is not a substitute |
...
| DM-20 | `data_model.md#retrieval-contract`: the adapter never reads a `workflow`; the engine never reads an external system; the daemon reads no step state; the review step owner reads no host review state as verdict | a full run, `RP` and `X` per credential | — | any read outside the actor's column | M |
| DM-21 | `data_model.md#write-contract`: each actor's writes ⊆ its column | a full run, `RP` per credential | — | any write outside the column (X-7 closed: the adapter's carried resolution and verdict are in its column, A-for-B) | M |
...
| DM-27 | `data_model.md#concepts` `finding`: severity, kind, scope, evidence, text; `PART_OF` the verdict, `REFERS_TO` the batch; a hold's finding with no verdict; a task `REFERS_TO` the finding it was produced from; deliberately no conclusion, no condition, no discharged flag | `B0`, `T-at(feature, pr_review)` | write a blocking finding with no evidence; a blocking one with kind absent; a hold finding, then a verdict on the same step; a finding carrying a `discharged` field | either of the first two lands; the hold finding is not readable with no verdict, or its renewals are not observations on it; the `discharged` field reads as declared | M (U-6 and X-14 closed) |
| DM-28 | `gates_and_workflows.md#what-a-step-leaves-at-close-what-it-produced-and-a-reference-to-what-it-read` (decision 40, the verdict-provenance narrowing): a verdict's `REFERS_TO` → `session_digest`, required where `SIGNED_BY` resolves to an agent and a digest exists for the session that produced the verdict; permitted, never required, where the signer is not an agent (a human verdict carried in through an adapter, `adapters.md#what-the-adapter-does-with-every-event`) | an agent step owner's verdict, with a `session_digest` on prod for its session; a `waived` verdict from the operator principal, no session behind it | read the verdict's `REFERS_TO` set | an agent-authored verdict with a `session_digest` for its session and no edge to it; an edge fabricated onto a `waived` or human-through-adapter verdict with no session behind it; the edge pointing at `agent_session` or at raw `conversation`/`conversation_message` rather than `session_digest` | M |
...
| AD-3 | the same: a conclusion from `cred-owner` on its open step is a verdict; from `cred-other` or `cred-none` an observation; a CI result is never a verdict | per adapter, a conclusion-shaped delivery from each credential; a check result | deliver | the wrong outcome for any of the four; a check closes a step | M |
| AD-4 | the same: the adapter never invents a binding or resolves an unrecognized credential to the operator | `cred-none` delivers an approval | deliver | a resolution or verdict attributed to the operator | M |
...
| AD-15 | `adapters.md#an-artifact-exists-only-once-its-external-systems-entry-does-and-the-interval-before-that-belongs-to-the-action` | a send whose confirmation is lost | read | an artifact with null `external_id`; the action reads anything but `unknown`; on re-claim the adapter submits without reading `X` for the key | M |
...
| AD-22 | obligation 2: identity, with the negative test that goes red when the fallthrough returns | the sixth adapter; its identity rule mutated to resolve unknown credentials to the operator | deliver a conclusion from `cred-none` | the mutant does not turn AD-4 red | M (meta) |
...
| AD-28 | the same: the arch review step checks the six obligations before the grant | as AD-27 | — | the grant permitted with no `arch` verdict citing the adapter's document | M |
| AD-29 | `adapters.md#when-an-adapter-is-wrong`: a wrong verdict is findable by provenance; corrected by a new verdict, never deleted; withdrawal is revocation | a mis-mapped verdict | query provenance by adapter and window; revoke | not found; the verdict deleted; a capability survives revocation | M |
...
| AD-34 | `adapters.md#the-relationship-to-decision-15-which-rules-adapter-packaging` and `adapters.md#the-adapter-and-the-engine-are-two-roles` (ruling 15): the obligations hold wherever the code lives; the roles meet only in the record | as AD-1, AD-30 | — | as those; no row in this matrix names a repository | M (ruling 15 changes no observable, which is what the ruling says of itself) |
...
| AD-36 | `adapters.md#github`, `adapters.md#gmail`, `adapters.md#telegram`, `adapters.md#calendar`, `adapters.md#payments`: each section points at its document and names the two rules its system erodes hardest; an artifact with no batch never receives retroactive step state | `X(github)` delivers a pull request opened before any task exists | read the artifact; route the task it yields | any step state, `not_required`, `not_applicable`, or clear written on the artifact; the batch that later addresses it skipping a step on the strength of the artifact's age; its `impl` verdict not citing the existing pull request in `artifact_refs[]` | M |
...
| AD-39 | `adapters.md#per-agent-credentials-where-the-system-issues-them-a-shared-credential-where-it-does-not` and `adapters.md#a-per-agent-credential-is-an-obligation-where-the-system-issues-one` (decision 69): where an external system issues per-agent credentials, each agent acting on it holds its own; a conclusion-shaped delivery from a shared credential on such a system resolves to no single agent and is an **observation**, never a verdict | two agents bound to act on `X(github)`; the credentials the `vendor_binding` names for each | census the credentials per role; deliver a conclusion-shaped event from a credential two roles share | two roles resolving to one credential on a system that issues per-agent ones; a shared-credential conclusion landing as a verdict; two roles on one credential holding different grants on that system | M |
| AD-40 | `adapters.md#when-attribution-cannot-be-external-the-record-does-not-suffice-on-its-own`: an outbound effect taken under a shared credential carries a mark in the artifact disclosing that an agent acted, and an unmarked one is refused | `X(telegram)` or `X(mail)`, whose credential the binding names as shared; an outbound action of a class the policy does not exempt | take the action with the mark, then with it removed | the unmarked send taken; the refusal recorded as a `dropped` delivery rather than a denial before the effect; the mark present in a log and absent from the artifact the external reader sees | M for presence and refusal; R for whether the mark's wording is the right disclosure, which is the operator's policy |
| AD-41 | `adapters.md#where-the-binding-is-declared-no-new-home-is-needed`: the credential a role presents to a system's instance is a `vendor_binding` capability slot naming a secret, never carrying it; the grant stays matched on the credential | the `vendor_binding` entities and the registered `agent_grant` type | read both | a credential value stored on the binding; a credential named as a dimension of the grant; a per-system credential column on `swarm_roster`; a credential field on the `agent` | M |
| AD-42 | `adapters.md#what-separates-a-binding-from-a-deployments-configuration` (decision 35's distinguishing rule, the vocabulary pass of 2026-09-15): a `vendor_binding` addresses an external system the swarm does not own; a `deployment_configuration` carries out and verifies a procedure on the swarm's own software; a verification target is the mark of the second and no binding carries one | the registered `vendor_binding` and `deployment_configuration` types, and the declared steps that read each | read both type declarations, and census which entity type each declared read resolves | a step's declaration resolving a deploy command from a binding; a deployment consulting a drop window, a routing field, or a declared staleness bound; a field whose purpose is to prove the binding itself took effect; a third binding-shaped type admitted on a shape argument with no rule that reads it differently | M for the type separation and the census; R for whether a newly proposed type is one of these two under a second name, which is a reading of the rule |
...
read and judged") is **R**: the suite checks that the verdict exists, from the right owner, at the right
step, with its fields — never that the judgement was sound.

| # | Rule | Setup | Action | Red when | Class |
|---|---|---|---|---|---|
| WF-1 | `workflows.md#how-to-read-a-workflow-section`: a failing conclusion does not advance; `on_fail` reopens; the cap escalates | a blocking conclusion at `pr_review` | read | the batch advances; `impl` not open again; past the cap no `rounds_exhausted` | M |
...
| WF-10 | feature fast paths: taken only where the declaration scope declares no workflow of that type | a scope declaring `bug`; a bug-class task | route | the fast path taken | M |
...
| WF-14 | `workflows.md#social-content`: `draft_lint` deterministic; `consent` carries the `publish` checkpoint; `approved` only where the gate would not checkpoint | `T-routed(social_content)`, `X(chat)` | run | the lint's verdict differs from the script's result; the `publish` action created before lint passes; `approved` taken with a `HIGH` ungraduated class | M |
...
| WF-16 | `workflows.md#outreach`: the draft is in the record; `disclosure_lint` before `review`; `review` before `consent`; the checkpoint carries the full draft; `send` read back by message id; a change after consent is a new draft, re-entering at `disclosure_lint`; `follow_up` through the same gate | `T-routed(outreach)`, `X(mail)` | run; change the draft after consent; draft names a third party or a fact about one with no `link` edge from this task | `send` confirmed from the call's return; the `dedup_key` unchanged after the change so no new checkpoint; a follow-up send with no action; `disclosure_lint` fails and the batch does not reach `consent` | M (`disclosure_lint`'s check closed the review-only half U-34 named); R for `review`'s remaining judgement on facts, voice, and scope |
...
| WF-24 | `workflows.md#whether-a-stage-names-anything-a-step-does-not` | — | — | P (decision 33, ruled; finding U-30 points at it, and the matrix observes nothing under either option) |
...
each (`PL1` under `OB1`), and never a record's text; a row reads edges, counts, actions, and verdicts.
...
| PM-12 | `planning_model.md#which-levels-an-instance-declares-and-what-it-calls-them` | — | — | P (decision 57, ruled; the design reads the mark and never a level by name, so no row goes red under any answer); decision 58 is ruled and tested under PM-9's operator case |
...
| # | Rule | Setup | Action | Red when | Class |
|---|---|---|---|---|---|
| GH-1 | `github.md#reviews-review-comments-and-threads`: `APPROVE` is a verdict only through identity; an automated account's never stands in | `T-at(feature, pr_review)`, `X(github)` | `APPROVE` from `cred-owner`, `cred-other`, `cred-none`, an automated account | any but the first yields a verdict; the first yields none | M |
| GH-2 | the same: the operator's `APPROVE` while a merge checkpoint awaits is a resolution, not a verdict | a held `merge_pr` action | `APPROVE` from `cred-op` | no resolution read back; a verdict | M (X-7) |
...
| GH-7 | the same: a force-update leaves verdicts pinned and readable as stale; nothing unsigns | a signed `pr_review`; force-update | read | the verdict changed; staleness not derivable; the steward merges without reading it | M |
| GH-8 | the same: a dismissed review is an observation, never an unsigning | dismiss | read | the verdict changed | M |
...
| GH-16 | `github.md#the-property-that-makes-this-a-control-and-not-a-list`, `github.md#what-the-outcomes-are-and-the-rule-against-a-fifth`: an event outside the tables is `dropped` with reason `unmapped` and counted; no fifth outcome; an event that invalidates a decision is an observation, and the pinned verdict reads stale | deliver an event type the tables do not name; a force-update after a verdict | read | the delivery has no disposition, or its drop is uncounted; an outcome type outside the four; the verdict changed | M (AD-21 and GH-7 at this host; kept because the document states it as its own control) |
| GH-16a | `github.md#issues` "What a verdict pins": a permitting conclusion on an issue, followed by `issues.edited` rewriting the body, must not be readable as a live conclusion with no stale or refuse signal — the stale-**permit** shape (decision 106), distinct from the stale-**hold** shape GH-7 covers at the pull request | a signed step whose verdict cites an issue in `artifact_refs[]`, the conclusion permitting; `issues.edited` rewrites the body afterward | read the conclusion through whatever surface presents it as current | the surface presents the conclusion as live with no stale or refuse signal naming the pinned state, the live state, and the resolving act; `issues.edited` lands with no observation at all; a stored `stale` flag appears in place of a derived comparison | M — **red today**: decision 106 is open and `github.md#issues` states no ruled pin for kind `issue` to compare against, so no derived read exists yet; the row stays red until decision 106 is ruled and a pin (or its dissolve-and-state-under-linkage) lands, at which point this row is the effect check for whichever candidate is chosen |
...
| CA-7 | `calendar.md#a-series-and-its-occurrences-are-each-artifacts-related-by-part_of`: both levels are artifacts, the occurrence `PART_OF` the series; a signal lands on the unit whose id it carries; a rule change is an observation on the series and every held occurrence is re-read before a step depends on its time; a verdict on an occurrence pins a dated fact, on the series the declaration as read | an occurrence modified independently; a rule changed; a task against one occurrence; a step depending on that occurrence's time | deliver; read | the occurrence lands on the series or is not minted `PART_OF`; the rule change lands on an occurrence; the step opens without the re-read; the task's reference moves | M |
...
| PY-8 | `payments.md#outbound-the-operations-a-step-takes-on-a-rail`: `payment` is in the never-set and never graduates; the adapter never writes `reconcile` | a hundred confirmed payments | the hundred-and-first | taken without a checkpoint; a `reconcile` verdict from the adapter's credential | M |
| PY-9 | `payments.md#what-the-adapter-refuses-and-why` | a full run | `X`, `RP` | any of the ten | M |
| PY-10 | `payments.md#a-payments-approver-is-shown-exactly-what-the-verifier-signed` | `T-at(payment, consent)`; then parameters changed before `pay` | read the checkpoint; take | `needed_input` lacks payee, amount, currency, period, rail, or the verifier's identity, differs from the `verify` verdict, or does not refer to it; `pay` taken on parameters differing from what the checkpoint carried | M |
...
| # | Rule | Setup | Action | Red when | Class |
|---|---|---|---|---|---|
| MG-1 | `migration.md#dispositions-and-the-primitive-that-carries-each`: no entity deleted and recreated; no conclusion written for a principal that did not write it; no field bulk-rewritten to a new spelling; no write to an entity a process that does not know the new shape is still writing | `LEG` | run leg two | a legacy id that stops resolving; a `verdict` attributed to the agent a step record names; a bulk correction of a field's spelling; a write to a type whose retired writer is live (MG-8) | M |
...
- **Identity × the conclusion-shaped delivery, for every adapter, from all four credentials** (twenty). The
  verdict is the only dangerous outcome and identity is the only thing that decides it, and each system
...
- **Redelivery × the decision-carrying outcomes, for every adapter** (ten): a redelivered verdict and a
...
- **A credential in neither role × every outcome that is not a verdict**: dropped, because the identity
...
| the record unreachable mid-task | the prior state; the lease lapses; re-derived on return (FP-5) | a conclusion parked on the host, on disk, in a chat, and replayed |
...
- **Lapse while a step is claimed**: the step reads open (claimable again); a second runner of the same
  role claims; the first runner's late verdict is refused — decision 44, ruled 2026-09-06 (U-24 closed):
  the late write does not land, the second runner's conclusion is the one that can stand, and two conclusions from
...
- **Renewal past the step's close**: a verdict written, then the lease renewed — the lease on a signed
...
- **A discovered condition, a dependency the batch created, a cycle** (rulings 13 and 14): a hold × each of
  its three ends (WM-31, WM-31a); a dependency × a verdict attempted before and after the edge is ended
...
CA-7); what a verdict pins at each level — a message outright, a thread's message set with its coverage, an
...
schedule. Kept: **duplicate-on-completion** — the closing verdict of the last batch creates exactly one
instance, with the edge and the copied rule (WM-35a), and its failure modes, a write that fails between the
creation and the verdict in either order (U-28 closed: create, read back, then sign); **the schedule-computed due date** across three cases — on time, late within the
...
| Removed | Rows that cited it | Conclusion |
|---|---|---|
...
| `hot path`, retired for the projection's own definition | none, and that was the finding | **lossless, and it exposed a missing row.** No row had cited the term, and the rule the term sat beside — the projection is proved equal to the verdicts by a reconciler, and neither is a second source of truth — had no row at all; GW-3a is added, and its artefact (a reconciler that reports agreement on a projection mutated to disagree) never depended on the term |
...
- **32, the `conclusion` field.** GW-19 (a contradictory conclusion refused at submission) and GW-20 (three
  ...
  its artefact through the verdict's immutability. The guarantee moves to two places: DM-1 would refuse a
  `conclusion` field on the registered type, and a derived conclusion would rest on the finding's shape — which
  ...
  (`gates_and_workflows.md#whether-the-conclusion-is-a-stored-field-or-a-read-over-the-findings-and-the-author`);
...
- **35, one binding type or two.** Four rows read a binding — TG-6 (which chat receives what), CA-6 (which
  ...
  `channel_config` (since retired) and `vendor_binding` differently, which is the condition the pass named as deciding it.
...
### Whether a verdict from a step owner whose lease has lapsed closes the step

**Ruled (decision 44, 2026-09-06): a verdict carrying `signed` or a blocking conclusion requires a held lease
by its signer at the moment of the write.** Registered as ruled in
`conformance.md#the-register-of-open-design-decisions`. A verdict written on a step whose lease the signer
does not hold — lapsed, returned, or held by another runner — is refused at submission, an error and never
swallowed, and the current lease holder's conclusion is the one that can stand. The record cannot hold two
conclusions from two runners on one step at one head. The operator principal's `waived` is the one verdict
written without a lease: it closes a step the operator does not own and never claimed, and it is already the
one conclusion a principal other than the step owner may write
(`gates_and_workflows.md#declaration-batch-projection`), so its rule is its own and this one does not reach
it.

**The question.** A step is claimed with a lease and closed by its owner's verdict, and the two are
stated separately. Nothing says the second depends on the first. So a runner whose lease lapsed, and whose
step a second runner of the same role has since claimed, may still write a verdict — and under "the latest
per step owner per artifact head stands", both conclusions are the step owner's and the later one wins,
whichever runner wrote it.

**If a verdict requires a held lease by the signer at the moment of the write**, the lease becomes
load-bearing for correctness and not only for exclusion: the late runner's write is refused, the second
runner's stands, and the record cannot hold two conclusions from two runners on one step at one head. The cost
is that a slow but honest verdict written a second after expiry is refused and its work re-derived, which
is the at-least-once cost the design already accepts for actions.

**If a verdict is accepted regardless of lease state**, the lease is purely about who works and never
about whose conclusion counts; two runners' conclusions on one head are both the owner's and the latest stands.
The cost is a zombie writer: a runner the swarm believed dead writes a conclusion on work it read before the
head moved, and the pinning rule catches the head but not the runner.

**What would decide it:** whether the design wants the lease to be the thing that makes a conclusion the
*current* owner's, or whether attribution to the role is enough. The claim primitive's own rationale —
"two agents must not both take a task" — reads as the first; the verdict rule's silence reads as the
second.

**Why the lease is load-bearing for the conclusion.** The claim primitive's own rationale is the first
...
holder). A verdict is the close of work held under a lease; a close from a lease not held is a close from
...
step owner whose write failed does not replay its conclusion later — "the work is re-claimed when the record
returns … The conclusion is re-derived then, not replayed" — because the artifact may have moved under it; a
late verdict from a lapsed lease is that replayed conclusion arriving by another road, and the pinning rule
...
making the conclusion depend on it is a read at the write — is the signer's lease `held` on this step now — and
not a field, a flag, or a process. The other branch would have left the lease as exclusion only, and let a
runner the swarm believed dead write a conclusion on work it read before the head moved.

**Cost accepted.** An honest verdict written a second after its lease expired is refused and its work
...
wants its verdict to land renews its lease until the write returns, which is what every claimed step already
does between claim and verdict.

**What would reopen it.** A step whose verdict must be writable by a principal that cannot hold its lease.
None is known: the operator's `waived` is the one such write, and it is excepted above by the rule that
already governs it.

**Matrix.** U-24 closes and the work-model axis's one missing cell fills: a late verdict refused is a
mechanical row, and two conclusions on one head from two runners is its failing artefact.
...
pass against the matrix are this revision's, and the conclusions in that section are read from the rows and
