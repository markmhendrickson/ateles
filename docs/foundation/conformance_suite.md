...
stand rather than removed. The operator's standing instruction
...
built elsewhere; the open decisions (31 to 36, and 43 and 44 below), whose rows are designed per candidate and held
...
test), and the suite cannot make a person read. The count of **R** and **U** rows is a measure of how much
...
|---|---|---|---|
| U-1 | `principles.md#3-validate-the-instrument-before-believing-the-measurement` | self-declared review-only; nothing names which instruments the swarm has | an enumeration of the swarm's counters (drops per window, lapses per task, blocked claims per window) each with a planted-positive check; the suite applies exactly this to its own three instruments |
| U-2 | `principles.md#6-extend-the-mechanism-that-already-generalizes-do-not-build-a-parallel-one` | "do not build a second one" has no observable unless the first is enumerated; a prompt contract binds weakly and the design-basis check checks a citation exists | a closed list of the design's singletons — one queue (`checkpoint`), one gate, one lease primitive, one engine, one step home — with the registry-closure observable: the registry holds no second type for a listed concept. The matrix tests that narrower rule (row DM-19) |
| U-3 | `principles.md#8-every-figure-carries-its-date-and-its-instrument-re-measure-before-acting`, and `conformance.md#phases-and-implementation-state`'s rule that a foundation document never carries state | the same token (an issue number, a date) is a decision record in one sentence and a state claim in the next, and only sense distinguishes them | a syntactic marker for a decision citation, so that a bare issue number or hash outside the marker is a lint failure |
| U-4 | `principles.md#10-handing-work-to-the-swarm-is-not-completion` | the document itself says the owning session "is a person or an agent, not a control" | a rule that a task whose batch produced a `merge_pr` action is terminal only when a `release` batch's `verify_deployed` is signed, or the project's policy declares it deploys its default branch — which makes "landed" a derived read over the chain |
| U-5 | `gates_and_workflows.md#findings-verdicts-and-what-a-blocking-finding-obliges`: a verdict carries no condition | a condition is prose inside a verdict or a finding; no field can carry one, so none can be refused | either a `conditions` field that the sign-off schema does not declare (so the shape fails closed) together with a lint over finding text for the conditional forms the section names, or acceptance that the rule is review-only |
| U-6 | the same section: a blocking verdict names its evidence | the sign-off row in `data_model.md#concepts` carries no findings at all, so severity, kind, standing axis, and evidence have no recorded shape; a "blocking verdict that names no executed check" cannot be refused because nothing carries the check | a `findings[]` shape on `sign_off` — severity, kind (implementation-only / decision-or-attestation), standing (one-off / standing / undetermined), and an `evidence` reference — after which rows GW-19 to GW-24 become mechanical |
| U-7 | `gates_and_workflows.md#a-finding-is-one-off-or-standing-and-a-standing-one-obliges-a-change-to-what-produced-it`: where a standing finding lands is decided by its specificity, narrowest first | a judgement; the fail-closed branch (`undetermined_scope`) is testable, the correct branch is not | nothing can make the judgement mechanical; the rule needs to say that the chosen scope is recorded on the proposed change with the finding it came from, so that a wrong choice is at least attributable and reviewable |
| U-8 | `gates_and_workflows.md#declaration-batch-projection`: scope amendment versus scope creep, three conditions | "disclosed", "escalated", and "defended" leave no record the design names; the only readable trace is the pinned head diverging | an amendment as a recorded thing: a correction to the batch's acceptance criteria attributed to the step owner, citing the finding it answers, so that a shipped change beyond the criteria with no such correction is the failing artefact |
| U-9 | the same section: a step that reads a type it did not declare is a declaration error, caught in the pull request | at runtime, nothing in the design gives a reader a per-read audit; at declaration it is checkable | at declaration: `reads_to_enter` ⊆ the step owner's `agent.context_entity_types[]` ⊆ the grant, checked when the declaration is written (row GW-6). At runtime the rule needs the record proxy's request log, which is the suite's instrument and not the design's |
| U-10 | `failure_posture.md#the-operator-invoked-halt-and-what-undoes-an-action-already-taken`: every action class names its recovery | recoveries live in prose tables across three documents and nowhere in the record; a class added to a policy with no recovery row fails nothing at runtime | a `recoveries` map on `action_policy` — class → recovery class, `forward_only`, or `none` — so that a listed class with no entry is refused at the policy write |
| U-11 | the same section: the restore obligation with a stated cadence | "declared where the path is declared" — recovery paths are declared nowhere in the record | a `recovery_path` entity with a declared cadence, and exercise observations dated by the exercise; "overdue" is then a derived read over them, never a maintained field |
| U-12 | `work_model.md#the-four-execution-mechanisms`: the interactive session holds no lease | definitional; the document says requiring a claim would be bypassed | the recovery half is testable: a `session_digestion` declaration exists and an interrupted session's transcript has a digest task. The "holds no lease" half stays a statement |
...
| U-14 | `gates_and_workflows.md#two-questions-who-may-claim-a-step-and-whether-an-action-may-be-taken`: governance writes are actions | ruling 41 (`authority_model.md#grants`) names the enforcement point for **admission** — a write to a type lands only where the writer's grant names the type, checked at the write — and AU-21 tests it. What it does not name is the point that ties an admitted governance write to a **permitted action**: a principal whose grant admits `workflow` may still write a declaration with no `action` behind it, and the record knows nothing of the gate. The writer, self-enforced, is a prompt rule (weakly binding) | the second enforcement point named: by principle 1's placement test it sits at the write, so either the record refuses a write to a governance type that carries no reference to a permitted `action`, or a proxy in front of it does. Until then the suite's observable for that half is audit-shaped (an observation on a governance type with no action naming it, row GW-33), which detects after the fact and refuses nothing |
| U-15 | `workflows.md#meeting-processing` `extract`; `gmail.md#what-this-adapter-refuses` refusal 1; `calendar.md#what-this-adapter-refuses` refusal 1: minimization at capture, nothing incidental or sensitive | a content judgement over free text; "sensitive" has no mechanical form | a declared field allowlist on `contact` writes from those steps and adapters, so that a write outside the allowlist fails closed on shape; the semantic half stays review-only and says so |
| U-16 | `telegram.md#messages`: a message "reading as a new ask" is a task; one that "asks nothing" is an observation | deciding whether text asks something is the intent read the same document refuses | every uncorrelated free-text message from a bound principal is a task for intake, and intake's `classify` closes it with no successor where it asks nothing — which is the design's own answer for every other surface |
...
| U-18 | `failure_posture.md#the-rules` rule 2: the announcement travels a path that survives the outage | nothing says what happens when the announcement path is itself unreachable; a swarm halted with its channel down is indistinguishable from an idle one, which is the failure the rule names | a second path, or a rule that the halt's local-disk capture (rule 1) is the announcement of last resort and is read by something |
| U-19 | `adapters.md#what-the-adapter-does-with-every-event`: drops are counted per window | the window's length is declared nowhere, and the count lives only in the announcement; nothing in the record holds it | the window declared on `channel_config`, and the per-window count written to the record as an observation when the record is reachable, so that "a number that should be zero" is a read and not a message |
| U-20 | `failure_posture.md#repeated-lapse-raises-a-checkpoint`; `gates_and_workflows.md#declaration-batch-projection` (the hold is bounded); rule 5 (every deferral has a ceiling) | the per-task lapse cap, the hold bound, and the deferral ceiling are declared nowhere; only the unclaimed-step interval has a home (the workflow). A test cannot know when the checkpoint is due | each bound declared on the workflow declaration beside the unclaimed-step interval, or on the policy, and an undeclared bound treated as the unclaimed-step rule treats its interval: no checkpoint, and the absence visible |
| U-21 | `data_model.md#record-conventions`: a sign-off is pinned to the artifact state it judged, by `head` | `head` is defined for a pull request; rulings 23 and 24 define what is pinned for a message (itself), a thread (the message set the read returned, with its coverage), an occurrence (a dated fact), and a series (the declaration as read); a transfer and a chat message have no pinned state the design names, and `artifact_refs[]` still carries one `head` where a set is what is pinned | a per-kind pinned state on `artifact_refs[]` — a set for a thread, a rail state for a transfer — in place of the one `head` field, so that the derived staleness read has a value to compare on every kind |
| U-22 | `workflows.md#security`: the closing sign-off's only permitted successor is `release` | `successors` lets a closing sign-off select one or none, always; a declaration has no way to forbid none, so the security constraint has no home and cannot be built from the declaration | a `none_permitted` value on `successors`, or a required successor on the workflow, so that a security batch closing with none is refused at the write |
| U-23 | `workflows.md#intake` `dedupe`: a duplicate closes terminal with an edge to the task it duplicates | the edge has no type in `data_model.md#relationships`; the record's own vocabulary already carries `DUPLICATE_OF`, which `migration.md#ordering-dependencies-and-what-each-stage-depends-on` counts among the types the record has, so the gap is the design's naming and not the record's | a `DUPLICATE_OF` row, task → task, in the relationships table |
...
| U-25 | `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`: attaching a task part-way through is a step owner's judgement written into that step's sign-off | the sign-off row carries no field for an attach judgement, so an `ADDRESSED_BY` edge written mid-batch with no sign-off naming it cannot be distinguished from one written with | a `tasks_attached[]` field on `sign_off`, after which row WM-28 is mechanical |
| U-26 | `gates_and_workflows.md#declaration-batch-projection`: an optional step's `applies_when` is evaluated against what the change touches | an optional step declared *before* any artifact exists — `ux` on `copy`, step 3, before `impl` — can never evaluate its condition, so it always takes the third value and always opens; its optionality is vacuous, and no test can make it inapplicable | either `applies_when` may read the task set as `fast_paths` does, or an optional step is placed after the artifact it reads exists, and the declaration check refuses the other placement |
| U-27 | `telegram.md#during-a-halt-a-read-on-the-channel-is-answered-with-the-halt-and-never-with-data`: the answer goes only to the chat the adapter's binding names, read at start and not from the record | a binding read at start is a cache, and `authority_model.md#grants` requires a cache to declare its staleness bound and to resolve to `Indeterminate` past it; this one declares none, so a chat unbound since start is still answered | a declared staleness bound on the start-time binding, past which the adapter answers no one, or a statement that the announcement path's own binding rule covers it |
| U-28 | `work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`: the next instance is created "as part of" the closing sign-off | the closing sign-off is one write and the task creation another; nothing says they are one atomic write, so a sign-off that lands and a creation that fails leaves a completed series with zero live instances — the never-zero rule violated by a partial write, and readable only as an absence | either the creation is read back before the sign-off is written (so a failed creation leaves the step open), or the recurring task's terminal status is written by the creation's read-back and not by the sign-off; the suite tests the first as the fail-closed reading (WM-35a) and records that the rule does not say which |
...
| U-31 | `work_model.md#what-a-claim-predicate-treats-as-claimable`: the predicate reads the record's live status vocabulary onto `open`, `blocked`, or terminal | the reader's half is testable (WM-15, WM-16); the writer's is not: the design names no canonical terminal spelling, so no row can say which value a closing sign-off writes, and every new writer may choose its own (`migration.md`, G7) | a declared terminal set on the registered `task` type, one spelling per meaning, with the tolerant reader kept permanent |
| U-32 | `data_model.md#write-contract`: a self-triggering daemon writes the tasks its poll produces and observations carrying its provenance, and nothing else; `failure_posture.md#the-rules` rule 2: a silently halted swarm is indistinguishable from an idle one | jointly unobservable: with no daemon report, a daemon whose poll succeeded and found nothing and a daemon that stopped polling write the same nothing, so no row can go red on a stopped daemon short of the halt announcement, which a daemon that died never sends (`migration.md`, G13) | what a successful empty poll writes — an observation on the daemon's own `agent_session` carrying the poll's coverage is the shape the contract already admits — so that a daemon silent past its interval is a derived read |
...
| X-3 | `data_model.md#relationships` `SIGNED_BY` sign-off → agent against `gates_and_workflows.md#declaration-batch-projection` — only the operator principal writes `waived` | a schema from the data model refuses a sign-off `SIGNED_BY` an `operator`; the waiver rows require it | registers `SIGNED_BY` → principal and reports |
| X-4 | `work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow` — a batch opens on a closing sign-off "and at no other moment" — against the same section's intake batch "opened on the task's creation" | one reading forbids a batch with no `FOLLOWS` and no closing sign-off behind it; the other requires exactly one such batch per task | tests the second (the intake exception) and reports the wording |
| X-5 | `workflows.md#bug` — a bug needing a design choice closes without a successor "and [has] the task enter intake again" — against `workflows.md#intake`'s entry condition — "no task meets it twice" — and now against ruling 38 (`gates_and_workflows.md#closed-work-is-reviewed-on-the-record-and-redone-through-intake-never-reopened`), under which further work on closed work is a **new** task through intake | one row requires a second intake batch on the same task; another requires its refusal | tests the entry condition and ruling 38 (a new task, referring to the closed batch's artifacts) and reports the bug section's wording, which ruling 38 makes a correction rather than a derivation |
| X-6 | `github.md#issues` — `assigned_to` "is written only by intake's `classify`" — against `work_model.md#assignment-restricts-eligibility-it-never-creates-a-lease` and `scenarios.md#c-assignment-then-the-named-principal-claims` — a principal writes `assigned_to`, "a field write like any other" | one row refuses an `assigned_to` write outside `classify`; another requires it to land | tests the work model's reading (WM-4) and reports |
| X-7 | `data_model.md#write-contract` — an adapter writes observations, confirmations, tasks, and drops — against `adapters.md#no-external-event-advances-a-step-by-itself`, `github.md#reviews-review-comments-and-threads`, `gmail.md#messages`, `telegram.md#callbacks-and-queries` — an adapter writes a checkpoint resolution attributed to the operator | a per-actor write allowlist built from the contract flags every adapter-carried resolution; the adapter rows require them | tests the adapter documents' reading and reports the contract row as incomplete (its own read-back column already names "a checkpoint resolution") |
| X-8 | `data_model.md#concepts` — an `action` is `PRODUCES` ← task and `gates_and_workflows.md#the-action-gate-is-pr-independent` — the checkpoint "keys on the action and its task" — against `work_model.md#contradictions-this-document-settles` C2 — a daemon's effect is an action through the same gate and passes through no task | one row requires every action to have a task; the daemon rows require an action with none | holds the daemon-action rows as **P** on C2, which the work model leaves open |
| X-9 | `adapters.md#what-the-adapter-does-with-every-event` — an outbound operation the adapter cannot take for want of a credential resolves to `dropped` — against the same document's rule that an unconfirmed effect reads `unknown`, and `authority_model.md#grants` — a denied capability raises a checkpoint `capability_denied` | three observables for one situation: a drop counter increments; the action reads `unknown`; a checkpoint exists | tests the authority model's (AU-9), because `dropped` is defined as a *delivery's* disposition (`vocabulary.md#disposition`) and an outbound action is not a delivery; reports |
| X-10 | `gates_and_workflows.md#one-step-set-defined-once-tested-for-parity` — "a data-sourced list may add steps and never remove one" — against `gates_and_workflows.md#an-unreadable-workflow-is-unknown-and-unknown-holds` and `failure_posture.md#contradictions-this-document-settles` C5 — nothing proceeds on an empty sequence, no floor | "add, never remove" presupposes a base list the data adds to; if the base is in code, an unreadable declaration would open its steps, which the second rule forbids | tests the second (GW-42: zero steps open) and reports the first as having no observable under it |
| X-11 | `data_model.md#concepts` `workflow` key fields (no `reads_to_enter`, `reads_to_close`, `freshness`, `applies_when`) and `sign_off` (no findings) against `gates_and_workflows.md#declaration-batch-projection`, which declares all of them, and the data model's own rule that a concept with no row is not persisted | a registry from the table cannot hold what the rules require declared | registers the declared fields and reports the table stale |
| X-12 | `workflows.md#feature` `impl` closes on a pull request existing with CI green, against `github.md#conditions-that-are-not-events` — the design's position is that `impl` closes on the pull request "existing and being mergeable" | the observable that closes `impl` differs by document | tests the workflow table's and reports |
| X-13 | `failure_posture.md#checkpoints-on-tasks-one-queue-one-protocol` — the `unclaimed_step` checkpoint is "routed against the owner role" and "awaits the operator" — against `data_model.md#concepts` — `AWAITS` → principal | a role is not a principal; an edge to a role has no target | tests `AWAITS` → the operator and reports |
| X-14 | `work_model.md#a-batch-may-hold-on-a-condition-discovered-mid-flight` — a hold is a non-blocking finding written with no sign-off and renewed by observations on it — against `vocabulary.md#sign-off` and `gates_and_workflows.md#findings-verdicts-and-what-a-blocking-finding-obliges` — findings are what a sign-off carries — and `data_model.md#concepts`, which gives a finding no row | one row requires a finding to exist, and be renewed, before any sign-off; the data model has nowhere to write it and the finding rule ties it to a sign-off | tests the hold's other observables (the held lease, no sign-off, no held field) and reports the finding half as unrecordable — U-6, made unavoidable by ruling 13 |
| X-15 | `work_model.md#a-batch-may-depend-on-a-task-it-created` — "the record refuses a `DEPENDS_ON` write that would close a cycle", its hierarchical types being cycle-checked at write — against the same section's own walk, which runs `DEPENDS_ON` then `ADDRESSED_BY` and is "a read the writer makes before it writes" | a loop that crosses `ADDRESSED_BY` is invisible to a per-type check, so the record cannot refuse it; only the writer's walk can, and the section attributes the refusal to both | tests the writer (WM-32a) and names the writer as the enforcement point, which is the U-14 shape again |
...
| X-17 | `work_model.md#what-a-claim-predicate-treats-as-claimable` and `work_model.md#the-transition-vocabulary` — `blocked` is a task status and a `blocked` task is not claimable — against `failure_posture.md#checkpoints-on-tasks-one-queue-one-protocol` and `gates_and_workflows.md#the-checkpoint` — a task the swarm cannot advance is held by a checkpoint, and on resolution "the task is re-claimed or closed" | for one task awaiting a principal, WM-15 needs `blocked` to make it not claimable while GW-48 and WM-19 need the open checkpoint alone to hold it with the status unchanged; nothing says which conditions write `blocked`, what clears it, or whether a resolution does — and if a resolution does, `blocked` is a projection and the design requires of it a reconciler it does not name | tests the checkpoint's hold (status unchanged; claimability read from the open checkpoint) and reports `blocked` as a status with no named writer (`migration.md`, G8) |
| X-18 | `workflows.md#meeting-processing` — *Artifacts: the transcript file* — against `work_model.md#artifacts-are-records-a-batch-leaves-never-its-subject` — a local file is reached through no adapter and has no `system` | WM-33 turns red on an artifact minted for the transcript; a disposition test generated from the workflow's Artifacts line would require one | tests the work model's (WM-33) and reports (`migration.md`, G11) |
...
| PR-3 | `principles.md#3-validate-the-instrument-before-believing-the-measurement` | the three instruments | plant one drop, one lapse, one request, one call, one announcement | any instrument reads zero on its planted positive | M (suite-level); U-1 (swarm-level) |
...
| PR-8 | `principles.md#8-every-figure-carries-its-date-and-its-instrument-re-measure-before-acting` | the documents | lint for a figure with no date | — | U-3 |
...
| PR-10 | `principles.md#10-handing-work-to-the-swarm-is-not-completion` | `T-at(feature, merge)` signed naming `release` | read the task's terminal status before `verify_deployed` is signed | the task reads terminal | M under the narrowed rule U-4 proposes; U as written |
...
| WM-4 | `work_model.md#assignment-restricts-eligibility-it-never-creates-a-lease` | `T1` | a principal writes `assigned_to` = B | a `LEASE` edge exists; or B's claimability read is anything but true and A's anything but false | M (see X-6) |
...
| WM-11 | `work_model.md#the-transition-vocabulary` | `T1` through a full life | read every status observation | a status value outside `open`, `blocked`, or the declared terminal set — a liveness word, a routing word, a queue word, `in_progress` | M |
...
| WM-15 | `work_model.md#what-a-claim-predicate-treats-as-claimable` | tasks in every status the record holds, one with a lapsed lease, one assigned | each agent evaluates claimability | a terminal or `blocked` task reads claimable; a lapsed lease blocks; an assigned task reads claimable to another | M |
...
| WM-25 | the same: bootstrapping | `B0` | — | — | U-13; **43** |
...
| WM-35a | `work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`: one live instance, never zero and never two; the closing sign-off creates the next, `FOLLOWS` task to task, the rule copied | a task with a `recurrence` rule, routed and completed; `RP` failing the creation write | read after the closing sign-off | zero or two non-terminal instances; the new one lacks `FOLLOWS` to the completed one or its `recurrence`; the completed one's status ever leaves terminal; the new one has no intake batch; under the failed creation the sign-off landed (U-28) | M |
...
| WM-36 | `work_model.md#the-four-execution-mechanisms`: a daemon receives no task; the pipeline never writes task status | a full run | `RP` per credential | a daemon credential holds a lease; the engine credential writes `task.status` | M |
| WM-37 | the same: the interactive session | — | — | U-12 |
...
| GW-6 | the same: a step that reads an undeclared type is a declaration error | a declaration whose step's `reads_to_enter` names a type outside the owner's `context_entity_types[]` | declare it | it lands | M (declaration); U-9 (runtime) |
...
| GW-14 | the same: `applies_when` has three values; unevaluable opens | `copy` at `legal` with an evaluable condition true, false, and unreadable | the step would open | false does not record inapplicable with the condition; unreadable does not open; inapplicable reads as signed anywhere | M |
...
| GW-18 | the same: scope amendment versus scope creep | — | — | U-8 |
| GW-19 | `gates_and_workflows.md#findings-verdicts-and-what-a-blocking-finding-obliges`: findings bind; a contradictory verdict is refused at submission | a sign-off with a blocking finding under `signed` | write it | it lands; the step closes | M once U-6's shape exists; U until then |
...
| GW-22 | the same: a verdict carries no condition | — | — | U-5 |
| GW-23 | the same: routing a remedy never transfers the verdict; a decision-kind finding is not routable | a blocking finding of kind decision | a task is created from it; an implementer's artifact arrives | a task exists for the decision-kind finding; the step reads signed on the implementer's work with no owner sign-off | M once U-6; U until then |
| GW-24 | the same: a blocking verdict names its evidence | — | — | U-6 |
| GW-25 | `gates_and_workflows.md#a-finding-is-one-off-or-standing-and-a-standing-one-obliges-a-change-to-what-produced-it`: a standing finding produces a proposed change with provenance | a standing finding recorded | read tasks | no task with provenance to the finding; the batch's work corrected and nothing else | M once U-6; U until then |
...
| GW-28 | the same: where the scope lands | — | — | U-7 |
| GW-29 | the same: a proposed change is a proposal until the gate lets it through, never a self-mutation | an agent's own standing finding on itself | read the agent entity | a write to `agent` or `agent_policy` by that agent's credential with no permitted action behind it | M (audit-shaped; U-14) |
...
| GW-30 | `gates_and_workflows.md#one-step-set-defined-once-tested-for-parity` | `B0` | read the step set from the declaration and from every code copy the reference implementation admits | a copy differs; a copy has no parity test | M (see X-10) |
| GW-31 | `gates_and_workflows.md#sequencing-is-data-successors-and-the-chain`: the last step is singular; one successor or none; parallel successors forbidden; `successors` naming intake is a declaration error | declarations with a parallel last step; a closing sign-off naming two; a `successors` list naming intake | declare; sign | any lands | M |
...
| GW-33 | `gates_and_workflows.md#two-questions-who-may-claim-a-step-and-whether-an-action-may-be-taken`: governance writes are actions | as WM-22 | — | as WM-22 | M (audit-shaped; U-14) |
...
| GW-51 | `gates_and_workflows.md#whether-the-verdict-is-a-stored-field-or-a-read-over-the-findings-and-the-author` | — | — | P (open decision 32; GW-19, GW-20, and GW-21 are the rows a ruling would rewrite — *The simplification pass, verified against the matrix*, below) |
...
| FP-3 | rule 2: announce entering and leaving, aggregated per window | as FP-1, then restore; three blocked claims in one window | — | `CH` shows no announcement on entering; none on leaving; three messages for one window | M (U-19 on the window) |
...
| FP-6 | rule 5: deferral is bounded; exhaustion escalates `rounds_exhausted` | a step whose `on_fail` loop has a declared cap; `CLK` | fail the step past the cap | no checkpoint `rounds_exhausted`; the loop continues | M (U-20 on where the cap lives) |
...
| FP-11 | the same: every class names its recovery | — | — | U-10 |
...
| FP-13 | the same: the restore obligation | — | — | U-11 |
...
| FP-15 | `failure_posture.md#repeated-lapse-raises-a-checkpoint` | as WM-17 | — | as WM-17; during a halt the count does not accrue; the checkpoint is not written when the record returns | M |
...
| FP-23 | the announcement path itself unreachable | as FP-1 with `CH` down | — | — | U-18 |
...
| # | Rule | Setup | Action | Red when | Class |
|---|---|---|---|---|---|
| DM-1 | `data_model.md#concepts`: every type, field, and edge the table names | `B0` | census | a type absent; a "deliberately not a field" present | M (X-3, X-11) |
...
| DM-6 | the same: a sign-off is pinned; a later head does not invalidate automatically; staleness is derived | a signed step; the head moves | read | `artifact_refs[]` lacks the head; the sign-off changes; a stored stale flag | M (U-21 for non-code kinds) |
...
| DM-21 | `data_model.md#write-contract`: each actor's writes ⊆ its column | a full run, `RP` per credential | — | any write outside the column (X-7 for the adapter's resolution) | M |
| DM-22 | the same: the daemon reads back each task it created | a daemon poll | `RP` | a create with no retrieve after it | M |
...
| WF-4 | `link` | — | — | U-17 |
| WF-5 | `dedupe` | two tasks alike | run | the duplicate not terminal; no edge to the original (U-23); the batch continues to a successor | M once U-23 |
...
| WF-11 | `workflows.md#bug` | `T-routed(bug)` | run | `ux` or `arch` opens; the re-route to feature (X-5) | M |
| WF-12 | `workflows.md#security`: `release` always; no exploit detail on public artifacts | `T-routed(security)`, `X(github)` | close naming none; write a PR body | none lands (U-22); the body carries the fields the adapter withholds | M for the successor once U-22; R for detail |
| WF-13 | `workflows.md#copy`: `copy` before `impl`, against `brand_voice`; `ux` optional | `T-routed(copy)` | run | `impl` before `copy`; no `brand_voice` read in `RP`; `ux` ever inapplicable (U-26) | M |
...
| WF-24 | `workflows.md#whether-a-stage-names-anything-a-step-does-not` | — | — | U-30 (open decision 33) |
...
| CF-6 | `conformance.md#phases-and-implementation-state` | — | — | U-3 |
...
| MG-11 | `migration.md#gaps-and-contradictions-the-mapping-exposed` | — | — | findings, not rules: the ones this suite's rows also hit are cross-referenced by number above (G1, G7, G8, G11, G12, G13, G15, G17, G21, G25) |
...
| `scenarios.md#c-assignment-then-the-named-principal-claims` | WM-1, WM-2, WM-4, WM-15 (X-6 for who writes `assigned_to`) |
...
| the announcement path down | — | U-18: the design names nothing, and this is the one mode with no loud mechanism |
...
it; and the transfer and chat-message kinds, which have one level and no pinned state the design names
(U-21).
...
- **Operator-initiated after bootstrap**: **open decision 43**'s second half; both branches designed and
  held. This is the cell where the design has not spoken: the operator's own write to `action_policy`
...
instance, with the edge and the copied rule (WM-35a), and its failure mode, a creation that fails after the
sign-off landed (U-28); **the schedule-computed due date** across three cases — on time, late within the
...
- **32, the `verdict` field.** GW-19 (a contradictory verdict refused at submission) and GW-20 (three
  ...
  its artefact through the sign-off's immutability. The guarantee moves to two places, and one of them
  does not exist: DM-1 would refuse a `verdict` field on the registered type, and a derived verdict would
  rest on a finding shape the data model does not declare (U-6; `migration.md`, G15). The matrix's reading
  is that the retirement is blocked on U-6 and not on the field, and that until the finding has a shape
  the refusal at submission is the only mechanical control the findings have.
...
**The defects the pass noticed and left, placed.** The governance list stated three ways is X-16
(`migration.md`, G1). The `unclaimed_step` checkpoint's subject is X-1, unchanged. The standing-finding
ruling routing an operator-specific preference to an agent's prompt is U-29.
