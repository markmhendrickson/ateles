...
| WM-14a | `work_model.md#what-distinguishes-a-task-being-assembled-from-one-intake-has-not-reached` (decision 84): an assembling task first becomes readable together with its intake batch, `ADDRESSED_BY` edge, and persistent `classify` hold finding; the unit is admitted whole or not at all; creation grants no lease, and only the declaration-resolved `pm` step owner may claim assembly; the exclusion survives lease lapse, crash, return, and transfer until a `classify` verdict exists; a genuinely awaiting-intake task carries none of the unit and stays claimable | one task created as still being assembled by a non-PM creator; one by several contributors; one complete task created for ordinary intake; an eligible PM holder that renews, crashes, and transfers the lease; mutants that expose the task early, delete the finding on lapse, or grant the creator a lease | read after every creation and lease boundary; evaluate ordinary claimability and the PM-only assembly claim | the assembly unit is partial; a creator or contributor receives a lease without resolving as `pm`; the task enters an ordinary claim pool before a `classify` verdict, including after lapse or transfer; an eligible successor PM cannot claim assembly; the ordinary task is withheld; any of the three mutants stays green | M — effect-shaped acceptance for decision 84; the persistent-exclusion and creator-ownership mutants prove the rule can fail |
...
| WM-39 | `work_model.md#where-tasks-come-from-every-source-indexed`: nine sources; ordinary complete tasks end creation with no intake batch, while decision 84's assembly exception ends creation with its intake batch and persistent hold; the creating principal holds no privilege over either shape | a full run exercising each source, once ordinarily and once where assembly is requested | read every task's provenance, first batch, hold, and lease | a task whose provenance names none of the nine kinds; a first batch other than intake; an ordinary task with a batch at creation; an assembling task without its intake batch and hold; a lease granted because the credential created the task rather than because it resolves as the declared `pm` step owner | M |
