...
| AU-22 | `authority_model.md#agent-inventory-review-disablement-and-retirement`: the inventory is a two-grain derived read; no stored status or canonical phase authorizes anything | one agent with two deployments whose host evidence differs; a rendered label changed without changing source evidence | read the inventory; evaluate one claim and one action before and after the label change; census the `agent` type | the two deployments collapse to one runtime state; a lifecycle/status/phase field is required; either decision changes with the label; a watchdog-written inventory row exists | M (decision 115) |
| AU-23 | `authority_model.md#agent-inventory-review-disablement-and-retirement`: decisions are mechanism-specific and runtime evidence is never a universal claim predicate | a task owner with a current grant and no process yet; a self-triggering daemon with fresh host evidence but no grant; an action whose policy requires matching deployment evidence | claim; write; take | the task claim is denied only because no process pre-existed; the daemon write lands on host evidence alone; the action is taken with missing or stale required evidence; a universal `may_accept_work` result is read by all three | M (decision 115) |
| AU-24 | `authority_model.md#agent-inventory-review-disablement-and-retirement`: access and purpose review is ordinary recurring work and binds only through grant expiry or an `action_policy` condition | one review task `REFERS_TO` an agent and is due; close it and create its successor; make a second live successor; leave another review overdue with a still-current low-risk grant and no review condition; attempt an action whose policy requires a recent review | read the task chain and authority decisions | the successor lacks `FOLLOWS`; zero or two live instances remain after a successful close; `review_current` exists or the overdue task universally denies the low-risk read; the conditioned action is permitted; the review verdict directly edits a grant, roster, or deployment | M (decision 115; ruling 30 supplies recurrence) |
| AU-25 | `authority_model.md#agent-inventory-review-disablement-and-retirement`: reconciliation fails closed at the affected predicate, availability-only drift does not block unrelated work, and remediation is separately gated | each mismatch row in the section, plus an unrelated agent and action; observe an undeclared process with no permitted stop action | evaluate the affected and unrelated decisions; reconcile | an affected unsafe decision permits; an unrelated action blocks on availability drift; the reconciler stops, restarts, revokes, redeploys, or edits a grant without a permitted action; stale independent evidence is treated as false or fresh rather than `Indeterminate` | M (decision 115) |
| AU-26 | `authority_model.md#agent-inventory-review-disablement-and-retirement`: disablement and retirement are derived from ended authority/routing/deployment relations; neither claims process shutdown; retirement preserves attribution and history | disable an observed-running agent; retire it; retain its historic verdicts and actions; try to re-seat the retired principal | read, claim, act, and census | a claim or effect is permitted; disablement reports the process stopped without host evidence; retirement deletes the `agent` or breaks historic attribution; the same principal becomes eligible again; a stored lifecycle status is the deciding evidence | M (decision 115) |
