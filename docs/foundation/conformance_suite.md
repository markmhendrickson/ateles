...
| 12 | the successor declarations the run exercises (`feature`, `bug`, `payment`, …), each per `(declaration_scope, type)` | named in intake's `successors`, or the closing sign-off cannot select them | **derived** | each declaration; `successors` naming intake is refused as a declaration error |
