...
| DM-29 | `data_model.md#the-registry-is-closed-by-default-and-registering-outside-the-declared-set-is-a-governance-write` (decision 113): an instance declares the set of types it holds; a registration outside that set is a governance write, refused at the record and admitted only on the engine's grant; an empty declaration is the absence of a boundary and not a closed registry | `B0` plus an instance with a declared set, and a second instance with none | register a type inside the declared set; register one outside it; register one on the instance that declared nothing | a registration outside the declared set is admitted without a governance write; a registration inside it is refused; the instance that declared nothing refuses an ordinary registration (an empty declaration read as deny-everything) | M |
| DM-30 | `data_model.md#what-the-declared-boundary-refuses-when-the-instance-cannot-evaluate-it` (decision 114): the boundary read fails closed — a write is refused where the instance cannot read its declaration of what it holds — and an absent declaration stays distinct from an unreadable one | `B0` plus an instance whose boundary declaration cannot be read, and one with no declaration at all | attempt a write against each | the unreadable-boundary write is admitted; the two are reported alike; the refusal is raised as a policy verdict rather than as an infrastructure condition a writer retries on | M |
