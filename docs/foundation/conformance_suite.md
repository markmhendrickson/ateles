...
| WM-14a | `work_model.md#what-distinguishes-a-task-being-assembled-from-one-intake-has-not-reached` (decision 84): an assembling task first becomes readable together with its intake batch, `ADDRESSED_BY` edge, held `classify` lease, and hold finding; the unit is admitted whole or not at all; a genuinely awaiting-intake task carries none of the assembly unit and stays claimable | one task created as still being assembled; one complete task created for ordinary intake; a creation writer mutated to expose the task before the assembly unit | read both tasks after each creation boundary; evaluate claimability | the assembling task is ever readable without all four companion records; it reads claimable while the lease is held; the ordinary task is withheld or reads not claimable; the early-visibility mutant stays green | M — effect-shaped acceptance for decision 84; it fails on the creation-time gap a later ordinary claim leaves |
