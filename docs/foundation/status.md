...
|---|---|---|
| a session binds no plan; a task's `PART_OF` to its planning record is written at `classify` | `.claude/hooks/_session_integrity.py` no longer binds a default plan (removed 2026-09-24, PR #1230): each session resolves its own workstream's plan, and `SUPERSEDED_PLAN_ID` survives only as a `_mentions_plan()` substring probe; `stop_finalizer.py` classifies a session as violated when its transcript touched no plan; `CLAUDE.md` asks each session to bind one plan and correct its fields each turn | `grep -c 'setdefault("bound_plan_id"' .claude/hooks/session_start.py` → 0; `grep -c SUPERSEDED_PLAN_ID .claude/hooks/_session_integrity.py` → 2 (definition + probe) |
