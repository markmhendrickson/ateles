...
|---|---|---|---|
| **13.** a step may **hold** on a condition discovered mid-flight: a non-blocking finding names the condition, no sign-off is written, the lease is renewed; there is no held state and no field; the ordinary active hold ends by sign-off, checkpoint, or lapse, and a hold owing nobody a decision is a rule-5 deferral (`rounds_exhausted` at the ceiling). Decision 84's creator-time assembly finding is the scoped exception: before a lease it derives a persistent exclusion, and after lapse that exclusion survives while only the PM-only `classify` claim reopens | the open decision's interim reading (raise a checkpoint or sign `unknown`, a verdict the design does not have) | **designed and not built.** No `LEASE` relationship exists to renew (the claim primitive is #733, open), no `finding` entity carries a condition, and the three places the design says already hold — a declared read returning `unknown`, the unconfirmed payment, the operator-only `await` step — are the revision 14 and 24 rows above and have no code | `lib/daemon_runtime/task_claim`; the step-execution path |
...
| multi-instance routing resolves at the proxy from the deployment's binding (decision 95's third consequence) | decision 91 rules the controlling instance the deployment names as where its governance writes go | not built; no deployment names several instances today | the proxy, which decision 97 permits and never makes the enforcement point; the resolution itself is the deployment's binding |
