...
other entry still uses the names the checkout uses, which are the old ones.
...
`vocabulary.md` 38.9k, `scenarios.md` 15.8k) is named as omitted on a change that keys one; the kernel
itself still fits. The budget, or the kernel's size, is the operator's call. Code comments in `apis.py`, `swarm_dispatch.py`,
