...
| workflow | `workflow` | `project`, `workflow_type`; `steps[]` (`phase`, `step_name`, `owner_role` — a role the roster resolves at claim time, never an agent name; `parallel_group`, `join_step`, `required`, `applies_when`, `on_fail`, `reads_to_enter[]`, `reads_to_close[]`, `freshness`, `unclaimed_after`, `hold_bound` — `gates_and_workflows.md#declaration-batch-projection`); `fast_paths[]`; `successors[]` | — | which batches are of it | — | a copy of the step list in code; a floor list |
