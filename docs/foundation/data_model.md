...
| reviewing lens | one `sign-off` on its review step, with the head it judged, the findings it recorded, and the evidence it executed | the sign-off | a verdict on a step another lens owns; a comment on the host in place of the sign-off; a verdict rewritten in place (a later judgement is a **new** sign-off, stated as such, and the latest per lens per head is the one that stands); a verdict contradicting its own findings (the write is rejected at submission); a verdict carrying a condition |
