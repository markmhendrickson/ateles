...
(`adapters.md#an-artifact-exists-only-once-its-external-systems-entry-does-and-the-interval-before-that-belongs-to-the-action`).
...
written. Carried by the canonical document but not by the projection: the per-update-kind enumeration below the level of the
...
Named so a reader knows what is deliberately absent rather than overlooked. Each is a real
