...
**Kind:** foundation companion; generated, never authored. **Generated by:** `execution/scripts/render_rule_inventory.py`, held equal to the measured system by `--check`. **Measured:** 2026-09-23.
...
| …of those, still flagged NEEDS-SPLIT | **16** |
| Statements of those rules, across all stores | **543** |
| **Duplication factor** | **10.4×** |
| Clusters whose statements DIVERGE on binding force | **15** |
| Normative statements scanned in total | 4444 |
| …of those, matching no known rule kind | 3969 |
| Statement values withheld from public output | 4444 |
...
The factor is computed over the 543 statements that match a known rule kind, not over all 4444 scanned. The remainder are procedure, context, or rules whose kind has no signature yet — counting them would inflate the figure with statements the migration has nothing to collapse.
...
This revision applies the merge test — two statements are the same rule only if a session cannot satisfy one while violating the other — and reports **52 rules at 10.4×**, with **16** clusters still flagged as buckets. The new figure is not proposed as final either: a NEEDS-SPLIT count above zero is the document saying so about itself.
...
|---|---|---|---|---|---|
| Skills (ateles repo) | `.claude/skills/<skill>` | 96 | 1377 | 2026-09-23 | yes |
...
| Claude Code project memory | `~/.claude/projects/<project>/memory` | 342 | 662 | 2026-09-23 | per-project |
| ateles/CLAUDE.md | `CLAUDE.md` | 1 | 84 | 2026-09-23 | yes |
| standing_rule entities | `<entity>` | 52 | 73 | 2026-09-22 | sidecar only |
| Codex | `~/.codex/AGENTS.md` | 1 | 67 | 2026-09-14 | yes |
| Claude Code hooks (ateles) | `.claude/hooks` | 20 | 52 | 2026-09-23 | yes |
| agent_policy entities | `<entity>` | 25 | 51 | 2026-09-23 | no |
...
| ateles/CLAUDE.md checkout copies | `~/repos/<checkout>` | 137 | 0 | 2026-09-23 | divergent |
| neotoma/AGENTS.md checkout copies | `~/repos/<checkout>` | 166 | 0 | 2026-09-23 | divergent |
...
**16 clusters below do not pass it yet.** They are listed as buckets rather than counted as clean rules. Each holds statements whose openings are nearly all distinct, which means the cluster is grouping by shared vocabulary rather than by rule identity — the same defect that hid the questions-tool rule.
...
| `R-fcd5a0` Dispatch work to the owning agent; file it as you recommend it | 22 | 21 | 0.95 | 8 |
...
| `R-ebd526` Persist every conversation turn to Neotoma | 14 | 12 | 0.86 | 6 |
...
| `R-fcd5a0` Dispatch work to the owning agent; file it as you recommend it | 22 | advisory, mandatory, prohibitive/binding | Claude Code hooks (ateles), Claude Code project memory, Codex, Skills (ateles repo), Skills (user root), ateles/CLAUDE.md, standing_rule entities, task_policy entities |
...
| `R-fcd5a0` | Dispatch work to the owning agent; file it as you recommend it | 22 | 21 | 8 | **NEEDS-SPLIT** | agent_policy |
...
| `R-ebd526` | Persist every conversation turn to Neotoma | 14 | 12 | 6 | **NEEDS-SPLIT** | agent_policy |
...
Target home: **agent_policy** · 22 statements, 21 distinct · **NEEDS-SPLIT**
...
| Store | Public location shape | At |
|---|---|---|
| Claude Code project memory | `~/.claude/projects/<project>/memory/<file>` | L82 |
| Claude Code project memory | `~/.claude/projects/<project>/memory/<file>` | L10 |
| Claude Code project memory | `~/.claude/projects/<project>/memory/<file>` | L12 |
| Skills (ateles repo) | `.claude/skills/<skill>/SKILL.md` | L61 |
...
| Store | Public location shape | At |
|---|---|---|
| Claude Code project memory | `~/.claude/projects/<project>/memory/<file>` | L70 |
| Claude Code project memory | `~/.claude/projects/<project>/memory/<file>` | L11 |
| Skills (ateles repo) | `.claude/skills/<skill>/SKILL.md` | L120 |
...
| Project memory: 329 files across 15 dirs | **342 files** | The current file total is derived from the measured store; private project-directory names are not emitted. |
