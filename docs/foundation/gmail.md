...
no code path, is `status.md`. Amendment history: `revisions.md#gmailmd`.
...
states once (`data_model.md#record-conventions`) and the same Art. 6(1)(f) basis the project operates
under — what a message says is durable input to work not yet conceived, and the record does not
pre-judge that at write time. What narrows is what a write, a draft, or a channel carries **outward** about
a third party: refusal 1, below, is now a constraint on surfacing and disclosure rather than on capture,
and the case that still binds at capture is Article 9 data about someone other than the operator, which
the broadened purpose does not reach (`data_model.md#record-conventions`, the special-category bullet).
That rule is stated once, under *What this adapter refuses*, and cited from the rows it governs.
...
The artifact's `external_id` is the **system's** id, because the artifact is by definition an entry that
system holds. The header id is a field the adapter may write onto the artifact as an observation, and it is what
...
**1. Capture is generous under the broadened purpose; what is bounded is a third party's Article 9 data,
and what leaves the record.** The record holds what serves the relationship and the work — who wrote,
when, on what thread, what the message concerns, what it commits anyone to — and, under the purpose
`data_model.md#record-conventions` states, is not limited to that: a detail with no use today is durable
input to work not yet conceived, and the adapter does not pre-judge relevance at write time by summarizing
or dropping it. What does not move with that broadening is Article 9 data about a third party — health,
finances read as a proxy for someone else's circumstances, family situations, political or religious views
belonging to a person other than the operator — which stays minimized at write into contact profiles and
observations, summarized rather than transcribed where the detail is sensitive and incidental
(`CLAUDE.md`, people-data processing; RGPD Art. 9; `data_model.md#record-conventions`, the special-category
bullet). As with `github.md`'s security narrowing, this is a constraint on what the adapter **writes** about
a third party's special-category data, not on what it reads or on capture generally, and it is the stricter
of the two available places to put it: material the record never held cannot be copied out of it by a
later summary, digest, notification, or rendered page. The observation's coverage states that fields were
withheld by policy, distinguishing "the adapter did not read this" from "the adapter read it and did not
write it" — which are different facts about the record. The mechanical half is the grant: the fields this
adapter may write on a `contact` are the allowlist its grant's parameter constraints name, and a write
outside them is denied at admission (`authority_model.md#grants`); what within an admitted field is
incidental or sensitive is the judgement this refusal states. **What is not minimized at write** — the
operator's own record of a relationship, and any non-Article-9 detail about a third party a message
carries — is bounded downstream instead, at the surfaces that carry it outward: `workflows.md#outreach`'s
disclosure check on a draft, and the reference-never-value rule every channel already applies
...
The reading projection needs this distinction, so it is stated as a table rather than left implicit. The
