...
external system's entry does), `work_model.md` (artifacts; at-least-once implies effect dedup), `gates_and_workflows.md`
...
|---|---|---|---|
| bank transfer | an entry the rail holds of the instructed transfer | the rail's own transfer identifier, assigned when the rail accepts the instruction | **only when the rail's response returns** — so a lost response leaves the swarm without it |
...
`taken_at`, no `result_ref` (`adapters.md#an-artifact-exists-only-once-its-external-systems-entry-does-and-the-interval-before-that-belongs-to-the-action`).
...
names no artifact, and whose content is stale the instant it is written — maintained state of exactly
...
**What is load-bearing and what is not, for the reading projection.** Nothing here is deleted from this document — decision 66 (`conformance.md#what-a-review-reads-is-a-projection-of-these-documents-not-a-shortened-copy-of-them`) rules that the canonical text keeps its full argument and that the review prompt reads a generated extract instead. This note says which statements that extract must carry. Load-bearing: the asymmetry in when the
...
about what does not exist. Carried by the canonical document but not by the projection: the per-class delivery comparison, the fee
...
Named so a reader knows what is deliberately absent rather than overlooked.
...
| `verify` and `reconcile` belong to a principal disjoint from the payer | **one process, one identity, one code path** loads the profile, previews, reads the approval, submits, declares the outcome, and writes the bookkeeping. The submitter and the `reconcile` step owner are the same, and the confirmation written is the submitter's own account of what it did. **No `reconcile` step and no transaction write by a disjoint principal exists**, and no payment workflow entity exists to declare them | `monedula.py` |
...
is reachable: the module that detects profiles which are active but cannot be acted on escalates
