...
- **G24 — formation admits no way to carry work part-way through a retired passage.** A batch opens only
  on a closing verdict; nothing lets a migration open a batch at a step. The design's own answer —
...
- **G33 — a write of a raw file is admitted without the declared boundary being consulted at all.** Decision
  113 closes the registry by a set the instance declares, and decision 114 rules that the read of that set
  ...
  *correct*, and the two readings are far apart. Either a file is not an entity of a type, so a boundary over
  ...
  boundary is a boundary *over* before options can be put, and that statement is decision 113's to extend if
  the operator wants it extended.
- **G34 — a property declared on a type has no stated failure direction when it fails to resolve.** Decision
  114 rules the boundary read alone, and says in terms that it is not extending that ruling to a gate reading
  ...
  applied to this subject, and decision 114 deliberately declines to apply them by proximity. Recorded here
