...
provenance, and no correction history, and every reference to the old one is silently broken. No conclusion
is written for a principal that did not write it: a legacy step record is never turned into a `verdict`
...
| `task.recurrence`, and the tasks that carry it | keep; hold the writer | `task` with its `recurrence` rule; the next instance created by the closing verdict, `FOLLOWS` task → task (`work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`) | the live instance the retired pattern left — one open task per obligation whose `due_date` a daemon moves after each completion — is already the design's one live instance, so nothing is rewritten and no `FOLLOWS` edge is fabricated for occurrences the retired pattern overwrote (their history was lost when the fields were). What changes is the writer: the daemon that moves the date must stop before the first instance completes under the design, or a completed instance is reopened beside the one its verdict created — two live instances, which the ruling forbids. The hazard section names it |
...
| `session_digest` | keep, now registered (decision 63) | `session_digest`, the `digest` step's output in `workflows.md#session-digestion` | the design names it as an entity in the record. Existing digests predate the workflow and belong to no batch; they are history, and the `verify` step's states already live in their claims. Decision 63 makes it eligible for the special-category mark; the ~369 rows on prod predate any mark or owner and are drift this row carries, not a reason the type stayed unregistered (`data_model.md#record-conventions`). Decision 40 additionally gives it a `REFERS_TO` ← verdict edge, required on an agent's verdict where a digest exists for its session; zero such edges exist among the ~369 rows or among verdicts on prod today, since neither side wrote the other before this ruling — carried here as the same kind of migration drift as the mark and owner gap, not an argument that the edge is wrong: a rule ruled today binds writes from today, and backfilling the edge onto pre-ruling verdicts is a population-plan question, not one this ruling answers |
...
| `plan_contribution` | derive; retire the writer | `finding` on a `planning` batch, `PART_OF` its verdict (`planning_model.md#maintenance-is-work-the-planning-workflow`) | the review, concern, question, and amendment kinds are findings a review step of the `planning` declaration records, and a verdict kind is that step's verdict; the rows the instance holds predate any `planning` batch and stay history, as the gate-model table's retired contribution rows do (gap G31 closed) |
...
| `participation_record` (retired name), rows at a terminal satisfied status | derive, as history only | none | a satisfied row is the retired engine's record that a step was judged. It is **not** turned into a `verdict`: the design's verdict is pinned to an artifact head the retired record never captured, carries a pinned agent version it never captured, and is attributed to a principal that never wrote it. The rows stay as the as-of read of what the retired engine recorded. Where the work they judged is still open, the operator's `waived` verdict per step, carrying the legacy row as its reason, is the design's own way to carry the judgement forward (stage 4) |
| `participation_record` (retired name), rows at a non-terminal status | retire | step state derived from batch, `LEASE`, and `verdict` | a row saying a step was opened for a step owner is, in the design, the existence of a batch at that step (open) or a lease on it (claimed), both derived. No batch exists for the work these rows name and no lease can be fabricated, so the rows are frozen history. Nothing is lost that the design would have kept: the design deliberately has no per-step status row |
| `gate_status` (retired name), `workflow_state`, `workflow_run` (retired name), `workflow_gate`, `release_gate`, `task_action`, `owner_history_entry` | retire | `step_status` projection on the task; the batch's verdicts | the per-artifact step maps the retired engines wrote. The design says step state or conclusions on an artifact belong to the batch and its verdicts and are deliberately not fields of the artifact (`data_model.md#concepts`). The maps were written by two engines that did not read each other, so their content is not trusted enough to derive from; they are frozen and readable |
...
| `batch`, `verdict`, `action`, `artifact`, `checkpoint`, and the edges `ADDRESSED_BY`, `FOLLOWS`, `LEASE`, `CLOSES`, `SIGNED_BY`, `PRODUCES`, `CHECKPOINTS`, `AWAITS`, `RESOLVED_BY`, `RAISED_BY` | introduce | themselves | none exists on any instance. The derivations above populate `checkpoint`, `action`, and `artifact` for the open held decisions only; everything else is written by the design's own writers from the cutover. For new work, a workflow-entering task's atomic creation unit opens its intake batch without a predecessor verdict, and every successor batch is opened by a closing verdict (`work_model.md#how-a-batch-is-formed-and-what-chooses-its-workflow`). A pre-existing workflow-entering task cannot retroactively satisfy that creation boundary merely because it lacks a batch; no migration writer is authorized to invent a third batch-opening path. Such tasks remain held as migration drift until G24's carrying rule is closed. Aggregate parents remain outside claim and need no batch |
...
session's own present state. No step owner would claim it and no verdict would close it, because it does no
...
verdict or observation carries their name (`status.md`). A migration carries a record; for these there is
...
executed** — a credential that has written, a task a runner has held, a verdict or observation carrying the
...
"the verdict is written and read back" has said everything the call shape said, minus the harness.
...
accountable: correspondence voice and verdict; channel and client routing per external system; scheduling
...
verdict is worded, what a finding must carry, that a response code is not evidence — is an `agent_policy`,
...
domain predicate, decide whether there is input, file a contribution record with a verdict or a concern; a
**consultation** protocol for asking another role a question; and, on the review roles, a **per-step
verdict** on the issue entity — correct the artifact's step map, append to its owner history, check the
...
and one candidate per review role described the same verdict with a different judgement in the middle.
...
review step is a step whose work is a judgement of the batch's change, closed by its owner's verdict like
any other step (`vocabulary.md#review-step`); the verdict is one entity with one write contract
...
conclusion and nobody else's. Each role's copy of the procedure is a hand-written implementation of that one
...
principal the roster resolves, every verdict is read back, the closing verdict names its successor or
...
the stage touched, and the batch's closing verdict naming no successor). `on_fail` on `verify` opens
...
**Stage 1 — the registry (operator act).** Register `operator`, `agent`, `workflow`, `batch`, `verdict`,
...
task by moving its `due_date`; the design closes the instance and has the closing verdict create the next
(`work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next`). If both run,
the first completion under the design leaves two live instances: the one the verdict created and the one
...
- **G15 — the design's finding has no row.** Findings bind, carry severity, and are what a conclusion may
  not contradict, and `data_model.md#concepts` gives the verdict no `findings[]` field and names no
  ...
  verdict that carries it and `REFERS_TO` the batch it judges (`data_model.md#concepts`); the instance's
...
- **G24 — formation admits no way to carry work part-way through a retired passage.** A new intake batch
  opens only in a workflow-entering task's atomic creation unit and a successor only on a closing verdict;
  nothing lets a migration retroactively complete that unit or open a batch at a later step. The earlier
  inferred answer — every pre-existing task simply re-enters intake — would invent a third opener and is no
  longer admissible after decision 84. Pre-existing workflow-entering tasks stay held as migration drift
  until a governed carrying rule closes this gap; aggregate parents remain outside claim and need no batch.
...
under what tool bounds an agent executed, so a verdict's pinned agent version pins the prompt and not the reach, and
a change to an agent's reach is invisible to the mechanism the design built to see changes to agents. **What would
decide it,** as the question was opened: whether the tool bound is part of what a verdict attests — if a review of a
...
of the design (above). The deciding test is met, and met without a new entity: ruled decision 40 has a verdict pin
...
verdict found to be judged on hook wiring or environment would move that mechanic from plumbing to something the
record must hold.
