...
**Ruled (2026-09-16, decision 104): candidates 3 and 4 together — coverage surfaced at the point of
presentation, and incorporation.** The rule above states what every observation carries; a reader who is
not a step now gets two things this ruling adds, in different registers, ruled together because the corpus
frames them as compatible rather than competing.

**Candidate 3: surface coverage at the point of presentation.** Any surface presenting adapter-sourced
state carries that state's coverage and sourced time with it, so a reader is told what was read and what
was not. Today a reader outside a formally declared step — a person looking at an entity, an ad hoc query —
has no prompt to go check coverage at all: the guarantee a step gets from its declaration
(`gates_and_workflows.md#declaration-batch-projection`) exists per step and nowhere else, and a reader
outside one gets whatever the record happens to hold with nothing telling them the question arises. This
closes that visibility gap without changing what the record claims — it is a requirement on presentation,
not a new claim about the world, and it is the same response
`telegram.md#delivery-webhooks-long-polling-and-what-the-dedup-rule-keys-on` already gives to an
irrecoverable coverage gap: making the gap legible rather than pretending it is absent.

**Candidate 4: incorporation.** The record's claim is left exactly as candidate 3 leaves it, and the
obligation moves to what an adapter's material becomes once it is in the record. Capture without
incorporation does not deliver what capture is for: a message held as an artifact with a body is captured,
and the company and the people that message names are not queryable until something extracts them into
typed entities with edges. The case that shows it is an archive of correspondence digested in full. It
produces the message artifacts, and it also produces the `contact` and `company` entities the
correspondence refers to — and those surface in contexts where correspondence is not the question at all: a
step drafting a proposal, one checking who a payment is for, one judging whether a task concerns a party
who has objected to being processed. None of those would think to read a mailbox, because none of them is
doing mail work. That is the difference this candidate turns on: coverage surfaced at read still assumes a
reader who would go and read, reading more or well or badly; incorporation names work that is
**structurally unavailable on demand**, because what is needed cannot be asked for by a reader who does not
know it exists. The need is invisible from where the reader stands, so no occasion to read ever arises. The
supporting argument is invariant 1's — a retrieval that depends on someone remembering to make it mostly
will not be made, and when it is, the result lands in that step's own context and not in the record, so it
serves one step and leaves nothing behind for the next reader.

**Incorporation continues a principle the design already applies, from capture to extraction.**
`data_model.md#record-conventions` already rules capture generous by design: under the stated purpose, a
piece of context with no use today is the input to work not yet conceived, and Art. 6(1)(f) legitimate
interest is broadened accordingly. This ruling is that same principle carried one step further — capture
the artifact, and now also extract the entities in it — not a new idea beside it. It composes cleanly
because it makes **no completeness claim at all**: nothing is asserted about whether the archive was read
in full, only about what the material actually read becomes, so principle 5 and the coverage rule are
untouched by it. It is a claim about form rather than about extent, and it is independently testable in a
way a completeness claim is not: whether a digested archive produced typed entities with edges is a
question a reader can ask of the record itself. The provenance machinery it needs already exists, since
`#what-the-record-supplies-and-what-an-adapter-therefore-never-builds` binds an observation to the
**interpretation** that produced it wherever a value was extracted rather than transcribed — the chain an
extracted entity rides on.

**What incorporation costs, and what it owes.** Extraction is a judgement, judgements are wrong, and a
wrongly extracted entity is a durable false fact that later work reads as true, where a missing entity is
only absent. So the obligation is not "extract everything," and three things already in the corpus bound
it. `data_model.md#record-conventions` states that the generous purpose does **not** reach Art. 9 for a
third party, and correspondence is precisely where third-party material arrives.
`calendar.md#what-this-adapter-refuses` refuses the nearby shape outright for its own adapter — recording an
attendee as a reference where the record already holds a `contact` and as an address otherwise, never
creating a `contact` for nobody it merely observes — so incorporation scopes its obligation to a step rather
than to the adapter, the way `workflows.md#meeting-processing` already does for a transcript. And decision
39 ruled that `link` attaches what a task **names** and nothing on relevance alone
(`workflows.md#what-link-attaches-and-what-it-leaves-to-hydration`), on the ground that a relevance-judged
pull has no stopping rule and is purpose-blind; incorporation is bounded by what the material names,
consistent with that ruling, and never by what might later matter, which would reopen it.

**What this ruling forecloses: candidate 2, a completeness guarantee.** Adapter-sourced state in the record
is not, and will not be made, complete for its artifact kind such that no reader needs to supplement it.
This is foreclosed on the corpus's own reasoning, not merely left unchosen: completeness cannot be known
without asking the external system, and asking is the read the guarantee exists to remove, so the record
would be made to state something no mechanism establishes — the shape principle 5 refuses, and the shape
`authority_model.md#grants` refuses again in its rule that a degraded read never synthesizes a value more
...
guarantee is the maintained state principle 11 forbids, for the reason the freshness rule above gives. What
this question is about is only what a reader — not a formally declared step — may infer from an
observation; what every captured observation itself records (source, time, coverage) is ruled elsewhere and
is not reopened here.

**A statement already made in passing, and its home, now fixed.** The posture candidate 3 makes explicit
was asserted once in the corpus before this ruling, as an aside answering a different objection:
...
which — and cited this section as where that rule lives, when this section did not yet carry it. It does
now: this ruling is that rule's argued home, and the calendar.md sentence stands as an instance of it rather
than as a second, uncited statement of the same claim (principle 9, one home).

**What the question turned on, and what it is not.** Whether the record is the swarm's system of record for
external state at all was the crux, not a framing of it. This corpus's position is that the record holds
what the swarm decided and on what evidence, and is deliberately not a copy of a system it does not own —
the source it keeps is not a second copy of what the external system currently holds, but the record of one
read, at one time, with the coverage that read had (*What the record supplies*, below). Candidate 2 would
have made it the second thing, a different object with different obligations, which is why it is foreclosed
above rather than merely declined. Candidate 4 never touched that crux — it leaves the record the first
thing and asks what the material read becomes once it is there — which is why it rules alongside candidate
3 rather than against it. What this question is **not** is the cadence question
...
`gates_and_workflows.md#declaration-batch-projection` answers by declaration and hydration (decision 103,
this ruling's neighbour rather than its bound, since a supplementary read would run through the declaration
scope that row puts in question). This one is about readers other than steps.

**One asymmetry any future extension must account for, stated here as an inference the corpus has not
otherwise written down.** Reads and attributions are not equally recoverable. A read that fell outside a
channel's retention is a coverage gap that a later read can sometimes close, and the design records it as a
gap when it cannot. An inbound approval lost the same way is not recoverable at all by reading the system
again: an approval is attributed to a principal and authorized against the required approvers
(`telegram.md#what-this-document-refuses-and-why`), and a later read finding a changed state carries no such
attribution. So a claim of completeness over adapter-sourced state — were one ever made, which this ruling
forecloses — would be stronger for the artifacts a re-read can establish than for the decisions it cannot.
The distinction stays live for incorporation too: an extracted entity is a durable claim in a way a missing
one is not, which is exactly why incorporation's bound (above) is what the material names and never what
might later matter.
