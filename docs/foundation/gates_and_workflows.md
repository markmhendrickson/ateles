...
**A required read that resolves to nothing is not a failed read, and it stops the step for a different
reason.** The rule above covers a read the swarm **could not make**: the record was unreachable, or the
adapter could not fulfil it, and the value is `unknown`. This rule covers the read that **was made and came
back empty** — the record holds no instance of the declared type for this task, and no adapter had one to
import. The two are distinct by the same principle that separates `unknown` from empty above, and they owe
the operator different things. An unreachable dependency is a condition that may resolve on its own, so it
holds with backoff and escalates as `undeclared_dependency` when it does not. An input that does not exist
resolves by nobody retrying: the record answered truthfully, and the answer is that the task does not say
what it is asking for. So the step does not open, and one checkpoint is raised on the task, reason
`underdetermined_inputs`, naming the read that resolved to nothing and what would satisfy it
(`failure_posture.md#a-task-whose-inputs-cannot-be-resolved-is-put-to-the-operator-not-executed-on-a-guess`).
Attaching a retry schedule to this condition would be the instrument error principle 3 names — a count that
rises forever against a world that is not going to change.

**Sufficiency is judged here, and this phase judges presence rather than adequacy.** Hydration is where a
step's inputs are resolved, so it is where their absence is visible, and judging it here stops the task
before a runner claims it and spends a lease discovering it never had what it needed. What hydration decides
is mechanical and stays mechanical: a required read resolved to an instance, or it did not. It makes no
judgement about whether an instance that *was* found says enough to act on — that is the step owner's, it is
made with the work in front of it, and the design already carries it: the step owner holds on a condition
discovered mid-flight, records the finding naming what it cannot yet judge, and the hold ends at its
`hold_bound` in a checkpoint (`work_model.md#a-batch-may-hold-on-a-condition-discovered-mid-flight`). Both
paths exist because both conditions are real, and the line between them is which component can tell:
hydration can tell that nothing was found, and only the principal doing the work can tell that what was
found is too thin. Neither substitutes for the other, and a step owner discovering an inadequate input does
not raise this class — it holds, as decision 13 says, because it is judging and not merely reading.
...
registered `conversation`, `conversation_message`, and `session_digest` as the types that hold them
(`data_model.md#record-conventions`), so the sentence this replaces, which called the turns "not an entity
the design has," is corrected: it was true only of `agent_session` and was written as if it were true of the
session generally.** What an `agent_session` is stays unchanged by this: the identity half a runner's
...
**Not the same claim as "no session is persisted."** The operator's standing practice — every turn of every
session stored, user and assistant rows alike, on the MCP server's own mandate — is not the drift this
ruling would name. It persists a different pair of types than the one this section rules on: the swarm's
...
finding supports. Widening the [finding scope](vocabulary.md#finding-scope) of a lesson is the same failure as narrowing it — one produces a rule
...
### A synced observation on a governance type is recorded, and never takes effect

Decision 55 (`adapters.md#whether-a-second-instance-of-the-record-is-an-external-system`) rules that a peer
instance is the record, extended by replication: a synced entity enters through the same observation
machinery as a local write, with no adapter and no action between the two records. None of decisions 18, 41,
or 56 was stated with a synced write in mind — 18 reserves each governance class to `NEVER` until the
...
**It is recorded, and it does not take effect — the reducer's own ranking gives half of this for free, and
admission gives the rest; no new mechanism is added (principle 6).** Two separate reasons stack, one for
each half of "does it take effect":

1. **It does not become the read snapshot on a tie.** The record's reducer ranks a synced observation below
   a local one as its default tie-break once `source_priority` is equal (`data_model.md#record-conventions`,
   "a registered type declares how competing observations resolve") — this is the free half,
   already true of every type, governance or not, and needs nothing added for this case. It is a default
   and a tie-break, not an absolute: a peer relationship configured with a different conflict strategy, or a
   synced row with a higher numeric `source_priority` than the local rows it competes with, can still win
   the snapshot. That gap is closed by the second reason, which does not depend on tie-breaking at all.
2. **It is refused the only path that would make it govern anything.** A governance type's write capability
   is held by exactly one principal, the engine, under decision 56; every other write — whatever observation
   it lands as, however it ranks against a local row — is refused at the record's admission under decision
   41, because admission is checked against a credential's grant and a synced write carries no such
   credential to begin with. So an `agent_policy`, a `workflow`, or an `agent` a peer replicates is stored
   as an observation on the type, exactly as decision 55 says a synced row is stored, and it is never the
   observation any rule that reads "the current `agent_policy`" or "the workflow a batch runs" resolves to,
   because nothing about being the most recent or the highest-ranked observation on a governance type confers
   the write capability decision 56 reserves to the engine. Fail-closed is not a posture chosen for this
   case; it is what decision 56 already says about every write but the engine's, applied to a write that
   happens to have arrived through peering rather than through a denied credential.

The two reasons are not redundant: the first says a sync write usually loses a tie it might otherwise win;
the second says it was never going to govern anything even had it won. Stating only the first would leave a
misconfigured peer or a `source_priority` collision as the one way a peer's `agent_policy` briefly becomes
live, which fail-closed does not allow for a governance type.
...
**What is owed instead is narrower than a new mechanism: the same reader gets one more thing to check.**
The component already positioned to notice is the one this document already names for the write side — the
engine, which holds the sole write capability on every governance type (decision 56) and is therefore the
only principal with reason to read a governance type's full observation history rather than just its
current snapshot. When it writes a governance type on a permit, reading back what it wrote
(`principles.md#2-a-write-that-reports-success-has-not-necessarily-happened-read-it-back`) is
already its job; extending that same read to check for a newer, unreconciled `observation_source: "sync"`
row on the type it just confirmed is not a new watcher, a new poll, or a new queue — it is the existing
read-back looking one field further. Finding one is not a `capability_denied` (nothing was denied; the
sync write landed as an observation, exactly as decision 55 says it should) and not a blocking finding on
some batch (no batch is in question) — it is new information the operator did not have, about a peer
...
is to turn a described change in the record into a task with no human step between — a peer could otherwise
cause this instance to create work by replicating a rule and nothing else. Nothing above is stated
specially for it: the rule is a write to a governance type like the other seven, decision 56's sole-writer
grant reserves its write capability to the engine, and a synced write is refused the capability the same
way. The evaluator that walks intake rules and writes tasks reads the type the same way any governance
reader does, so decision 65 — whatever it settles for surfacing an inert governance observation — surfaces a
synced `intake_rule` sitting unrecognized under a locally-written one by the same read; no rule creates a
task from it, because the write that would need to have happened — the engine's, on a permit — never did.

**The reverse: this instance's governance entities syncing outward.** `sync_peers` on an entity's own
snapshot is the control decision 55 names, and it is per entity, chosen by whoever writes the tag — which
for a governance type is, again, the engine alone under decision 56. **A governance type should not carry
`sync_peers` by default**, for the reason the closed list gives these eight types in the first place: each
one states what this swarm may do or who its principals are, and a peer instance's principals, grants, and
workflows are its own operator's to author, not a value this instance's engine should be the one to widen
by tagging a row for export. Nothing above forces this — a peer relationship is `sync_scope: "all"` or
`"tagged"` at the operator's own configuration, and the eight types are ordinary rows the moment sync is
asked to reach them — so the default is a **standing constraint on the engine's own write**, alongside its
sole-writer grant: it does not set `sync_peers` on a governance type unless a future decision names a
governance class meant to propagate (a shared `action_policy` baseline across an operator's own fleet of
instances is the shape such a case would take, and no case has been named). Whether an instance ever wants
that is left open rather than ruled here, because no workflow this design was tested against needs it, and
principle 6 does not add a mechanism nothing yet uses.

**Ruled (decision 65, 2026-09-06): extend the engine's existing write-back read; no carrier is invented.**
The engine already reads back what it wrote to a governance type
(`principles.md#2-a-write-that-reports-success-has-not-necessarily-happened-read-it-back`); the read is
what makes that write evidence rather than an unread success code. This ruling asks it to check one field
further on the same read — whether a newer, unreconciled `observation_source: "sync"` row now sits on the
type it just confirmed — and record what it finds, in the same shape `adapters.md`'s admission contract
already gives every delivery: **every delivery resolves to a disposition, and the disposition, not receipt
alone, is what is recorded** (`adapters.md#the-admission-contract`, obligation 1). A synced governance
observation is not a delivery and this is not a `dropped` disposition — `dropped` stays the vocabulary
entry `adapters.md` scopes to an adapter's own deliveries (principle 6's substitution test, invariant 12) —
but the shape it borrows is that one: a disposition with a reason, aggregated per window rather than
written once per occurrence, readable rather than queued.

1. **Where it is written.** Aggregated per window on the engine's own `agent_session`, beside the window
   observation `adapters.md#what-the-adapter-does-with-every-event` already has a daemon write — the
   coverage of the writes confirmed in the window and the dispositions counted. Not a per-observation write
   on the synced entity itself: a row written once per sync arrival on a replication stream is exactly the
   maintained state invariant 11 counts against, where the existing per-window aggregate already carries a
   count and a reason under a type the registry holds.
2. **What reason it carries.** Not `capability_denied` — that names a principal whose grant was checked and
   refused, and a synced write names no principal's credential to check at all
   (`gates_and_workflows.md#a-synced-observation-on-a-governance-type-is-recorded-and-never-takes-effect`).
   No existing reason class states that absence, so this ruling names one: `no_credential`, distinct from
   `capability_denied` on the same basis decision 60 gave `capability_unavailable` its own class rather than
   folding it into `capability_denied` — a different fact precedes each, and collapsing them would blur a
   distinction the operator reads differently.
3. **Who reads it, and when.** Whoever reads the engine's `agent_session` window observations, on the same
   cadence as any other window's dispositions — this is stated plainly as readable when looked at, not a
   queue item owed a clearing. That is a stated cost, taken deliberately: a rare inert governance sync
   could sit unread for a window or several, because inventing a checkpoint or a finding to force it into a
   cleared queue would be reaching for a shape principle 6 forbids reaching for before principle 1 requires
   it. What would make it loud rather than quiet is already the adapter drop's own shape: a per-window count
   crossing a ceiling is announced on the off-record path a halt uses (`vocabulary.md#dropped`), and the
   same ceiling-crossing announcement is the mechanism that would fire here, should `no_credential` ever
   start arriving at a rate worth naming.

**Why.** Principle 2 — the read is what makes the refusal evidence rather than silence, and the engine's
read-back already exists; extending it by one field is not a new watcher, poll, or queue. Principle 6 — the
adapter's disposition rule is the closest existing precedent for "every write of this kind resolves to a
recorded outcome," and reusing its per-window shape (counted, reason-bearing) answers the carrier question
without registering a second mechanism beside `checkpoint` and `finding` for a condition neither was built
to hold, which the opening pass already found true of both.

**Cost accepted.** Named in point 3 above: a `no_credential` disposition is discoverable, not delivered: no
notification fires until a window's count crosses a ceiling an instance sets, so a lone inert sync could
predate the operator's attention by as long as the operator goes without reading a window observation. This
is the same cost the design already accepts for every other drop reason, extended to a case where the drop
is a governance-type sync rather than an adapter delivery.

**What would reopen it.** A workflow that needs to act on a synced governance observation before a window
closes — gate a step on it, or route it to a person synchronously — would need a subject to hold on, which
is the checkpoint shape this ruling declined to stretch; that need, once named, reopens whether the
per-window aggregate is still the right carrier or whether a subject now exists that did not before.
...
for a record the swarm did not produce (`gmail.md#every-inbound-event-and-what-it-becomes`, a sent message
...
**Ruled in part, 2026-09-06 (shape); ruled, 2026-09-06 (values, on the operator's decision).** Registered in
`conformance.md#the-register-of-open-design-decisions`. Blast radius
...
requirement on the first side: it is a constraint on whether the action the step is about to take may
proceed, not a constraint on who may claim the step. A minimum on the workflow declaration would blur that
line, since a step's owner role is claim eligibility and a tier floor is a gate input, and principle 6
...
**The values, ruled (2026-09-06).** The operator set a floor only for the classes that leave nothing to
undo or that only a forward fix can answer for: payments, sends, merges, and governance writes. Every other
action class is left unset. Reason: those four are exactly the classes `failure_posture.md`'s recovery
table already treats as forward-only or hardest to reverse (`merge_pr`, publish, release tag, deploy) or as
`NEVER`-tier by default (governance, decision 18) — a class where a wrong model's mistake is not merely
denied on its merits later but has already left an effect nothing undoes cleanly is the class where
disqualifying a weak model *before* it acts earns its cost; a class where the worst case is a low-blast
action taken again does not.

**Cost accepted.** A class left unset runs at whatever tier the acting agent's `vendor_binding` carries,
with no floor checked at all — the ordinary case today, unchanged for every class outside the four, and
the trade the operator accepted rather than writing a floor for every class up front. Should experience
show a fifth class wants one, the mechanism ruled above (`action_policy.min_tier`, evaluated at the same
take as blast) already generalizes to it; only the value is added, not a new place to write it.
