...
`conversation`/`conversation_message` bookkeeping and the authored `session_digest` are not the
`agent_session` this ruling constrains, and neither is registered in this design's own type registry
(`status.md#revision-28-2026-09-05-the-record-inventory-the-migration-plan-starts-from`). What this ruling forecloses is narrower
than it can read out of context: an `agent_session` — the step-linked, task-linked identity record — growing
a transcript field, or a sign-off copying a session's turns beside its findings. It does not foreclose a
session's content existing as its own entity elsewhere, reached by its own path; `session_digest` is named
above as exactly that path, already licensed. A reader taking the ruling as "the design refuses to store
reasoning" is reading past the sentence that grants the exception.

**A sign-off may carry a reference to the session that produced it, at no cost to the ruling.** Naming a
session is not copying one: a `REFERS_TO` edge from a sign-off to the `agent_session` (or, where the session
was digested, to its `session_digest`) is the same reference-never-value shape decision 40 already applies to
every other read a sign-off names, extended to one more source. Nothing about that edge writes reasoning
onto the sign-off — the transcript, if read at all, is read where it lives, at whatever time the reader
resolves it, exactly as an artifact's `head` is resolved rather than copied. The cost is one edge per
sign-off and a `REFERS_TO` target the schema does not yet name; the design already permits the shape and has
not stated it as a rule. This is a proposal, not a ruling: whether it is worth writing depends on a reader
in practice wanting it, which is the same bar the cost-accepted paragraph above sets for reopening the
ruling itself.
