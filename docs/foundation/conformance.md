...
| 61 | where an ancestry expectation is declared for a planning level — a field on the level's registered type, an `intake_rule` condition, or a `completion_criteria[]`/`applies_when` entry on the `planning` workflow's own declaration | `planning_model.md#whether-an-orphan-is-a-defect-at-all` | decision 57, which fixes what the levels are before a per-level field has anything to attach to | **open** (the ancestry pass, 2026-09-06; the design is indifferent among the three, each an existing mechanism, since no rule reads the expectation by its storage location, only by whether `judge` finds one declared) |
