...
| 45 | whether the host a daemon runs on is an external system reached through an adapter — its processes and checkouts artifacts, process control that adapter's outbound classes — or inside the boundary, with the runner's `agent_session` as its only record and process control an operator act | `adapters.md#whether-the-host-a-daemon-runs-on-is-an-external-system` | a declaration whose step reads host state; nothing before one | **open** (workflow-format pass, 2026-09-06; the boundary definition leans to the first) |
| 46 | what owning confers — sole decision below the domain's blast tier, a required seat above it and on cross-domain actions, or both | `authority_model.md#principals` | — | **open** (the brief's Q7, marked open in `authority_model.md` since its first revision; registered by the consistency pass of 2026-09-06) |
...
| 48 | the counting rule for quorum and separation of duties — an agent counts as its bound principal, as itself, or as itself for attribution only | `authority_model.md#structural-checks-quorum-and-separation-of-duties` | — | **open** (the brief's Q1; registered 2026-09-06) |
| 49 | whether structural checks are count and disjointness over the checkpoint's principals or a second mechanism | `authority_model.md#structural-checks-quorum-and-separation-of-duties` | — | **open** (the brief's Q2; registered 2026-09-06) |
| 50 | which structural checks apply at a dozen principals, and where the threshold lives | `authority_model.md#structural-checks-quorum-and-separation-of-duties` | — | **open** (the brief's Q3; registered 2026-09-06) |
| 51 | whether initiative approval is the checkpoint or a second approval object | `authority_model.md#initiative-proposal-reprioritization` | the initiative, proposal, and reprioritization types | **open** (the brief's Q4; registered 2026-09-06) |
...
| 53 | whether budget is a scope term that attenuates or a blast tier, and over which resources | `authority_model.md#initiative-proposal-reprioritization` | — | **open** (the brief's Q6; registered 2026-09-06) |
| 54 | whether credit is a stored object or a read model | `authority_model.md#initiative-proposal-reprioritization` | — | **open** (the brief's Q8; registered 2026-09-06) |
...
rather than their number, as 13 and 14 are. Twenty-one rows are open — 31; 36, opened on the operator's 2026-09-05 memos on how tasks come into existence and
...
each argued under a heading naming its subject in the document that owns it; and 42, opened by the skills leg of the migration; and 43 and 44, which the conformance-suite design opened from the two places a from-zero run found no rule to test; and 45, opened by the workflow-format pass from what ten real declarations could not say, and argued in `adapters.md`; and 55, opened by the second workflow-format pass from what nine more could not say, argued beside 45 in `adapters.md`; and 46 to 54, the questions `authority_model.md` had carried as the brief's Q1–Q8 and the raiser question since before the register existed, registered by the consistency pass of 2026-09-06 without renumbering the brief's labels (C13, marked open beside them, was found settled by C9 and decision 37 and is marked so in place); and 56, opened by the testability pass of 2026-09-06 from the one enforcement point the conformance suite found unnamed, and argued in `gates_and_workflows.md`; 37 to 41 were opened and ruled
