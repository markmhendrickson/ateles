...
| 96 | how the proxy authenticates to the instance on an agent's behalf — passing through the agent's own credential, so the instance matches grants on the `sub` presented and attribution is the agent's directly, or holding its own credential and acting for the agent, with the A-for-B attribution the design already requires carried in the write | `authority_model.md#where-a-harness-reaches-the-record-and-what-admits-the-request` | decision 97, which ruled that a proxy is permitted and never the enforcement point, so this row is reached by any deployment that puts one in the path; the open tenancy rows, which would make the answer load-bearing once a second operator's data is on the instance | **open** (2026-09-08, opened by the proxy ruling): pass-through keeps the record's own admission meaningful and duplicates the check; proxy-held makes the proxy a principal whose compromise reaches everything and puts the whole boundary on one hop |
