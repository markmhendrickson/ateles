...
**74**. This line has now been stale four times: it read 63 with 64 already on the table (flagged by the
self-awareness pass), was left unfixed by the peering pass that opened 65, was corrected to 66 by the
close-out pass while 67, 68, and 69 were landing, and is corrected again here. **Read the table, not this
line, when it disagrees with the table** — the table is the record and this line is a convenience, and a
convenience that has been wrong every time it was checked earns that caveat in place.

**70 and 71 are assigned on branches not yet merged here, and this branch leaves them unused.** Decision 70
(the `project` field against the scoping key on a declaration) is ruled on PR #766's branch and 71 (a task
whose inputs cannot be resolved) on PR #770's, neither of which has merged into this one. The naming pass of
2026-09-06 therefore took 72 and 73 rather than the two numbers the table here appears to leave free, so
that no number carries two questions when those branches land. The next author checks the open PR branches
as well as this table before assigning, which is what this pass did to arrive at 74.
