...
| 95 | whether each harness the swarm runs configures its own credential to the instance of the record, or reaches the record through the swarm's own proxy | `authority_model.md#the-swarm-reaches-the-record-through-a-proxy-that-admits-never-through-per-harness-credentials` | decisions 41 and 94, whose default-deny admission has no enforcement point at all under per-harness credentials; decision 91, whose several-instance routing this locates; decision 42, which ruled which tools a harness may invoke and not what the harness points at | **ruled** (2026-09-08): through the proxy — not for configuration economy but because the proxy is the only interposition point where admission can be refused, and per-harness credentials put a bearer token to the whole graph in every agent's hands, leaving decisions 41 and 94 unenforceable by construction rather than imperfectly enforced. Three consequences are rules: an unresolved caller identity is refused and never passed through, principle 5 putting the default on the field carrying the safety meaning; decision 94's read boundary is checked here, this being the only component every read passes through; and multi-instance routing resolves at the proxy from the deployment's binding rather than in each harness's configuration |
| 96 | how the proxy authenticates to the instance on an agent's behalf — passing through the agent's own credential, so the instance matches grants on the `sub` presented and attribution is the agent's directly, or holding its own credential and acting for the agent, with the A-for-B attribution the design already requires carried in the write | `authority_model.md#the-swarm-reaches-the-record-through-a-proxy-that-admits-never-through-per-harness-credentials` | nothing in decision 95, whose interposition point exists under either branch; decision 82, which makes the answer load-bearing once a second operator's data is on the instance | **open** (2026-09-08, opened by the proxy ruling): pass-through keeps the record's own admission meaningful and duplicates the check; proxy-held makes the proxy a principal whose compromise reaches everything and puts the whole boundary on one hop |
