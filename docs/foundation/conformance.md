...
**76**, established on 2026-09-07 by sweeping the open pull requests against this directory rather than by
trusting this line — which had gone stale on five consecutive passes, and was stale again at 66 while 67, 68, and 69
sat on the table. The numbers taken on unmerged branches at that sweep: 70 (#766), 71 (#770), 72 and 73
(#773), 74 (#774), and 75 (this pass). **A line naming the next free number cannot be kept true by the pass
that reads it**, because the numbers that invalidate it are opened on branches that have not merged — so
the rule is the sweep, and this line records what one sweep found on its date rather than promising to stay
current (principle 11: a value a process must keep true goes quietly wrong when that process stops).
