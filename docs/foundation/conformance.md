...
| 100 | whether a newly declared workflow is proven before it binds production work, and against what — the declaration write is gated as a governance action and the first batch routed through it is the first time any step of it has ever run, so what the gate judges is the declaration as written and never as executed; candidates are the implicit proving the review step already performs, an explicit proving run before the declaration binds, and proving conditioned on the blast tier the write already resolves to | `work_model.md#whether-a-newly-declared-workflow-is-proven-before-it-binds-production-work` | nothing in the design; whether `migration.md#ordering-and-the-cutover-for-the-skills` is right that its cutover batch is the test the conformance suite runs for any declaration | **open** (2026-09-09, on the operator's question). Four mechanisms bear on it and none covers it: the migration's dual-run cutover, which proves on a real batch and guards a skill's retirement rather than a declaration with no predecessor; `failure_posture.md#the-rules`' exercised-on-a-cadence rule, scoped to `recovery_paths[]`; the conformance suite's disposable instance, whose subject is the engine's rules from zero; and the review step, which is what makes implicit proving a candidate rather than a vacancy. Decision 76 is what makes a non-production instance coherent rather than a fork. Any answer is bounded by invariant 6, since a proving step that re-does the review step's work is a second gate, and by principle 11, since `workflow` is a governance type with one writer and an `enabled` field on it would be maintained state |
