...
| 87 | whether a harness may provide a principal any capability its grant does not name — the general rule decision 41 states for entity-type writes and decision 42 states only as a parity test between two lists, which detects divergence rather than forbidding it; and, if it may not, what non-exceedance requires of a capability surface no one can enumerate (the shell, the filesystem a process reaches by where it runs) and what a harness's default configuration must therefore be | `authority_model.md#whether-a-harness-may-provide-a-capability-the-grant-does-not-name` | nothing in the design; whether the measured gap between every harness allowlist and every grant is a violation or a vacancy | **open** (2026-09-07, on the operator's principle that no agent runs in a harness offering reach beyond what its grant confers, and his stated intent to grant any possible access rather than tool-mediated access alone. Registered open and deliberately unruled. Was blocked on decision 86, which ruled on 2026-09-08: the shell and the harness's own tools are nameable now, as reserved surfaces, so this row is unblocked and still open — 86 supplied the vocabulary and left the rule) |
