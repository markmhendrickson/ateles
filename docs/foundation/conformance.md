...
| 91 | whether a deployment that names several instances of the record names one of them as where its own governance writes go, or whether the design requires a deployment to hold exactly one such instance | `adapters.md#where-a-swarm-is-deployed-and-what-its-deployment-names` | decision 90, which leaves it open rather than settling it by omission; the explicit instance binding, which fails closed when ambiguous and is the mechanism an answer would use | **open** (2026-09-08, opened by the deployment-target pass): no rule in decision 90 depends on the answer, and the several-instance case is a routing question that ruling does not reach. **The operator has since supplied the substance of an answer and the row stays open** (2026-09-08, the decisions 81 to 83 pass): in the memo that retired decision 82's framing he stated that an instance will have at least one instance of the record or many, that where there is more than one **one of them needs to be the controlling instance**, and that the others stay relevant because the swarm navigates and leverages them — a user may want some workflows in one and others in another. That is this row's first disposition, named, and it is recorded here rather than ruled because the memo answers the question the row asks while leaving the terms a ruling needs: what "controlling" governs beyond the governance writes this row names, whether it is a property of the deployment or of the record, and what a deployment with several instances and no controlling one does. Ruling it inside another row's pass is the collision this register exists to prevent, so it is argued and ruled where it lives |
