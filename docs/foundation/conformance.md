...
Amendment history: `revisions.md#conformancemd`.
## Purpose
...
| Decision 78 ruling | `execution/scripts/check_foundation_decision_78.py`, in `scripts/lint.sh` | the decision-register row beginning `\| 78 \|` does not have `**ruled**` in its status cell, or `adapters.md#whether-the-instance-of-the-record-serving-a-swarm-is-an-external-system-when-the-swarm-operates-it` is missing or still opens with `**Open` rather than `**Ruled` |
...
| Register narrative | `execution/scripts/check_foundation_register_narrative.py`; fail-then-pass coverage in `execution/scripts/test_check_foundation_register_narrative.py` | a **present-tense** claim in this document's prose naming which rows are open whose list does not equal the table's own open rows; any prose stating a next free number as a value, right or wrong, since no value survives a concurrent branch taking it; or prose asserting a named decision's present status that its row contradicts. The register's **dated** statements — "the open rows *were then* four", "after this pass: 40, 55, and 64" — are history, are correct as written, and are deliberately not read: the check reads tense, not archives, so a historical sentence whose numbers are simply wrong is invisible to it. It reads no document but this one and proves no general prose-to-table consistency — it binds the shapes that recurred |
| Front matter and rule indexes | `execution/scripts/check_foundation_front_matter.py`, in `scripts/lint.sh` | a `Revised by …` clause still in a document's front matter, or such a document with no `revisions.md` pointer to reach its history through; and a section stating three or more full-sentence rules with no **The rules in this section.** list opening it — the two halves of decision 74, which the amendment obligation re-accretes one clause and one rule at a time unless something reads for them (`#a-documents-amendment-history-lives-in-revisionsmd-not-in-its-front-matter`). `status.md` and `revisions.md` are exempt: the first is a dated report outside the reading list, the second is the companion itself |
...
| Core workflow step lists / fast paths / successors | the `workflow` entity for (declaration scope, type) | `docs/foundation/workflows.md` tables via `render_workflow_docs.py --check` (prose authored in the file) |
...
- Every ruled decision now has a heading of its own.
- Decisions 37 to 41 were ruled by derivation on 2026-09-06 and held for the operator's veto; the operator reviewed all five the same day and upheld them.
- Decisions 31, 33, and 42 are argued under headings of their own in authored companions.
...
| 13 | whether a batch may hold on a condition discovered mid-flight | `work_model.md#a-batch-may-hold-on-a-condition-discovered-mid-flight` | — | **ruled** (2026-09-05): yes; the step owner records a finding naming the condition, writes no verdict, and renews the lease; no held state, no field; bounded by verdict, checkpoint, or lapse |
| 14 | whether a batch may depend on a task it created | `work_model.md#a-batch-may-depend-on-a-task-it-created` | — | **ruled** (2026-09-05): yes, as a case of 13; a `DEPENDS_ON` edge from the batch to the task, never a field; the verdict is refused while it is unended; a cycle is refused at write and at attach, and one found later escalates every batch in it as `dependency_cycle` |
...
| 27 | whether a payment's approver must be shown what the verifier signed | `payments.md#a-payments-approver-is-shown-exactly-what-the-verifier-signed` | — | **ruled** (2026-09-05): yes; the checkpoint carries payee, amount, currency, period, and rail as the `verify` verdict recorded them, and `pay` is taken only on those parameters |
...
| 30 | how a recurring task is modelled | `work_model.md#a-recurring-task-is-one-live-instance-and-its-completion-creates-the-next` | — | **ruled** (2026-09-05, on the operator's proposal): one live instance carrying its own `recurrence` rule; its closing verdict creates the next instance, `FOLLOWS` task to task, with `due_date` computed from the schedule and never from the completion; the reschedule-instead-of-complete pattern is superseded for tasks modelled this way; an action series is a different thing and the two meet only at the gate |
...
| 32 | whether a verdict's `conclusion` is a stored field or a read over its findings and its author | `gates_and_workflows.md#whether-the-conclusion-is-a-stored-field-or-a-read-over-the-findings-and-the-author` | — | **ruled** (2026-09-06): a stored field, kept as the verdict's own projection of its findings and its author and reconciled at the write by the refusal at submission; under derivation a verdict with no finding would be `signed`, and the stated conclusion is what the findings are read back against |
...
| 36 | whether an intake rule may key on the work model's own records (task, batch, lease, verdict, action, checkpoint, agent session), or only on artifacts and the swarm's other entities | `work_model.md#whether-an-intake-rule-may-key-on-the-work-models-own-records` | the evaluator's subject set; a rule on any other type is unaffected | **ruled** (2026-09-06): a rule keys on no work-model record type — `task`, `batch`, `lease`, `verdict`, `action`, `checkpoint`, `agent_session` — and on any other type, a field a step wrote on an ordinary entity included; a rule naming one of the seven is refused at the write; the operator's lean toward every type was considered and set aside, the listener it asked for being met on every ordinary entity |
...
| 40 | what a verdict records at close about the session that produced it | `gates_and_workflows.md#what-a-step-leaves-at-close-what-it-produced-and-a-reference-to-what-it-read` | — | **ruled** (2026-09-06; narrowed by the sign-off-provenance pass — the pass name keeps the since-retired spelling, 2026-09-06): a verdict records what its step produced and names what it read, as of `signed_at`, never copying it; it does not itself carry the step's reasoning, and reasoning is not re-derived from one. The sentence this replaces overreached — it called the session's turns "not an entity the design has," which decision 63 has since made false by registering `conversation`, `conversation_message`, and `session_digest` (`data_model.md#record-conventions`) as the types that hold them; the corrected text says so in place rather than leaving the false claim beside a later ruling that contradicts it. **The edge revision 46 proposed is now ruled, not merely proposed:** a verdict may carry `REFERS_TO` → `session_digest` (never `agent_session`, which carries no content, and never the raw turn store, the wrong grain for a reference a reader resolves as of a time), and must where the signer is an agent and a digest exists for its session; permitted, never required, for a non-agent signer, since a human verdict carried in through an adapter (`adapters.md#what-the-adapter-does-with-every-event`) has no session behind it to name. The minimization question revision 46 flagged for this row is settled elsewhere, not here: decision 63 registered the three types and their special-category eligibility (`data_model.md#record-conventions`), and decision 64 ruled their owner and reader in part, leaving only their field-by-field shape open — this row's own question was never minimization, and does not reopen it. |
...
| 44 | whether a verdict from a step owner whose lease has lapsed closes the step | `conformance_suite.md#whether-a-verdict-from-a-step-owner-whose-lease-has-lapsed-closes-the-step` | — | **ruled** (2026-09-06): a verdict carrying `signed` or a blocking conclusion requires a held lease by its signer at the moment of the write; a late one is refused at submission, and the current lease holder's conclusion is the one that can stand; the operator's `waived` is the one verdict written without a lease, under its own rule |
...
| 54 | whether credit is a stored object or a read model | `authority_model.md#credit-is-a-read-model-over-attribution` | — | **ruled** (2026-09-06): a read model over attribution — verdicts, actions, and observations with the principals they carry — never stored, as the `authority_chain` is not |
...
| 68 | whether registering an entity type needs an amendment obligation parallel to opening a decision or adding a vocabulary term | `#amending-a-foundation-document` | decision 67, whose fourth mode this rule closes | **ruled** (2026-09-06): yes — a PR that registers a type, or names one the swarm now depends on, adds its `data_model.md` row in the same change, the same obligation `#amending-a-foundation-document` already states for a decision and a term. `finding` (G15) and `conversation`/`conversation_message`/`session_digest` (decision 63) are the two prior instances this rule would have caught before a later pass had to supply the row |
| 69 | whether a per-agent credential on an external system that issues them is an obligation or a preference, and what carries attribution where a system issues none | `adapters.md#a-per-agent-credential-is-an-obligation-where-the-system-issues-one` and `adapters.md#when-attribution-cannot-be-external-the-record-does-not-suffice-on-its-own` | — | **ruled** (2026-09-06): an obligation where the system issues one — a shared credential there loses external attribution, makes revocation all-or-nothing, and leaves the host unable to carry a verdict at all, since a conclusion-shaped delivery from it resolves to no single agent; where the system issues none, the shared credential is admitted and the outbound artifact carries a mark disclosing that an agent acted, its absence a refusal on the same footing as a credential-less operation, its wording the operator's policy; the binding is declared on the `vendor_binding` as a capability slot on decision 42's pattern, with the grant matched on the credential as before and no new home |
| 70 | whether the `project` field on `workflow` and `batch` is a reference to the planning record at decision 57's `project` level — a task acquiring one from its ascent — or a separate deployment-scope term that shares the word and must be renamed under invariant 12 | `planning_model.md#project-names-a-planning-level-the-scoping-key-on-a-declaration-is-a-different-term` | how a task acquires the scope its intake batch carries (`conformance_suite.md#what-the-documents-leave-unspecified-here-and-how-each-is-recorded`); whether `*|intake`'s `*` is a wildcard or a rendering convenience; the roster's per-scope binding against one global map (`migration.md`, G20) | **ruled** (2026-09-06, the operator's): two concepts sharing one word. The planning level keeps `project`; the scoping key on `workflow` and `batch` is renamed `declaration_scope`, the term [declaration scope](vocabulary.md#declaration-scope), swap-tested against `instance`, `domain`, and `tenant`. G20's global roster is what settles it — a planning record cannot key a map there is one of on an instance — and invariant 12 forbids the one word carrying both. How a task acquires its scope stays a gap, with the ascent removed as a candidate answer |
| 71 | what happens to a task whose inputs cannot be resolved — the record holds no instance of a required read and no adapter could import one — and who judges sufficiency, hydration or the step owner | `failure_posture.md#a-task-whose-inputs-cannot-be-resolved-is-put-to-the-operator-not-executed-on-a-guess` and `gates_and_workflows.md#declaration-batch-projection` | — | **ruled** (2026-09-06): both, split on presence versus adequacy — hydration raises `underdetermined_inputs` on a required read that resolved to nothing, before the step opens; an input found but too thin to act on is the step owner's judgement and it holds under decision 13 |
| 72 | whether the record that closes a step was named for one of its own outcomes — it carries three conclusion values of which only `signed` is an approval, and `sign-off` named that one value — and, if the record is renamed `verdict`, what the field it carries is then called | `vocabulary.md#whether-the-record-that-closes-a-step-is-named-for-one-of-its-own-outcomes` | nothing in the design; a rename of ~970 sites across the corpus, executed as one pass | **ruled** (2026-09-07): the record is the **`verdict`**; the field it carries is the **`conclusion`**. `sign-off` and `sign_off` are retired. `outcome`, the first choice on both sides, **failed the swap test on two independent grounds**: a verdict *is* one of the four inbound outcomes `data_model.md#concepts` enumerates, so `verdict.outcome` would reproduce one level out the same recursive defect this decision removes; and `outcome` is already named on the planning record's forbidden-field list ("a stored `status`, `outcome`, or progress"), so the word would sit on a forbidden list and a required one at once. `resolution` failed on three bound senses over 220 uses with `disposition` already downstream of *resolve*; `type` failed earlier on ~858 uses in a different sense. `conclusion` passes: one generic occurrence corpus-wide, reworded here because it was pejorative, with no entry, anchor, field, or plural use. **The section's own reasoning is corrected in the same pass:** it had recommended `outcome` because the word "carries no entry of its own in this file," and that test is unsound — a term is bound if the corpus enumerates, numbers, anchors, or forbids it, whether or not it has an entry. Not renamed: the `signed` conclusion value, `SIGNED_BY`, `signed_at`, the verb phrase *to sign off*, and `status.md`'s historical revision log. |
| 73 | whether a vocabulary term should prefer a single word, and what exception admits the qualified compounds the no-overlap half of invariant 12 currently depends on | `principles.md#12-as-few-terms-as-the-design-needs-and-no-fewer-no-term-overlaps-another` | the form of any future term | **ruled** (2026-09-07): no single-word preference — a term takes as many words as the design's distinction requires, and no more, and the qualifier must be load-bearing. Stated as a test in the form the invariant already uses: drop the qualifier; if what remains is a term already bound elsewhere, or no longer names what the entry defines, the qualifier is load-bearing and the compound stands; if what remains names the same thing, the compound is redundant with its head and the swap test retires it. There is no exception because there was never a rule to except from — the swap test was always sufficient, applied to the qualifier rather than to the whole term, and 73 was open only because the exception had not been named. A single-word preference was rejected on three grounds, each checked against the corpus. It would have forced coinages that hide kinship: `parent task`, `child task`, and `recurring task` are each defined as a kind of `task`, and single words for them would conceal that. It would have collided with terms already bound: `decision point` and `enforcement point` both collapse to `point`, and `enforcement point` is defined in terms of `decision point`; `gate` is already bound, as the abbreviation of `action gate` and nothing else; `separation of duties` has no single-word form in any control vocabulary. And it would have undercut the no-overlap half of the very invariant it was meant to serve: `declaration scope`, `waiver scope`, `finding scope`, and `permission scope` are four distinct terms *because* the qualifier distinguishes them, each entry's own **Not for:** line forbidding its use for the others, so the preference pushes toward either four coinages that no longer show they are scopes of one kind or one word `scope` overlapping four ways — which is the violation invariant 12 exists to prevent. Where a preference and the invariant pull against each other, the invariant wins. No term is renamed: the ruling changes no term's form, it states the test. Measured on this branch, `vocabulary.md` carries 136 `###` headings, of which 41 are multi-word; one of the 41 is the prose subsection `### Two things a reader is looking at, and how they are written`, which is the only non-term among the 136, leaving 135 terms and 40 multi-word ones |
...
| 78 | whether the instance of the record serving a swarm is an external system under decision 45's precedent, the record under `vocabulary.md#record`'s singular definition and the boundary sentence that states Neotoma into the swarm, or a third thing — and what writes the record of an operation that suspends the record, since a restart, a redeploy, or a migration of the serving instance leaves the verdict and the gate that judge it with nowhere to be written | `adapters.md#whether-the-instance-of-the-record-serving-a-swarm-is-an-external-system-when-the-swarm-operates-it` | decision 45, whose action classes are the candidate to extend; decision 55, which rules a peer's instance and not the swarm's own; decisions 76 and 77, which hold several instances as identity and routing and not as lifecycle; `failure_posture.md`'s halt, which binds the unreachable interval whatever its cause and leaves only the deciding half open | **ruled** (2026-09-07, on the operator's answer to how far he intends the swarm to operate its own infrastructure unattended): **it is an external system when the swarm operates it, and decision 45's action classes extend to the substrate** — the instance is the record when it is the place a write goes and an external system when it is the object of the operation, a classification by moment and not a relocation of the boundary, so the record stays inside for every rule that reads or writes it and what sits outside is the serving process decision 45 already placed there. The boundary sentence is met on its own next clause — a governance write and a lossy record mutation are actions "not because of where they go, but because of what they can destroy" — and suspending the instance is the limiting case of that test. `vocabulary.md#record`'s **Not for** clause survives limb by limb: no adapter reaches the record under this ruling, since every read and write of an entity still goes to the record directly and the adapter reaches the process, whose artifact is a process and a host and never an entity; the entry's **See** list gains the section, its prohibition unchanged. **Write ordering is ruled with it**: the permitting verdict and the gate's decision are written before the operation begins, because a verdict that cannot reach the record is not one, and the completion is written after the instance answers again — nothing is parked off-record in between. An operation that never completes leaves the permit standing and the completion unwritten, resolved by the operator from `recovery_paths[]` and never by a retry; one that returns in a state the permit did not approve is caught at the completion write, which is not written, the divergence recorded as a finding and the correction taken as a second action under a second permit. Two dispositions were eliminated by the operator's answer that he wants the swarm operating its own infrastructure unattended as far as governance allows: **the operator's by hand outside the swarm**, which caps unattended operation permanently by construction and still leaves his own act unrecorded, and **out of scope for P1–P2**, which would leave the design silent about an operation already being planned. **Record elsewhere and reconcile after** was the closest rival and lost on his qualifier "within governance": it puts the deciding half outside the gate model and owes a second write path its own authority, durability, and conflict rules — governance-adjacent rather than governed — which is the write-ahead-log anti-pattern `failure_posture.md`'s rule 4 names by name |
| 79 | whether `tenant_id` is an opaque UUID or a human slug readable in an AAuth subject (`monedula@acme-swarm`) and in logs — the document leans slug with immutable UUID backing, and records the lean as the operator's to take | `multi_tenant.md#the-tenant-identifier-is-a-human-slug-with-an-immutable-uuid-behind-it` | the subject form every key is minted under, which decision 80 shares | **ruled** (2026-09-08, the operator taking the lean this row records as his to take): a **human slug**, with an **immutable UUID** behind it — the slug is what appears in an AAuth subject and in operational output an operator reads, where readability is operationally real, and the UUID is what every stored reference keys on, so a rename rewrites the display form and re-keys nothing. The rename pain the UUID side was priced against is paid off by the backing identifier rather than accepted. **Decision 80 is the load-bearing ground**: the tenant is matched on the grant as `match_tenant` and is not derived from the subject, so the slug is an addressing and display convenience and never an identity source — which is what makes a rename survivable, since no admission decision reads it. Decision 80 stated the compatibility from its own side, that `match_tenant` holds whatever value this row settles on. **Cost accepted**: two identifiers for one tenant, and the discipline that the slug is never the thing matched on, stated as a rule because the shape does not enforce it. **What would reopen it**: a consumer matching on the slug rather than on `match_tenant`, which would make the slug identity-bearing and reintroduce the rename problem decision 80 prevents |
| 80 | whether a tenant derives from the AAuth `sub` (`<name>@<tenant>-swarm`, self-describing, no extra field) or from a separate `match_tenant` on the grant (flexible if one identity ever spans tenants) | `multi_tenant.md#7-settled-decisions-were-the-operators` (decision 2) | every key already minted as `<name>@ateles-swarm`; the document states "decide before more keys are minted", and a cutover mints keys | **ruled** (2026-09-07, on the operator's answer that a grant should govern everything an agent can do via tooling): **matched on the grant**, as `match_tenant`, and not derived from the subject. The ground is that the grant is the whole statement of what a principal may do — a capability names the tools a principal may invoke and a harness allowlist is one enforcement of it (`vocabulary.md#grant`, decision 42) — so a subject-derived tenant would put one authorization question in a second place, which invariant 9 forbids; and a grant's tool half, the half the operator's answer turns on, carries no subject to derive from, so deriving would leave exactly that half outside the tenant boundary. `authority_model.md#grants` already binds a human's grant to a principal and a tenant, so deriving for agents would give two classes of principal two homes for one term. Decision 76 does not settle it the other way: it bounds how many identities a principal holds, on a case its own row states is one tenant, and where the tenant is recorded is a different question (invariant 12). **Deriving from the subject was rejected**, and its cheapness — every key already minted in the `<name>@ateles-swarm` form, which would need no change — is explicitly not a ground: these documents state the design and the implementation follows, so a mechanism chosen because the existing keys already have its shape is the implementation choosing the design. Decision 79's identifier form is untouched and `match_tenant` holds whatever it settles on. **Buildable in full since decision 86 ruled on 2026-09-08**: a tool capability is an ordinary entry of `capabilities[]`, so `match_tenant` scopes it as it scopes a record capability, and a shell or filesystem capability is now a capability there is something to scope. **Migration is a backfill, not a reshape**: no minted key is invalidated, every existing grant takes the single sentinel tenant `multi_tenant.md` section 2.2 already prescribes, and the assignment is mechanical with no per-agent judgement until a second tenant exists — with the one obligation that a grant carrying no `match_tenant` fails closed rather than reading as any-tenant |
| 81 | whether within-tenant default visibility is private-first ("see only my own book") or shared-first ("see everything in the tenant") when the org case arrives — a product and RGPD posture question, not a technical one | `multi_tenant.md#what-this-design-requires-of-the-records-sharing-model-and-of-an-operators-reach-within-it` | the soft-wall of that document's section 5 | **ruled** (2026-09-08, with 83, on the operator's answer that this swarm works with whatever sharing model the instance it is configured with applies): the default belongs to the record's design, not to this corpus, and what this corpus states instead is a requirement of whatever the record sets — **a read this design performs never narrows what the record admits, and never widens it**. Grounds: deciding the default here would put one answer in two places (invariant 9) and would decide it where the check does not run, since decision 97 puts the enforcement point at the record; a component that narrowed an admitted read would be the second gate 97 rejected under principle 6 and would vanish with the component (principle 1); and a projection returning what the record would refuse is an enforcement point failing open, which principle 5 forbids. A degraded read fails closed and is put to the operator. Under decision 82 the rule is live on every instance, not only an org one |
| 82 | whether the fork case is a single hosted instance serving many forkers or one instance per forker — Goal 6 says their own, which makes tenant isolation deployment-level; a hosted multi-forker offering would move it to row level and make section 3 load-bearing rather than belt-and-suspenders | `multi_tenant.md#every-instance-may-have-one-operator-or-many-and-hosting-does-not-decide-it` | whether that document's section 3 is a hedge or a requirement | **ruled** (2026-09-08, on the operator's answer): **the question is retired rather than answered**. Every instance may have one operator or many, whoever hosts it and wherever it is deployed, so multi-operator is a property every instance has and not a consequence of a hosting arrangement — there is no answer to give on the axis the row named, since it asked which of two arrangements makes several operators possible and both do. **Section 3 is load-bearing**, settled in the direction the row reserved for the hosted-multi case by a different route: its per-operator dimension is what separates two humans sharing an instance, and a deployment boundary does not supply it (principle 1). The framing is retired explicitly because the operator rejected it twice — the second time saying he was unsure what "forker" meant, a word this document coined for a role nobody occupies, since a person who forks is an operator of what they then run. The deployment-versus-row distinction survives as a statement about tenants and is silent about operators. Section 1's table and section 6.2's trigger are falsified a second time (decision 76 was the first) and are annotated rather than edited, their rewrite being a pass of its own |
| 83 | the capability ceiling of a non-owner operator within a tenant: whether minting agent keys, amending an agent's definition, or changing the `priority_rubric` tenant-wide are theirs or the owner's — the enforcement is deferred, but the intended ceiling is to be recorded now so grants are shaped consistently | `multi_tenant.md#what-this-design-requires-of-the-records-sharing-model-and-of-an-operators-reach-within-it` | the shape of grants minted before the org's own ceiling is enforced | **ruled** (2026-09-08, with 81, on the operator's answer that it depends how an operator is configured and that an operator may itself need configuration data in the record): the ceiling belongs to the record's authority semantics, and this corpus states the requirement instead — **a principal's reach through this design never exceeds what the record's own model permits it**, since every action is taken as some principal under its grant, no component holds a credential reaching further, and no ceiling is enforced here that the record does not enforce (decision 41's default-deny, at decision 97's enforcement point). The ceiling is therefore inherited by construction rather than by discipline. Separately ruled on the operator's invitation to challenge his distinction: **an operator is not a tenant owner, and the corpus already holds them apart** — `operator` is a principal type (C9), owning is a relation carried by an `ownership_grant` conferring the approver's seat and nothing else (decision 46), section 2.1 enumerates four distinct actors, and a tenant is a boundary no read or key crosses rather than a party. No conflation was found; section 4.1's "tenant owner" is a roster-resolved seat, not a fifth actor |
| 84 | what distinguishes a task with no intake batch because it is still being assembled — several agents each writing part of it — from one with no intake batch because intake has not yet picked it up; `work_model.md#intake-is-every-tasks-first-workflow` makes both simply unrouted, and nothing separates them | `work_model.md#what-distinguishes-a-task-being-assembled-from-one-intake-has-not-reached` | nothing in the design; whether an assembling task is claimable before it is finished being written | **open** (2026-09-07, on the operator's question; a `draft` **status** specifically is disfavoured on three independent grounds — C1 and invariant 11, the invariant-12 collision with `draft` as a step name in `workflows.md`, and the status vocabulary's `open`/terminal scope — but the need it answers is real and unmet) |
| 85 | where an artifact a principal produces about an issue or a pull request lives: in its own place on the host, one artifact one author, or merged into a body several principals rewrite — the general question behind the specification block agents contribute sections to today, and it reaches a review verdict and a triage summary the same way | `github.md#where-an-agents-artifact-about-an-issue-or-a-pull-request-lives` | nothing in the design; which outbound row of `github.md` a principal's own artifact takes, and whether the answer is the same for every artifact class | **open** (2026-09-07, on the operator's question; registered open and deliberately unruled. Five rules bear on it without settling it — invariant 9's one home, C1 and invariant 11 read one level up from a task's fields, `authority_model.md#attribution` binding a write in the record but not a rendering on the host, `adapters.md`'s four outcomes classifying only what arrives inbound, and decision 69, which ruled the identity half and not the content half) |
| 86 | by what grammar a capability names a tool, and therefore what the two sides of decision 42's parity test compare: whether the surface half is the MCP server alone or a capability surface that also admits the harness's own tools and the shell, whether a wildcard is expressible and at which tier, what a grant means for a provider that receives no allowlist, and whether the harness's list is eventually derived from the grant or only held equal to it | `authority_model.md#a-capability-names-a-tool-as-toolsurfaceoperation-and-that-is-what-a-harness-allowlist-is-compared-against`; the mapping in [`docs/tool_grant_grammar.md`](../tool_grant_grammar.md), ratified by the ruling | — | **ruled** (2026-09-08): one grammar, `tool:<surface>:<operation>`, an ordinary capability of the tuple with `param_constraints` for the argument bound; the five senses of "tool" differ in what mediates them and not in how a grant states them, and a second entry form would be a second home for one bound. A record operation over entity types keeps its own capability and is not re-expressed as a `tool:` op; filesystem reach is what a harness or shell capability does, not a domain beside them. Two reserved surfaces, `harness` and `shell`. `tool:*` is expressible and refused at the write; `tool:<surface>:*` is admissible where the surface is enumerable at check time, resolving `Indeterminate` where it is not — with `tool:shell:*` admitted knowing its bound cannot be met, because refusing it leaves the reach outside the record. The parity test compares normalized sets, reading the grant's parsed tool capabilities with empty as failure and the harness's built arguments rather than its declared field, failing on divergence in either direction. An unnamed tool is denied; breadth is written by enumeration rather than by absence, so "any possible access" is a grant of every surface wildcard and stays expressible. Decision 87's rule, the derivation-versus-parity sequencing decision 42 left open, and which tools each principal should hold are all left unruled |
| 87 | whether a harness may provide a principal any capability its grant does not name — the general rule decision 41 states for entity-type writes and decision 42 states only as a parity test between two lists, which detects divergence rather than forbidding it; and, if it may not, what non-exceedance requires of a capability surface no one can enumerate (the shell, the filesystem a process reaches by where it runs) and what a harness's default configuration must therefore be | `authority_model.md#a-harness-provides-only-what-a-grant-names-and-a-provider-that-does-not-enforce-is-a-capability-the-grant-names` | decision 86, which supplied the grammar this rule is stated in and deliberately admitted the `tool:shell:*` case this row had to judge | **ruled** (2026-09-08, on the operator's principle that no agent runs in a harness offering reach beyond what its grant confers): a harness provides a principal only the capabilities its grant names, the configuration closed by default and a grant the only thing that opens anything, stated over the whole surface rather than over its enumerable part because a rule scoped to what can be listed is decision 41's rejected wildcard shape one level up. The measured gap is therefore a violation and not a vacancy. A harness whose non-enumerable reach cannot be bounded may carry granted work where a grant names that reach, and may not where none does — its use is itself a capability in decision 86's grammar, the reserved surfaces being what name it, so refusing the unbounded harness and naming its use are one act rather than two dispositions; the stricter reading is declined because no harness the swarm runs could satisfy it, which would relocate the reach rather than remove it, the failure 86 named one capability down. Routing a principal to a provider that enforces no bound provides that principal the capability of executing unbounded, which the grant must name: the router widens nothing on its own account because a grant has already opened or refused the path, and an unnamed routing is `capability_denied` before the effect, never `capability_unavailable`, which decision 60 gave to a capability temporarily missing rather than to a bound the path cannot supply. Breadth stays writable — the operator's intent to grant any possible access is every surface wildcard enumerated, unnarrowed by this rule, which removes only the unwritten reach. Left open: decision 42's derivation-versus-parity half, which tools each principal should hold, how a provider declares whether it enforces, and the migration from an instance whose conforming set is empty) |
| 88 | whether the design states an autonomy goal — that a return to the operator not required by risk, authority, or an input the record cannot supply is a defect — and what a proposed return is checked against | `principles.md#the-autonomy-the-design-is-for-the-operator-is-involved-wherever-involvement-is-needed-and-nowhere-it-is-not` | decision 18, whose per-class reservation is the involvement a fresh instance starts with and which this goal makes a thing to be justified rather than merely accepted; the attention paragraph of `principles.md`, which protects the cost of an interaction and not whether it happens | **ruled** (2026-09-08, on the operator's stance): the operator is involved always wherever involvement is needed and never when it is not, bootstrap inside the rule rather than exempt from it. It is a criterion applied per return, never a target the design converges toward and never a count: a return is necessary where only a principal can supply the missing authority, judgement, or fact — the never-set at the gate, `capability_denied`, `undetermined_scope`, `unclaimed_step`, `lossy_record_mutation` — and a return with none of those properties is a finding against whatever produced it, judged on the standing axis like any other. A return that reaches nobody is worse than an unnecessary one, which is the reason `blocked` as a status stays retired in favour of the checkpoint edge. No mechanism reads a count of returns, because a rule that made them scarce would create pressure to hide them |
| 89 | whether the design states that a write a subsequent step depends on must be readable by the principal that will execute that step, and where that condition is stated | `gates_and_workflows.md#declaration-batch-projection` | what principle 2's read-back obliges, and of whom; what a declared read returns when a predecessor's write is not yet readable | **ruled** (2026-09-08): yes — a step does not open until the write it depends on is readable by the principal that will execute it; stated as ordering and never as an interval, on the step's opening rather than on its declaration, because the reader is the only principal that can check it; principle 2's writer-side read-back is untouched and does not cover the cross-principal case |
| 90 | where a swarm is deployed, and what a deployment must name: whether the target host is a parameter of the deployment or a property of the software; what a deployment does when it names no instance of the record to read and write; and whether an off-host recovery path is a deployment-time obligation or an operations one | `adapters.md#where-a-swarm-is-deployed-and-what-its-deployment-names` | decision 45, whose host-as-external-system ruling and action classes this extends to a teardown rather than adding to; decision 35's one binding type, which carries the obligation and gains no third name; the restore obligation at `failure_posture.md#the-operator-invoked-halt-and-what-undoes-an-action-already-taken`, whose `recovery_paths[]` this constrains in where it may point rather than in what it is | **ruled** (2026-09-08): a deployment **names a target host and the instance of the record it will read and write, and deploys a companion instance where it names none**. Three rules: the target is a parameter, and naming none selects the host the deployment is run from, so a swarm is never written for one host and ported to others; a deployment with no instance to name deploys a companion one, because the design keeps no state outside the record and a swarm with nowhere to write has nowhere to put a verdict, a gate decision, or a task; and the recovery path is named **at** the deployment and points **off** the host being deployed to, since a path held on the host whose loss it recovers from passes every cadence check until the moment it is needed. Teardown is governed by the existing gate and adds one property the gate does not supply — a teardown reaches only what its own deployment created, because the gate judges whether an action may be taken and never what it is aimed at. Destruction stays destructive; what the off-host path changes is the size of what is lost. **Deployment** is the single name, installation being a step within it and not a second act; no term is added to `vocabulary.md` and no type is registered, each piece having a home already. Held out of this directory as operational: which hosts exist, what any is called, how a companion is placed on a given provider, and every command — all per-instance binding, on the line `conformance_suite.md#from-zero-the-disposable-instance-and-why-it-cannot-be-the-production-one` already draws when it marks creating an instance an infrastructure item named there and built elsewhere |
| 91 | whether a deployment that names several instances of the record names one of them as where its own governance writes go, or whether the design requires a deployment to hold exactly one such instance | `adapters.md#which-of-a-deployments-several-instances-takes-its-governance-writes` | decision 90, which leaves it open rather than settling it by omission; the explicit instance binding, which fails closed when ambiguous and is the mechanism an answer would use | **ruled** (2026-09-08): a deployment naming several instances of the record names one of them as its controlling instance; the mark is a parameter of the deployment and not a property of any instance; and a deployment naming several instances and no controlling one fails closed. *Controlling* governs the governance writes this row names and nothing further — the narrow reading of the two the question bears, taken because narrowing later is a breaking change and widening later is not, with the wide reading named in the ruling (the swarm's own working state in the controlling instance, secondaries holding only domain content) so a later widening amends a stated scope rather than a silence. Keeping the scope narrow is also what leaves every named instance fully the record, which is decision 55's ruling, and refusing the wide reading here is decision 86's shape one level up. The mark sits on the deployment on two grounds: principle 11, since a mark on an instance would need something to clear it when the deployment is decommissioned and nothing would, and decision 90, which already made the target host and the instance parameters of the deployment. The consequence is accepted rather than refused — whether an instance is controlling is answerable only relative to a deployment, and two deployments naming it may disagree, which no check can refuse without enumerating deployments no instance holds. The no-controlling-one case is ruled to decision 76's fail-closed binding rather than decision 90's companion rule, whose scope is stated here as a deployment naming no instance at all: nothing is ambiguous where nothing is named, and 90 applied to this case would stand up an instance nobody asked for. A first-named or own-host default is the resolving-to-whichever-instance-is-configured-first that decision 76 refused by name. Cost: a deployment that would have run does not, until the operator names one. Decision 98 gains a candidate home for swarm-level planning and is not otherwise reached |
| 92 | what makes a task **live** — the predicate a queue reads to partition a backlog into work some principal could take now and work that is held, and whether stating it needs a term beside `claimable` | `work_model.md#a-task-is-live-when-some-principal-could-claim-it-now` | decision 37's `blocked` retirement, which is what leaves a task carrying that status in neither part of the partition | **ruled** (2026-09-08): live is `claimable` read existentially over the backlog — not terminal, no lease held, no open checkpoint holding it — rather than the 1:1 "is this mine" a claim asks from one principal's seat; the alternatives are refused on the corpus's own grounds, an open-checkpoint test being exact but not partitioning (an unheld uncheckpointed task and an abandoned one take one value under it) and a recency test being the stored staleness principle 11 refuses on the same reasoning as a liveness flag; no term is minted (invariant 12) and no `live` field exists, the word naming a reading of a defined term the way *the claimable pool* already reads it. **Amended (2026-09-08, revision 89):** the partition is three parts, not two — live, under a held lease, and finished or held — because a nonterminal task whose lease is held is claimable by nobody and is neither terminal nor checkpointed, so the two-part statement lost every actively executing task between its halves; the middle part is described in the terms `lease` and its derived `held` state already supply and is given no name, a one-word "executing" being the status beside the lease edge principles 6 and 11 refuse; a row holding the retired `blocked` status is therefore not a further part beside the three but one of exactly two — an open checkpoint is raised on it naming what holds it, or it loses the status and returns to the pool — and the count of such rows is a measurement for `status.md` |
| 93 | whether principle 11's heading states the invariant it is cited for — the body tests state that *would need a watchdog, a sweeper, or a reconciler to stay correct*, while the heading reads as a categorical preference for edges over fields, and the registered `task` type carries `title`, `description`, `priority`, `due_date`, and `recurrence` as fields that no process maintains | `principles.md#11-state-that-needs-a-watchdog-belongs-in-a-relationship-not-a-field` | the conditional already in the principle's own second sentence and restated at `data_model.md#concepts`, which is the form every citation of it across the corpus uses; the descriptive task fields, which the principle has never been read to forbid | **open** (2026-09-08, opened by the liveness-partition amendment): raised by an external review reading the heading as the test and finding it false against the task type's descriptive fields. The body is already conditional and no citation in the corpus reads it categorically, so nothing depends on the answer and no repair follows from it; what is open is whether a heading that overstates the body should be narrowed, the operator's to decide, since a heading is what a reader meets first and this one was read as the rule by a reader who had the body in front of it |
| 94 | whether the design states read admission per entity type, as decision 41 states write admission — and whether reads and writes are admitted symmetrically, given that a per-type read refusal discloses which types an instance registers | `authority_model.md#grants` | decision 41, which ruled the write side and deferred the read side to a pointer that describes a boundary rather than ruling one; the exposure an unbounded read leaves across entity types within one owner's data, no multi-operator premise being available since decisions 81, 82, and 83 are all open | **ruled** (2026-09-08): default-deny per entity type, the grant is the allowlist, and the check runs at the read; symmetric with the write side, the existence-disclosure objection refused because what a per-type denial discloses is a registered type — schema data this corpus already publishes — while what symmetry prevents is every type of the grant owner's own data reaching a principal with a reason to hold one, which no later write retracts; `context_entity_types[]` is not the admission mechanism, being a provisioning list on the agent that says what is handed over and is consulted at no read, while admission is matched on the credential — but it remains the inner bound decision 41 and the record-usage contract state, and a read must satisfy both (amended, revision 71, 2026-09-08, which also narrowed the cross-operator claim and removed a citation of decision 82 as ruled); the rule admits a type and not a row, leaving which rows a principal sees to the open tenancy rows |
| 95 | whether each harness the swarm runs configures its own credential to the instance of the record, or reaches the record through the swarm's own proxy | `authority_model.md#where-a-harness-reaches-the-record-and-what-admits-the-request` | decisions 41 and 94, whose default-deny admission the ruling claimed had no enforcement point under per-harness credentials; decision 91, whose several-instance routing this located; decision 42, which ruled which tools a harness may invoke and not what the harness points at | **reopened** (2026-09-08, revision 71) after being ruled the same day. The ruling rested on a proxy being the only place a request can be refused, leaving 41 and 94 "unenforceable by construction" under per-harness credentials. That ground is false and contradicts decision 56, ruled 2026-09-06, which put the same question, named a proxy in front of the record among its candidates, rejected it on principle 6 as a second gate on the swarm's own record, and located the refusal at the record. Decision 95 did not cite 56. Its own reopening condition — a record whose admission check reads the requesting principal — is the topology 56 ruled. Superseded by decision 97, which ruled the enforcement point at the record and permitted a proxy that is never it; nothing in decision 94 depended on decision 95 |
| 96 | how the proxy authenticates to the instance on an agent's behalf — passing through the agent's own credential, so the instance matches grants on the `sub` presented and attribution is the agent's directly, or holding its own credential and acting for the agent, with the A-for-B attribution the design already requires carried in the write | `authority_model.md#a-proxy-passes-through-the-agents-own-credential-and-holds-none-of-its-own` | decision 97, which ruled that a proxy is permitted and never the enforcement point, so this row is reached by any deployment that puts one in the path; the tenancy rows, which make the answer load-bearing once a second operator's data is on the instance | **ruled** (2026-09-08): **pass-through** — a proxy authenticates by presenting the agent's own credential, holds none of its own, and acts for no agent, so the instance matches grants on the `sub` actually presented and attribution is the agent's directly. Three grounds. Decision 97 put the enforcement point at the record and never at the proxy, and pass-through is what makes that check run on the principal it is for: a proxy credential would have the record matching the proxy's grant against a request made for an agent, which either defeats the check or forces a second identity resolution inside the request, the first failing open at the moment the enforcement point is reached and the second making the proxy load-bearing for admission, which 97 ruled it is not. Invariant 9 gives one home to the answer to which principal made a request; a proxy credential plus A-for-B attribution puts it in two places that can disagree, with nothing in the path obliged to notice. And decision 97 already names the proxy as the single point of compromise whose credential is not scoped to a principal — a proxy holding its own credential is that hazard realized, where pass-through keeps every credential's blast radius one principal's grant. **Cost accepted**: the one proxy benefit 97 names that pass-through forfeits — one place to rotate a credential rather than one per harness — since rotation is per principal; instance routing (decision 91's need) and observability survive untouched, neither requiring the proxy to hold a credential. **What would reopen it**: a deployment topology in which a harness genuinely cannot hold its own credential, making pass-through unimplementable rather than merely costly |
| 97 | where the enforcement point for an agent's read and write to the record sits — the record's own admission check against the requesting principal's grant, the swarm's proxy in front of it, or both with one of them load-bearing | `authority_model.md#where-a-harness-reaches-the-record-and-what-admits-the-request` | decision 56, which ruled the enforcement point at the record for a governance write and rejected a proxy on principle 6; decision 41, which says its allowlist is read at every enforcement point; decision 95, reopened into this row; decision 94, which is indifferent to the answer | **ruled** (2026-09-08): the record's own admission check against the requesting principal's grant; a proxy in front of the record is permitted and is never the enforcement point. Three grounds — decision 56 ruled this two days before decision 95 restated it, naming the same three candidates, ruling the sole-writer grant, locating every other write's refusal at the record, and rejecting the proxy on principle 6 by name; a record-side check needs no component alive for a refusal, where a proxy-side one is absent whenever the proxy is down or bypassed by a direct connection (principle 1); and a proxy is a single point of compromise for every operator's data, which is the hazard decision 96 names. What a proxy keeps is real and none of it is enforcement — one place to rotate a credential, one place to resolve which instance a request reaches (decision 91's need), and observability — so a deployment may put one in the path and loses nothing, and one that does not is no less bound. Decision 95's three consequences survive as conditionals on a proxy existing, each a correct statement about a proxy and none an argument for one; the bearer-credential blast radius is accepted and answered under custody, not with a second gate |
| 98 | whether an instance's planning records are per instance — each instance of the record carrying its own missions, strategies, projects, and plans — and, where a swarm is configured with several, whether a declared read may relate one instance's planning to a task bound to another | `planning_model.md#whether-an-instances-planning-records-are-per-instance-and-how-a-swarm-reading-several-relates-them` | decision 91, whose controlling instance is the candidate home for any swarm-level planning; decision 76, whose non-merge rule any answer must satisfy | **ruled** (2026-09-08, opened the same day by the decisions 81 to 83 pass from the operator's memo that retired decision 82's framing): **per instance**, and a **declared read may name another instance** under three conditions the corpus already supplies. The per-instance half is a **consequence** and not a choice — the ascent gives a planning record and a task at most one `PART_OF` edge upward, the collision mechanism refuses an `amend_<level>` whose referred record is not the first on the producing task's ascent, and decision 76 makes one operator's several instances several records; a hierarchy spanning two would put two records on one ascent and leave the first record on it undefined, so the shape is one the ruled mechanism cannot express rather than one refused on a reconsiderable ground. The cross-instance read reverses the recommendation that had been argued from decision 76: that rule's fail-closed posture is about an **ambiguous** binding — a read resolving to no instance, or to several with no rule to pick — and a read that names its instance is the explicit binding 76 asks for, not the configuration-order resolution it refuses by name. The corpus already permits reads further away: a step declares reads of systems outside the boundary through `adapters.md`'s admission contract with a stated freshness, and decision 55 rules a peer instance is the record extended by replication and not an external system, so forbidding the nearer case would be incoherent. The three conditions: the read **names its instance explicitly**, an unnamed one still failing closed under 76; it **declares a freshness**, which stays a derived read over sourcing and coverage and never a stored field (`vocabulary.md#freshness`); and **admission applies at the named instance**, decision 97 putting the enforcement point at the record and decision 94 making read admission default-deny per entity type, so a cross-instance read is admitted or refused by the far instance's grant and not by the reader's — the property that makes the mechanism safe. This is a **read and not a write**: decision 91 puts governance writes at the controlling instance, and the collision mechanism refuses a cross-instance amendment unchanged. Decision 76's separation rule survives intact and is about a different question — an orchestrator combining two stores into one brief has crossed a sensitivity boundary however the content arrived, which binds every use of this mechanism without being a prohibition on the mechanism existing. The third half the row opened with — whether the swarm has planning of its own — is split out as decision 99 rather than left as an unruled remainder, following decisions 82 and 91 |
| 99 | whether a swarm configured with several instances of the record has planning of its **own** — records about its operation belonging to no one instance — or whether every planning record is some instance's | `planning_model.md#what-this-does-not-reach-a-read-is-not-a-write-and-the-separation-still-binds` | decision 91, which supplies a controlling instance as a candidate home without supplying a record needing one; decision 98, which rules the two halves this one does not share premises with | **open** (2026-09-08, separated from decision 98 when its other two halves were ruled): kept as its own row rather than as 98's remainder because a row partly ruled and partly open cannot be read from its status field, which is the division decisions 82 and 91 each took; and because what remains turns on whether a class of record about the swarm's operation exists at all, not on the ascent, the collision check, or the admission point that settle 98. A home being available is not a record needing one |
| 100 | whether a newly declared workflow is proven before it binds production work, and against what — the declaration write is gated as a governance action and the first batch routed through it is the first time any step of it has ever run, so what the gate judges is the declaration as written and never as executed; candidates are the implicit proving the review step already performs, an explicit proving run before the declaration binds, and proving conditioned on the blast tier the write already resolves to | `work_model.md#whether-a-newly-declared-workflow-is-proven-before-it-binds-production-work` | nothing in the design; and `migration.md#ordering-and-the-cutover-for-the-skills`, whose claim that its cutover batch is the test the conformance suite runs for any declaration this row put in doubt and this ruling corrects rather than makes true — the suite runs from zero against named fixtures and tests whether the engine enforces the design's rules, never whether a particular declaration does useful work | **ruled** (2026-09-09): **proving is conditioned on the blast tier** — a declaration above some tier must be proven before it binds production work, and one below need not. Three grounds. The declaration write already resolves to a tier under the instance's `action_policy` at the moment the action would be taken, so conditioning proving on that resolution extends machinery the design has rather than standing a second one beside it, which is what invariant 6 asks; the rejected universal obligation would have been a new gate in front of every declaration write, where this is a condition on the existing one. Proving establishes what signing does not, which is the invariant-6 objection any answer had to survive and is answered rather than deferred: a step owner's verdict judges a declaration **as written** and a proving run judges it **as executed**, the same claim for a step executed by code — where a proving step would indeed be the review step repeated — and different claims for a step executed by a principal reasoning under a model, since two declarations that read identically can behave differently. And the tier is the right axis on proportionality: a declaration's blast radius is the reason to prove it, since a declaration changes how every future batch of its type is executed, and the tier is the design's existing expression of blast radius, so the obligation sits where the argument for it is strongest and lifts where it is weakest. What stands unchanged: the recovery-path cadence rule, still scoped to `recovery_paths[]`; the review step and its correction loop, which below the threshold remain the whole of what catches a bad declaration; and the refusal of `replay`, so proving is a first doing on a written-for-the-purpose population and never a second doing of a stored one. Principle 3 binds whatever proving turns out to be — a proving run that has never gone red proves nothing, so it needs its own planted positive — and the shape the ruling points at is the advisory path, `route_task` resolving as the enforcing gate does with a parity test holding the two equal. **Cost accepted**: the tier is assigned to the **write**, the act of declaring, and not to the declaration's future behavior, so it is a proxy and is accepted as one; the case this does not catch is a declaration whose write scores below the threshold and whose executed behavior turns out consequential, and the correction loop remains what catches it, as it does today. **What would reopen it**: a demonstrated class of low-tier declarations whose executed behavior is harmful often enough that the correction loop is not the answer, which would be evidence the write's tier is too weak a proxy rather than evidence proving should be universal; or the tier ceasing to resolve at the write, which would remove the axis. **Three things this leaves open deliberately**: which tier is the threshold, a value in the instance's `action_policy` and not a design constant, the same shape as the design's other tier-conditioned rules; what a proving run runs against, bounded already by the refusal of replay and by the disposable instance refusing any non-empty store, with decision 76 making a separate instance coherent rather than a fork, so a substrate is expressible and naming the actual ground is not this ruling; and how the unproven state is expressed, which is an edge or the absence of a proving record and not a flag, since a `workflow` is a governance type with one writer and an `enabled` field on it is the maintained state invariant 11 forbids |
| 101 | what fields the credential-binding edge carries, and what resolving a credential to a principal reads — `data_model.md#concepts` states the binding many-to-one and names no edge type for it, gap G17 recording that the instance holds the bindings as a scalar field on the agent and on the grant, which it calls a credential binding living on the wrong entity; candidates are a credential entity with an edge to the principal, properties on the binding edge itself with one edge per credential, and the grant as the binding with no separate edge | `authority_model.md#what-the-credential-binding-carries-and-what-a-check-reads-to-resolve-a-credential-to-a-principal` | stage 1 of the migration, which registers `principal_binding` among thirteen relationship types and cannot be written as enumerated under either candidate — an added edge is a fourteenth type, and loading the existing one changes what that type carries; G26, which makes the registration one-way, so the shape registered is the shape the design lives with | **open** (2026-09-09, on the operator's question). Three constraints bound any answer and the corpus already supplies each. The many-to-one is a requirement rather than an observation, since a principal holds a store identity, a host login, an address, and a chat identity at once. Rotation makes that plurality simultaneous: `authority_model.md#grants` rules the dual-admit window, in which two credentials bind to one principal with the set of matching grants never empty, so an edge that cannot express two live bindings makes the rotation rule unimplementable and any candidate failing it is refused on that ground alone. And what a check reads is a second half of the question, since decision 96 ruled pass-through and decision 97 put the enforcement point at the record, where grants are matched on the credential and reach the principal only through whatever resolves one to the other. `conformance_suite.md` already proposes a field set — the credential kind and value, the principal, and an expiry — and records it as proposed rather than assumed, which is the vacancy this row registers. Bounded by invariant 11, which is the argument for the edge and equally the test of its fields, and by principle 5, whose unattributed-write rule any answer inherits |
| 102 | whether acyclicity is a property of a relationship type or of the graph, and what the check on relationship writes does when a type is registered — the corpus reads it as per-type in two places and states neither reading as its own requirement; candidates are per-type declared at registration, no check with acyclicity left to the caller, and a universal check the design's edges conform to | `data_model.md#whether-acyclicity-is-a-property-of-a-relationship-type-or-of-the-graph` | stage 1 of the migration, since a check applied without regard to type governs every type the vocabulary comes to hold, and two of the design's own thirteen are the direct case — `FOLLOWS`, which the design reads as a sequence and never as constrained, and `ownership_grant`, whose object may itself be a principal; G25, which holds the same stage on the record admitting new relationship types at all and is a separate dependency this one does not wait behind | **open** (2026-09-09, on the operator's question). The per-type reading is asserted twice and argued nowhere: `work_model.md#a-batch-may-depend-on-a-task-it-created` calls `PART_OF` and `DEPENDS_ON` the record's hierarchical relationship types, cycle-checked at write, and names a loop through `ADDRESSED_BY` invisible to that per-type check, which is why the writer's walk is the second check; `planning_model.md#the-hierarchy-is-edges-and-a-task-has-one-line-upward` leans on the same reading to bound the ascent. Contradiction X-15 noticed the seam and closed it on the division of enforcement — the record refuses what its per-type check sees, the writer refuses the cross-type loop, the shape decision 56 ruled — taking the per-type reading as premise rather than subject, which is why this is a row and not that contradiction reopened. This is the record's code and the design's question in the form `#what-the-design-requires-of-the-substrate-regardless-of-who-builds-it` already states: the requirement is stated here and the specification that implements it belongs beside G25's. Bounded by invariant 6, since a declaration at registration extends type registration rather than standing a second mechanism beside it; by principle 1, since a check on one of several write paths reports rather than binds, so an answer must say whether making it bind is part of the answer; and by principle 5, which makes the unstated type's default the field carrying the safety meaning |
| 103 | whether a step's declaration is a floor or a ceiling, and whether it covers writes as well as reads — the declared reads bind asymmetrically today, since a step that cannot read a type it declared does not proceed at runtime while a step that reads a type it did not declare is a declaration error caught in the pull request, and no counterpart to `reads_to_enter` and `reads_to_close` names what a step writes; candidates are requirements only with the floor made explicit and the undeclared left to review, an exhaustive declaration of reads and writes refused at runtime when exceeded, and symmetry for writes without a runtime ceiling on either side | `gates_and_workflows.md#declaration-batch-projection` | decision 41 and decision 94, which rule admission per entity type per principal, default-deny, checked at the write and at the read — the outer bound this question sits inside rather than reopens; decision 97, which puts the enforcement point at the record, where the requesting principal is visible and the step it executes is not, so a step-scoped refusal is not available without changing what that point reads; the write-declaration gap itself, which is a defect however this rules and is tracked as its own issue | **open** (2026-09-09, on the operator's question). Three rules bound any answer without settling it. Invariant 1 holds candidate 1 to owning that its ceiling half never binds — nothing checks that a step touched only what it declared, so review is reporting and not a control at runtime. Invariant 9 and invariant 6 hold candidate 2 to saying why a per-step ceiling is not a second home for what a principal may touch and not a second gate beside the record's admission check, the shape decision 97 already rejected; `data_model.md#concepts` supplies the relation it would have to lean on, that a step's declared reads narrow what the grant admits and neither may widen the other. And the attestability argument for candidate 2 is decision 100's, that a declaration as written and as executed are different claims where no code determines the step — which is why the row states it rather than treating the enforcement-point cost as decisive. What none of the three may do is collapse `underdetermined_inputs` into `undeclared_dependency` (`failure_posture.md#a-task-whose-inputs-cannot-be-resolved-is-put-to-the-operator-not-executed-on-a-guess`): hydration runs before the step, so a genuinely unanticipated read is not a declared dependency the step could not reach, and candidate 2 owes which class it raises. External systems are not the subject and are not reopened — `adapters.md#the-adapter-runs-before-and-after-a-step-never-during-it` leaves no live path out of a step to constrain, which is topology and not admission |
...
pass argues its own ruling under its own heading. **The sign-off-provenance pass (named for the since-retired spelling of [verdict]), rebased onto both, ruled
40** (narrowed to a verdict's shape at close, its overreaching sentence corrected, its proposed
`REFERS_TO` → `session_digest` edge ruled, required on an agent's verdict) — the last of the two rows
...
new carrier). **The open rows after all four passes: zero** — every decision 1 through 65 was then ruled,
ruled in part with a stated remainder, or withdrawn/unassigned (19, 20, 21, 22, the existing gaps below).

**The naming pass of 2026-09-06 opened 72 and 73, from the operator's memos, and ruled neither.** 72 asks
whether `verdict` names its record neutrally, argued in `vocabulary.md` beside the `Owner` section, which
is the same kind of finding about a different word; 73 asks whether a term should prefer a single word,
argued under invariant 12 in `principles.md`, which is the invariant it would qualify. They are registered
open rather than ruled deliberately, on two grounds. First, both are the operator's own: each came from his
...
now, while the evidence is gathered, rather than left to be re-derived later. Both have since been ruled — the
verdict-rename pass of 2026-09-08 executed 72, and the decision-73 ruling found that the sequencing bound
only 72, the ruling stating a test and renaming nothing. **Which rows are
open is read from the status column of the table above, and is not restated here:** a list written in prose
is a second source for what the table already answers (principle 9), and it goes stale the moment the next
pass rules a row, which is how this sentence came to name two rows while the table named twelve.
...
- A decision opened in a document is registered in the same change.
- A registered type is amended into `data_model.md` in the same change.
- A revision is recorded in `revisions.md`, not in the document's front matter.
...
need not be complete on the type's every field the day it lands — `data_model.md#record-conventions`
...
9,462 characters standing between the title and the first rule; on `vocabulary.md`, 19 clauses and 8,298
...
- **Mode 4 is the design's.** The type already exists in a correct Neotoma the moment it is registered; what
  ...
  `#amending-a-foundation-document` and the term rule already close for a
...
method is that every rule has a failing artefact (`conformance_suite.md#the-method-every-rule-gets-its-failing-artefact`), and a
