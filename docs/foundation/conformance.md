...
| 98 | whether an instance's planning records are per instance — each instance of the record carrying its own missions, strategies, projects, and plans — and, where a swarm is configured with several, whether a declared read may relate one instance's planning to a task bound to another, and whether the swarm has planning of its own belonging to no one instance | `planning_model.md#whether-an-instances-planning-records-are-per-instance-and-how-a-swarm-reading-several-relates-them` | decision 91, whose controlling instance is the candidate home for any swarm-level planning; decision 76, whose non-merge rule any answer must satisfy | **open** (2026-09-08, opened by the decisions 81 to 83 pass from the operator's memo that retired decision 82's framing): registered in `planning_model.md` rather than absorbed into 82, whose subject is the tenancy axis and not the hierarchy (invariant 12); the per-instance half looks derivable from decision 76 and the cross-record collision rule, and the two halves that are not — a read across instances, and planning belonging to the swarm itself — are what keep it open |
