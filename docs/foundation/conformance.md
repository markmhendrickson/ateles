...
| 84 | what distinguishes a task with no intake batch because it is still being assembled — several agents each writing part of it — from one with no intake batch because intake has not yet picked it up; `#intake-is-every-tasks-first-workflow` makes both simply unrouted, and nothing separates them | `work_model.md#what-distinguishes-a-task-being-assembled-from-one-intake-has-not-reached` | nothing in the design; whether an assembling task is claimable before it is finished being written | **open** (2026-09-07, on the operator's question; a `draft` **status** specifically is disfavoured on three independent grounds — C1 and invariant 11, the invariant-12 collision with `draft` as a step name in `workflows.md`, and the status vocabulary's `open`/terminal scope — but the need it answers is real and unmet) |
