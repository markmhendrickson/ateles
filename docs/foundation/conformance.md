...
| 33 | whether a stage, and the `phase` field on a step, names anything a step does not | `workflows.md#whether-a-stage-names-anything-a-step-does-not` | — | **ruled** (2026-09-06): drop `steps[].phase`; the Stages line stays as authored prose, one home for the grouping, and no mechanism reads either; the prose's own wording is left to the condensation pass |
...
| 40 | what a step records at close about the session it ran in | `gates_and_workflows.md#what-a-step-leaves-at-close-what-it-produced-and-a-reference-to-what-it-read` | — | **ruled** (2026-09-06): what it produced is written as the entities it is; what it read is named on the sign-off and reproduced by an as-of read at `signed_at`, never copied; its reasoning is not written; `agent_session` stays the identity and liveness half. **Restated by the session-reconciliation pass (2026-09-06),** narrowing the flag the Human Inversion mapping pass left here: the operator's standing practice of persisting every session turn (`conversation`/`conversation_message`, unregistered in this design; `session_digest`, 369 on prod, already named above as the licensed path for a session whose content is itself work input) is not the drift the essay-contradiction framing suggested — those are not the `agent_session` this ruling constrains, and the ruling was never a claim that no session is stored. The live question this row now poses is narrower: does a sign-off gain a `REFERS_TO` edge to the session (or its digest) that produced it, and is a session's content, once reachable from a task, obliged to the special-category and minimization rules `data_model.md#record-conventions` states for a *registered* type — given `conversation_message` and `session_digest` are not currently registered types this design's own minimization-at-write rule reaches. Proposed, not ruled, in `gates_and_workflows.md`'s decision-40 section; the minimization question is carried as F27 below rather than settled here. |
...
| 61 | where an intake class's *default ancestor* is declared, now that every task is expected to have one — a field on the level's registered type, an `intake_rule` condition, or a `completion_criteria[]`/`applies_when` entry on the `planning` workflow's own declaration | `planning_model.md#whether-an-orphan-is-a-defect-at-all` | decision 57, which fixes what the levels are before a per-level field has anything to attach to | **open** (the ancestry pass, 2026-09-06, reframed by the priority pass of 2026-09-06 after the orphan default reversed; the design leans toward `intake_rule` as the natural site for a per-class default, without pre-empting 57, since an `intake_rule` default is the one candidate that names a record rather than a level and so does not wait on 57 at all; this pass's ruling on the pre-reversal framing of 61 is superseded by the reversal and not applied) |
| 62 | whether an instance may bind a principal to take the highest-standing claimable task of its class ("must"), rather than the default read a principal merely consults ("may") | `work_model.md#priority-orders-the-claimable-pool-it-does-not-enter-it` | — | **open** (the priority pass, 2026-09-06; this document's default is "may" — a 1:1 fitness judgment a principal may decline any claimable task on, priority included — argued from `#pull-is-the-only-delivery-assignment-constrains-eligibility`'s refusal of routing-by-inference; a "must" per class would need its own fallthrough rule for the fitness case a "must" cannot itself judge, which is the operator's to write if a class ever needs it, and no rule above builds one) |
...
the highest-standing claimable task), argued in `work_model.md`. The open rows are now seven — 33, 34, 55,
57, 59, 60, and 62.
