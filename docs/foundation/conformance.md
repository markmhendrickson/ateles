...
review (2026-09-04), and the operator's 2026-09-05 review of review relevance (revision 19: the `applies_when` condition on an optional step, and two terms retired in favour of `review step`), and revision 21 (2026-09-05: the `gmail.md` and `calendar.md` keyed rows and their canonical-source entries), and revision 24 (2026-09-05: the `telegram.md` and `payments.md` keyed rows and their canonical-source entries), and PR #745 operator review (2026-09-05, rulings 13–14, 16–18, 23–29: twelve register rows moved from open to ruled), and the operator's 2026-09-05 rulings of decision 15 and decision 30 (revision 27: the last open row moved to ruled, and the recurring task registered as ruled on the operator's proposal), and revision 28 (2026-09-05: `migration.md` registered as an authored companion, and decision 31 opened), and PR #745 operator review (2026-09-07, revision 70: decisions 76 and 77 opened — the several-instances topology, and the out-of-set citation this document's scope does not rule on). Amendment history: `revisions.md#conformancemd`.
...
| 74 | where a foundation document's amendment history lives, given that the front-matter chain had reached 28 clauses and 9,462 characters on this document alone — ahead of every rule it states, and inside the budget decision 66's reading projection is meant to protect | `#a-documents-amendment-history-lives-in-revisionsmd-not-in-its-front-matter` and `revisions.md` | decision 66, whose projection this competes with for the reader's first screen, and `#amending-a-foundation-document`, whose obligation this relocates without weakening | **ruled** (2026-09-06): a companion, `revisions.md`, one table per document, one row per revision — the obligation to record an amendment in the same change is unchanged, and only its location moves. A per-document collapsed section at the foot was rejected because it leaves twenty places to look for one corpus-wide question ("what changed in the foundation this week") and still grows the file the projection is extracted from; compressing the chains in place was rejected because it treats a placement problem as a length problem, and the chain would re-accrete at the same rate. The rollout is complete and corpus-wide, not staged: all twenty authored documents carry a pointer to their `revisions.md` table instead of a chain, 156 clauses moved, 44,004 characters of front matter removed (55% of the corpus's), and every clause preserved verbatim except a bare relative anchor qualified so it still resolves from the new file. `status.md` is exempt — a dated report, never keyed and never inlined, whose revisions are its own sections. The second half of the ruling, that a section stating three or more full-sentence rules opens with the list of them, reached 33 sections in the same pass. What holds it is mechanical rather than editorial: `check_foundation_front_matter.py` runs in `scripts/lint.sh` and fails on a `Revised by …` clause left in a front matter, on a document with neither a chain nor a pointer, and on an un-indexed rule section — so the amendment obligation cannot re-accrete one clause at a time. Per-document coverage and what each pass changed are in `revisions.md`; the same file records that no document remains unmigrated |
...
| 78 | whether the instance of the record serving a swarm is an external system under decision 45's precedent, the record under `vocabulary.md#record`'s singular definition and the boundary sentence that states Neotoma into the swarm, or a third thing — and what writes the record of an operation that suspends the record, since a restart, a redeploy, or a migration of the serving instance leaves the sign-off and the gate that judge it with nowhere to be written | `adapters.md#whether-the-instance-of-the-record-serving-a-swarm-is-an-external-system-when-the-swarm-operates-it` | decision 45, whose action classes are the candidate to extend; decision 55, which rules a peer's instance and not the swarm's own; decisions 76 and 77, which hold several instances as identity and routing and not as lifecycle; `failure_posture.md`'s halt, which binds the unreachable interval whatever its cause and leaves only the deciding half open | **open** (2026-09-07): dispositions are extend 45 to the substrate, treat the serving instance as the record and put the operation outside the swarm as the operator's by hand, record the operation elsewhere and reconcile it after, or declare it out of scope for P1–P2 and revisit; the operator's, and a non-goal is an answer |
...
whether `sign-off` names its record neutrally, argued in `vocabulary.md` beside the `Owner` section, which
