...
| 94 | whether the design states read admission per entity type, as decision 41 states write admission — and whether reads and writes are admitted symmetrically, given that a per-type read refusal discloses which types an instance registers | `authority_model.md#grants` | decision 41, which ruled the write side and deferred the read side to a pointer that describes a boundary rather than ruling one; decision 82, whose several-operators-per-instance ruling is what makes an unbounded read a cross-tenant exposure rather than untidiness | **ruled** (2026-09-08): default-deny per entity type, the grant is the allowlist, and the check runs at the read; symmetric with the write side, the existence-disclosure objection refused because what a per-type denial discloses is a registered type — schema data this corpus already publishes — while what symmetry prevents is another operator's rows reaching a principal with no reason to hold them, which no later write retracts; `context_entity_types[]` is not this rule, being a provisioning list on the agent that says what is handed over and is consulted at no read, while admission is matched on the credential; the rule admits a type and not a row, leaving which rows a principal sees to decision 82's tenancy question |
| 95 | whether each harness the swarm runs configures its own credential to the instance of the record, or reaches the record through the swarm's own proxy | `authority_model.md#admission-is-checked-at-the-record-and-the-proxy-is-a-second-place-it-is-checked` | decisions 41 and 94, whose default-deny admission this decision locates rather than creating; decision 91, whose several-instance routing this locates; decision 42, which ruled which tools a harness may invoke and not what the harness points at | **ruled** (2026-09-08): through the proxy — because the proxy is the only interposition point where admission can be refused, leaving decisions 41 and 94 with nowhere to be checked under per-harness credentials. **Amended (2026-09-08, revision 71): that reason was false and the conclusion does not stand on what remains.** Admission is matched on the credential a request presents, and the record holds both the request and that credential, so the record can run the check and the proxy is not the only place it can — the built state is measured in `status.md`. The rule is therefore that admission is default-deny per entity type and the design names no single component that must run it; **the record's check is authoritative**, because it holds however a harness connects, cannot be bypassed by connecting directly, and does not make access depend on a swarm component being up. The harness still reaches the record through the proxy and holds no direct instance credential, but as a deployment posture resting on the reasons the ruling named and disowned — one place to configure and rotate, one place that sees every request, a second refusal in front of the record's — and never as a property the design requires; where the two checks disagree the record refuses. Of the three consequences, two stand as rules — an unresolved caller identity is refused and never passed through, principle 5 putting the default on the field carrying the safety meaning; and multi-instance routing resolves at the proxy from the deployment's binding rather than in each harness's configuration — while the third is narrowed: decision 94's read boundary is checked at the proxy as well and is not located there, the claim that the proxy is the only component holding the request and its matched credential being false of the record. The amendment is taken by the decision's own reopening test, which had already been met when it was written |
| 96 | how the proxy authenticates to the instance on an agent's behalf — passing through the agent's own credential, so the instance matches grants on the `sub` presented and attribution is the agent's directly, or holding its own credential and acting for the agent, with the A-for-B attribution the design already requires carried in the write | `authority_model.md#admission-is-checked-at-the-record-and-the-proxy-is-a-second-place-it-is-checked` | nothing in decision 95, under either branch a request reaches the record; decision 82, which makes the answer load-bearing once a second operator's data is on the instance | **open** (2026-09-08, opened by the proxy ruling): pass-through keeps the record's own admission meaningful and duplicates the check; proxy-held makes the proxy a principal whose compromise reaches everything and puts the whole boundary on one hop. **Weighted, not ruled, by decision 95's amendment (2026-09-08, revision 71):** once the record's own check is authoritative, proxy-held is the branch that disables it — the instance matches grants on the proxy's `sub`, so every agent behind it is admitted as one principal holding the union of their capabilities |
| 97 | which admission check is authoritative where both the record and the proxy run one, and whether a deployment may run only the proxy's | `authority_model.md#admission-is-checked-at-the-record-and-the-proxy-is-a-second-place-it-is-checked` | decision 95's amendment, which states the record's check as authoritative and the proxy's as defence in depth, and leaves the conformance obligation unstated; decisions 41 and 94, which state the admission without naming a component that must run it | **open** (2026-09-08, opened by the decision 95 amendment): requiring the record's check of every deployment makes conformance testable at one component and refuses substrates that cannot check; permitting a deployment to rely on the proxy alone, recording the weaker posture, admits a deployment against a third-party record that cannot enforce |
