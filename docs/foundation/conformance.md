...
| 36 | whether an intake rule may key on the work model's own records (task, batch, lease, sign-off, action, checkpoint, agent session), or only on artifacts and the swarm's other entities | `work_model.md#whether-an-intake-rule-may-key-on-the-work-models-own-records` | the evaluator's subject set; a rule on any other type is unaffected | **open** (2026-09-06, on the operator's 2026-09-05 memos; the operator's lean is toward every type, the document proposes the exclusion) |
...
| 43 | what the bootstrap set is — the closed list of records an operator writes before the gate can hold — and whether the operator's own governance writes after bootstrap are gated or exempt | `conformance_suite.md#what-the-bootstrap-set-is-and-whether-the-operators-later-governance-writes-are-gated` | the audit shape of the operator's own governance writes; decision 56, put beside it | **ruled in part** (2026-09-06): the bootstrap set is closed and enumerated, and the list is the thirteen-record table in `conformance_suite.md#the-minimal-record-set-in-order`, every member read back, a write to those types after the set exists never provisioning; **open**: whether the operator's own governance write after bootstrap is gated or exempt — the design leans to gated, and the residue is what the operator wants to read back later; ruled with 47, or not at all |
...
| 47 | whether the raiser of a checkpoint may resolve it | `authority_model.md#approval` | decision 43's audit shape for the operator's own governance writes; `payments.md` and `telegram.md` name it as bearing on their checkpoints | **open** (registered by the consistency pass of 2026-09-06) |
...
| 50 | which structural checks apply at a dozen principals, and where the threshold lives | `authority_model.md#the-thresholds-home-is-the-action_policy-per-class` | — | **ruled in part** (2026-09-06): the thresholds' home is the `action_policy`, per action class — `quorum` and `disjoint_roles[]` beside `confidence_threshold` and `consent_tolerance`, fail-closed where absent (every awaited principal, every named pair); **open**: which checks apply — an m-of-n on which classes, which role pairs beyond payer and verifier — organizational values the operator holds |
...
| 52 | what unit stops when an initiative is accepted, who confirms it, and who may propose | `authority_model.md#initiative-proposal-reprioritization` | the initiative, proposal, and reprioritization types | **open** (the brief's Q5; registered 2026-09-06) |
| 53 | whether budget is a scope term that attenuates or a blast tier, and over which resources | `authority_model.md#budget-is-a-scope-term-that-attenuates` | — | **ruled in part** (2026-09-06): a scope term — a parameter constraint on a capability or a term of a delegation's `scope`, attenuating down the chain — with consumption a derived read over confirmed actions, never a maintained balance; **open**: which resources are budgeted — money per class is in reach through the payment classes; compute, tokens, and tasks per window are not in the design and are the operator's to admit |
...
| 56 | where the enforcement point sits that ties an admitted governance write to a permitted action — the record's admission check reading the action, a proxy in front of the record, or a sole-writer grant | `gates_and_workflows.md#where-the-enforcement-point-for-a-governance-write-sits` | the conformance suite's audit-shaped rows on governance writes; the cross-type cycle walk's enforcement point | **open** (the testability pass, 2026-09-06; put beside decision 43, whose two questions decide it) |
...
rather than their number, as 13 and 14 are. Seven rows are open — 33 and 34, which the simplification pass of 2026-09-05 opened as removals whose guarantee coverage would shift rather than be exactly preserved, each argued under a heading naming its subject in the document that owns it, and each deferred to the condensation pass; 36, opened on the operator's 2026-09-05 memos on how tasks come into existence and argued in `work_model.md`; 47 and 52, two of the questions `authority_model.md` had carried as the brief's Q1–Q8 and the raiser question since before the register existed, registered by the consistency pass of 2026-09-06 without renumbering the brief's labels (C13, marked open beside them, was found settled by C9 and decision 37 and is marked so in place); 55, opened by the second workflow-format pass from what nine declarations could not say, argued beside 45 in `adapters.md`; and 56, opened by the testability pass of 2026-09-06 from the one enforcement point the conformance suite found unnamed, and argued in `gates_and_workflows.md`. Three rows are ruled in part — 43, 50, and 53 — each stating its open half beside its ruled one. The rulings pass of 2026-09-06 (revision 38) took 31, 32, 35, 42, 44, 45, 46, 48, 49, 51, and 54 whole and those three in part, on the operator's standing instruction to apply the recommended side of each decision the design's own logic settles and to leave him every one that turns on his values, strategy, or appetite; 42 was opened by the skills leg of the migration, 43 and 44 by the conformance-suite design, 45 by the workflow-format pass, and 46 to 54 by the consistency pass from `authority_model.md`'s long-open questions; 37 to 41 were opened and ruled
