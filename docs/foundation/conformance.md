...
| 40 | what a step records at close about the session it ran in | `gates_and_workflows.md#what-a-step-leaves-at-close-what-it-produced-and-a-reference-to-what-it-read` | — | **ruled** (2026-09-06): what it produced is written as the entities it is; what it read is named on the sign-off and reproduced by an as-of read at `signed_at`, never copied; its reasoning is not written; `agent_session` stays the identity and liveness half. **Flagged by the Human Inversion mapping pass (2026-09-06):** a public essay series' Part 4 argues for "session replay: the reasoning chain of the agent … available months later," which this ruling contradicts. Noted here as a flag, not reopened. |
