...
72 — the table above carries 71 as of the underdetermined-inputs pass, 2026-09-06, and 70 is assigned on the
branch that rules it. This line has now gone stale three times, each time because a pass added a row and left
the sentence naming the number it had just consumed; it is maintained here as part of adding a row, and a
pass that adds one without advancing it leaves the same defect again.
