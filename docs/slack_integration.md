# Slack integration (read side)

Gives the swarm read access to Slack — search and channel history — the way
`gws` gives it Google Workspace. Implemented by
...
- **Read-only.** No send subcommand. Posting into a shared team workspace is
  outward-facing and stays operator-gated; outbound alerts keep using the
  webhook.
...
- **User token, not bot token.** `search.messages` is only available to user
  ...
  what the operator can already read.
