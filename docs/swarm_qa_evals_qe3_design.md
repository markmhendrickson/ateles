...
2. **Prepare a PR-branch worktree (neotoma only, qa lens only).** A new helper `prepare_pr_worktree(repo, pr_number, agent) -> path | None`:
   ...
   - Authenticate pushes **without writing any token to disk**: set `origin` to a clean `https://github.com/<repo>.git` and configure `gh` as the worktree's credential helper (`credential.https://github.com.helper = !gh auth git-credential`). At push time `gh` supplies the token from the child's `GH_TOKEN` env (injected by `run_skill` from the per-agent #109 token). A failed push echoes only the clean URL — never a secret. (Earlier draft embedded the token in `remote.origin.url`; rejected because git leaks the full URL incl. token in push-error output — Loxia #134.)
