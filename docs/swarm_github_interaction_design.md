...
1. **Formal PR Reviews** (`gh pr review --approve` / `--request-changes` / `--comment`). Vanellus + lenses emit real GitHub Review verdicts so outcomes show in GitHub's review state + branch protection, not just prose. Map the shared verdict vocabulary → the review event. (Best-effort + operator-gated for merge, per current policy.) **Binding** `APPROVE` / `REQUEST_CHANGES` events never use the shared author/dispatch account when that account authored the PR: Apis posts them with the reviewer GitHub App installation token when configured, otherwise `VANELLUS_AGENT_PAT`, and fails closed rather than self-reviewing.
