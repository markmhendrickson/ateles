...
- [**Rule inventory**](foundation/rule_inventory.md) — dated, perishable measurement of every place a rule
  is stated across the system, per-rule rather than per-file, clustered by kind rather than by value:
  stage 0 for `migration.md`'s rule-migration stage. Regenerated by `render_rule_inventory.py`, never
  maintained.
