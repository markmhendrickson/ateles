...
- [**Decision state**](foundation/decision_state.md) — projection of the open-design-decision register onto
  ruled, merged, and implemented; generated by `render_decision_state.py`, held equal by `--check`. States
  no rule; never keyed or inlined.
...
- [**Skill inventory**](foundation/skill_inventory.md) — dated, perishable measurement of every skill on
  disk and in Neotoma, by name + content hash rather than path: stage 0 for `migration.md`'s
  skill-migration stage 11, since a skill never inventoried is never classified, migrated, or retired.
  Regenerated by `render_skill_inventory.py`, never maintained.
- [**Rule inventory**](foundation/rule_inventory.md) — stage 0 of the rule migration: every place a rule is
  stated across the entity stores, the harnesses, the repositories and the skills, clustered by kind so a
  rule stated in many places counts once, with the locations that disagree about whether it binds.
  Generated by `render_rule_inventory.py`. States no rule — it reports where the rules that do are
  written down; never keyed or inlined.
