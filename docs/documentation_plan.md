...
`foundation/*` (21 authored documents: a 3-document always-read kernel; 13 path-keyed documents, five of
them one per external system — `github.md`, `gmail.md`, `calendar.md`, `telegram.md`, `payments.md`, one
the acceptance suite's design, `conformance_suite.md`, and one the layered planning records above a task,
`planning_model.md`; and 5
companions — `charter.md`, the purpose the corpus serves; `scenarios.md`, `workflows.md`, `migration.md`,
and `foundation/status.md`, the dated implementation-gap measurement. `foundation/projection/` is generated by
`execution/scripts/render_reading_projection.py` (decision 66) and is not authored documentation, so it is
not counted here), `swarm_smoke_test_plan.md`, `smoke_test_runbook.md`,
