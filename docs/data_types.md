...
**Agent-local generalization, as proposals** (`lib/daemon_runtime/generalizer.py`): when ≥ the agent's `agent_strategy.drift_signal_threshold` independent same-theme `strategy_drift_signal`s accumulate, the generalizer **proposes** an `agent_policy`; it never writes one (operator ruling on ateles#1270, 2026-09-25). The proposal is a `strategy_revision_proposal` with `target_entity_type: agent_policy`, `target_entity_id` the agent the rule would bind, and `proposed_change` the canonical JSON `{op: create, entity_type: agent_policy, fields: {...}}` of the exact row (`scope: agent`, `rule_kind: advisory`, `overridable_by: columba@ateles-swarm, operator`, and a `body` maturation record with `auto_generated: true`). A contradicting signal against a live auto-generated policy produces a proposal `{op: correct, entity_id, fields: {status: suspended}}` in the same shape. A proposal becomes a live rule only when a different signed swarm identity approves it through a checkpoint, plus the operator's approval when the rule reaches sessions; that approval step is not built yet. **The approval must check the verified signer on the stored record, not a field, and per field — and the signer is identified by its key, not by a label:** every observation that set `proposed_change`, `target_entity_type` or `target_entity_id` must carry `provenance.agent_sub` equal to the proposer's swarm identity AND `provenance.agent_thumbprint` equal to the RFC 7638 thumbprint of the proposer's own key, at a verified-signature tier (`neotoma_signed.check_observation_attribution`, passed the proposer's `NeotomaWriter.thumbprint`), at least one of them must have set `proposed_change`, and otherwise the proposal is refused. `agent_sub` alone is not enough: it is a label the writer's own token claims, which Neotoma does not verify against any key, so a different key whose token carries the same `sub` label would pass a sub-only check; the thumbprint is what the signature actually proves, and it is the same value an `agent_grant`'s `match_thumbprint` pins. Some other signed observation on the entity (a signed evidence correction, say) does not count. `proposing_agent_sub` is self-reported and anyone holding the bearer can write it. The approver applies only `proposed_change` and approves its digest; for a suspension, `target_entity_id` must equal `proposed_change.entity_id`. **One open proposal per rule:** a proposal's identity is its proposed change (canonical rule text, `scope`, target `agent_sub`, `applies_when`), never its evidence. Before opening one, the generalizer reads the open (`status` and `operator_decision` pending or unset) proposals against `agent_policy`; when one has the same identity, a cluster that has grown since only adds its new signals to that proposal's `drift_signal_refs`, leaving `proposed_change` untouched. With Anthus's switch `shadow` or `on`, it adds evidence only to a proposal whose defining fields pass that same per-field signer check as `anthus@ateles-swarm`; a same-rule proposal someone wrote with the bearer is passed over and a signed one opened beside it, and if the signer cannot be read nothing is opened that tick. If that read fails, nothing is opened that tick. Operator/Columba-authored policies are never proposed for supersession. Writes are signed as `anthus@ateles-swarm` when `ATELES_SIGNED_WRITES_ANTHUS` is `shadow` or `on` (switch-on runbook: [`aauth/signed_daemon_writes.md`](aauth/signed_daemon_writes.md)). Revert a legacy auto-policy with `scripts/revert_auto_policy.py`.
...
Created automatically when a pattern of `strategy_drift_signal` lines accumulates. The generalizer opens one (`operator_decision: pending`) for every cluster that reaches threshold: against the agent's `agent_definition` for clusters **outside the agent-local envelope** (cross-cutting/strategy themes, a conflict with an operator-authored policy, or the per-agent cap), and against `agent_policy` for agent-local clusters and contradicted auto-policies (see `agent_policy` above, including the rule an approver must follow: check the signer on the stored record and refuse an unsigned proposal). An agent-local rule has at most one open proposal; later evidence is added to it. Fields: `proposing_agent_sub` + `target_entity_id` + `proposed_at` (required), plus `target_entity_type`, `summary`, `drift_signal_refs`, `proposed_change`, `affects_higher_layer`, `operator_decision`, `operator_notes`, `status`.
