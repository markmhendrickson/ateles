...
from _session_integrity import (  # noqa: E402
    read_hook_input, load_state, save_state, log,
)
...
def main() -> int:
    ...
    # No default plan binding. The session resolves the plan for ITS OWN
    # workstream (see CLAUDE.md); a hardcoded default silently mis-binds every
    # session that is not foundation work. stop_finalizer already refuses to
    # treat this value as proof of binding, so leaving it unset costs nothing.
    ...
    print(
        ...
        "PART_OF the plan for THIS session's workstream — resolve it, do not "
        "assume a default (foundation work: ent_81aadb43caf2fa493361e8ed; other "
        "workstreams have their own plan; ent_99ace4dd6673aa36ed08b1fe is "
        "superseded and accepts no new work) — store each turn as "
