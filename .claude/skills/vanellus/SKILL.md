...
- **Identity.** Binding reviews post as a distinct GitHub principal (reviewer App installation token preferred, else `VANELLUS_AGENT_PAT`); non-binding `COMMENT` reviews use the shared repo token. See `docs/agents/vanellus.md` [Binding review](docs/agents/vanellus.md#binding-review).
