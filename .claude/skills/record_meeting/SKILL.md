...
- `record_meeting:stop`
  ...
  - **Speaker diarization** is enabled automatically when `ELEVENLABS_API_KEY` is set (passes `--diarize`), because meetings need speaker separation and the local model cannot do it. On failure it falls back to the **free local `whisper-cli` backend** (single-speaker text, no labels) — not to the metered OpenAI API. Set `RECORD_MEETING_DIARIZE=0` to force plain local mode.
  ...
  - **Auto-invoke `/process-meeting` (general):** After transcription completes successfully, **automatically invoke [`/process-meeting`](../process-meeting/SKILL.md)** with the transcript file path as the source, in the same turn. Do not wait for the user to ask. This preserves the full capture set in Neotoma, records a reusable `transcription_profile`, resolves the matter and participants against existing entities, reconciles extracted facts into the graph, creates and updates swarm tasks, runs per-topic strategic sub-analyses, evaluates each participant's positions, researches open questions externally, and assesses the meeting's value and relationship significance.
    - **Nothing outbound is drafted or sent.** `process-meeting` writes no recap emails, no messages, no Gmail drafts, and no rendered pages. Where a follow-up message or a participant-facing recap page is worth producing, it files a recommendation plus a task carrying intent, key points, and tone — the writing stays with the operator. This is why the general auto-invoke is safe to leave on by default.
    - Gate: skip when `RECORD_MEETING_AUTO_ANALYZE_MEETING=0` is set in env (default: on). Also skipped by `/process-meeting` itself when the transcript is empty / too short / clearly not a meeting — in that case it still preserves and transcribes, but runs no analysis (see that skill's "Skip silently on non-meetings" rule).
    - Depth defaults to `standard`. Pass `--depth deep` for a full sweep (every topic sub-analysed, every open question researched) or `--depth quick` to skip sub-analyses, argument evaluation, and research.
    - `process-meeting` does NOT open real GitHub issues by default — it stages drafts. Set `MEETING_ANALYSIS_OPEN_GH_ISSUES=1` (or pass `--open-issues` when invoking manually) to actually open.
    - Both analyze skills are complementary and BOTH fire when the Neotoma heuristic below also triggers. They cross-link via the shared `transcription` and `contact` entities.
  - **Neotoma-related call detection (auto-analyze):** After transcription completes, scan the transcript text for Neotoma-related signal using this heuristic: the transcript qualifies if it contains the literal word "Neotoma" (case-insensitive), OR contains at least 2 of the following domain terms: `entity`, `schema`, `MCP`, `memory`, `observation`, `store`, `transcription`, `knowledge graph`, `feedback_analysis`. Apply this heuristic to the full transcript text returned by `transcribe_audio.py`.
    - If `RECORD_MEETING_AUTO_ANALYZE_NEOTOMA=1` is set in env and the heuristic fires: **additionally invoke `/analyze-neotoma-feedback`** with the transcript file path as the source, in the same turn. This runs alongside `/process-meeting` (above), not instead of it. Do not wait for the user to ask.
...
4. **Transcription:** the default backend is **local `whisper-cli`** (free, on-device, no key). `transcribe_audio.py` uses ElevenLabs only when diarization is requested — `--diarize`, or `RECORD_MEETING_DIARIZE=1` with `ELEVENLABS_API_KEY` set — giving **multichannel** for stereo (merges `[System]` / `[Mic]` **in time order** when word timestamps exist, otherwise channel order: system then mic) and **diarization** for mono. `RECORD_MEETING_DIARIZE=0` or `--no-diarize` forces the local backend.
   - The **metered** OpenAI Whisper API is now an explicit opt-in only: `--backend openai` or `TRANSCRIBE_BACKEND=openai`. It is never selected automatically and is never a silent fallback — a misconfigured local backend fails loudly instead of starting to bill.
   - Local model: `~/.cache/whisper-cpp/ggml-large-v3-turbo.bin`, overridable with `TRANSCRIBE_WHISPER_MODEL`. Needs `ffmpeg` (voice memos are `.m4a`; whisper-cli reads only flac/mp3/ogg/wav).
