...
4. **Transcription:** the default backend is **local `whisper-cli`** (free, on-device, no key). `transcribe_audio.py` uses ElevenLabs only when diarization is requested — `--diarize`, or `RECORD_MEETING_DIARIZE=1` with `ELEVENLABS_API_KEY` set — giving **multichannel** for stereo (merges `[System]` / `[Mic]` **in time order** when word timestamps exist, otherwise channel order: system then mic) and **diarization** for mono. `RECORD_MEETING_DIARIZE=0` or `--no-diarize` forces the local backend.
   - The **metered** OpenAI Whisper API is now an explicit opt-in only: `--backend openai` or `TRANSCRIBE_BACKEND=openai`. It is never selected automatically and is never a silent fallback — a misconfigured local backend fails loudly instead of starting to bill. The reverse direction is different: if OpenAI itself was explicitly chosen and then fails with a provider-level error (quota/spend-limit exhausted, invalid/revoked key, org permission denial, or OpenAI's own 5xx/connection/timeout), `transcribe_audio.py` falls through ONCE — to ElevenLabs if `ELEVENLABS_API_KEY` is set, else to local `whisper-cli` — rather than failing the run. It never retries OpenAI itself and never tries more than one alternate backend. A malformed/corrupt-file error does NOT fall through (every backend would reject the same bytes). The result is marked so this is never mistaken for an ordinary run on the preferred backend: `transcription_engine` gets a `_fallback_from_openai` suffix and the result carries a `provider_fallback` dict with the reason and the backend actually used. This matters for quality, not just bookkeeping — the local backend does not produce Whisper's `no_speech_prob` / `avg_logprob` confidence fields (OpenAI-API-only; see the hallucination-filter discussion below), so a reader relying on those fields needs to know a fallback transcript may not carry them.
