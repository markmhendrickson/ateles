...
- `stop` — stop the Audio Hijack session. Audio Hijack finalizes the `*remote*`/`*mic*` files; the **Tyto daemon** then transcribes (`transcribe_audio.py`, routing local `whisper-cli` for mono / ElevenLabs for multichannel per the backend rules below) and auto-invokes `/analyze-meeting`. This skill does not transcribe inline.
...
- **Direct-to-prod transport:** `transcribe_audio.py` calls the CLI with `--api-only --base-url $NEOTOMA_PROD_BASE_URL` (default `http://localhost:3180`) and inherits `NEOTOMA_BEARER_TOKEN` from the shell env. It performs an auth preflight and fails loudly if the bearer token is missing rather than silently falling back to the unauthenticated dev server.
...
3. **Transcription backend:** the default backend is **local `whisper-cli`** (free, on-device, no key) — `~/.cache/whisper-cpp/ggml-large-v3-turbo.bin`, overridable with `TRANSCRIBE_WHISPER_MODEL`. Needs `ffmpeg` (whisper-cli reads only flac/mp3/ogg/wav). `transcribe_audio.py` uses **ElevenLabs** instead when diarization is requested (`--diarize`, or `RECORD_MEETING_DIARIZE=1` with `ELEVENLABS_API_KEY` set) — giving multichannel merge for stereo and diarization for mono. `RECORD_MEETING_DIARIZE=0` or `--no-diarize` forces the local backend. The **metered** OpenAI Whisper API is opt-in only (`--backend openai` / `TRANSCRIBE_BACKEND=openai`) — never automatic, never a silent fallback.
