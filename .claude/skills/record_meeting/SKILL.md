...
- `record_meeting:stop`
  ...
  - **Speaker diarization** is chosen by the AUDIO, not the API key: a mic-pair or a multi-channel file gets ElevenLabs diarization (`--diarize`); a mono single file is transcribed on-device with whisper.cpp. Falls back to the API path on local failure. Set `RECORD_MEETING_DIARIZE=0` to force plain mode, or `TRANSCRIBE_ENGINE_LEGACY_ROUTING=1` to restore the old key-presence routing.
...
4. **Transcription:** `transcribe_audio.py` routes on the audio. **Multi-channel** (a mic+system capture) goes to ElevenLabs, which merges `[System]` / `[Mic]` **in time order** when word timestamps exist (otherwise channel order: system then mic) — needs `ELEVENLABS_API_KEY`. **Mono** (a voice memo) is transcribed locally by whisper.cpp and never leaves the machine — needs `whisper-cli` on PATH (or `WHISPER_CPP_BIN`) and a ggml model (`WHISPER_CPP_MODEL`, default `~/.cache/whisper-cpp/ggml-medium.bin`). `OPENAI_API_KEY` backs the remaining API fallback. Force an engine with `transcribe_audio.py --engine {auto,local,elevenlabs,openai}`.
