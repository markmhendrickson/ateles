...
# `status.md` is a dated report, not a design document. `decision_state.md` is GENERATED by
# `render_decision_state.py` from `origin/main` and the remote branches — never from the working
# tree — so a branch that retires a name in a register question cannot make the generated mirror
...
#
# `rule_inventory.md` is GENERATED by `render_rule_inventory.py` and its body is QUOTED TEXT: the
# statements other rule stores make, reproduced so a reader can see where each rule is written down.
# It necessarily carries those stores' vocabulary, including retired names — that a harness file
# still says `lens` or `dispatch` is the inventory's FINDING, not its defect, and rewriting the
# quotation to satisfy the checker would destroy the measurement. The fix for a retired name it
# surfaces is in the store that states it, which this document's job is to locate.
SKIPPED_FILES = {"status.md", "decision_state.md", "rule_inventory.md"}
