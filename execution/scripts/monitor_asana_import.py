#!/usr/bin/env python3
"""
Monitor and auto-restart Asana import script if it stalls.

Checks every run:
1. Is import process running?
2. If running, is it making progress? (log file updated recently, CPU activity)
3. If stalled, kill and restart with --resume

Usage:
    python execution/scripts/monitor_asana_import.py

Or via cron:
    */5 * * * * cd /path/to/finances && execution/venv/bin/python execution/scripts/monitor_asana_import.py >> /tmp/asana_import_monitor.log 2>&1
"""
...
IMPORT_SCRIPT = PROJECT_ROOT / "execution" / "scripts" / "import_asana_tasks.py"
LOG_FILE = Path("/tmp/asana_import.log")
CHECKPOINT_FILE = get_data_dir() / "logs" / "import_checkpoint.json"
MONITOR_LOG = Path("/tmp/asana_import_monitor.log")
...
def send_notification(title: str, message: str, subtitle: str = ""):
    ...
    try:
        ...
        if terminal_notifier.returncode == 0:
            ...
            cmd.extend(["-group", "asana_import_monitor"])
            cmd.extend(
...
def find_import_processes():
    """Find running import_asana_tasks.py processes."""
    try:
        result = subprocess.run(
            ["pgrep", "-f", "import_asana_tasks.py"],
            capture_output=True,
            text=True,
            timeout=5,
        )
    ...
    try:
        ...
        if result.returncode == 0:
            log(
                ...
                notify=True,
                notify_title="Import Monitor Warning",
            )
        else:
            log(f"Process {pid} terminated", notify=True, notify_title="Import Monitor")
    except Exception as e:
        log(
            f"Error killing process {pid}: {e}",
            notify=True,
            notify_title="Import Monitor Error",
        )


def start_import():
    """Start import script with --resume flag."""
    try:
        log("Starting import script with --resume")
        cmd = [sys.executable, "-u", str(IMPORT_SCRIPT), "--resume"]
        # Start in background
        subprocess.Popen(
            cmd,
        ...
        log(
            ...
            notify=True,
            notify_title="Import Monitor",
            notify_subtitle="Resuming import",
        )
        return True
    except Exception as e:
        log(
            f"Error starting import script: {e}",
            notify=True,
            notify_title="Import Monitor Error",
        )
        return False
...
def check_import_completion():
    """Check if import has completed successfully."""
    if not LOG_FILE.exists():
        return False, None
    ...
    try:
        ...
        with open(LOG_FILE) as f:
            ...
            if (
                "Import completed successfully" in last_lines
                or "=== Import Complete ===" in last_lines
            ):
...
def get_progress_info():
    """Get current import progress from checkpoint or log."""
    progress_info = []
...
def main():
    ...
    completed, stats = check_import_completion()
    if completed:
        message = "Asana import completed successfully"
        if stats:
            # Extract key stats for notification
            stats_lines = stats.split("\n")[:4]  # First 4 lines
            message = "\n".join(stats_lines)
        log(
            f"Import completed: {stats}",
            notify=True,
            notify_title="Import Complete",
            notify_subtitle="All tasks imported",
        )
        # Don't restart if already completed
        return

    pids = find_import_processes()

    if not pids:
        log("No import process running, starting new import")
        start_import()
        return

    log(f"Found {len(pids)} import process(es): {pids}")
