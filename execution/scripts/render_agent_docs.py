...
Usage:
    ...
    render_agent_docs.py --check    # differ/orphan: relative paths + Fix: (no --check); zero rows: different non-zero, does not say regenerate
...
# Same sentence scripts/lint.sh prints for drift. No `--check` on it.
MIRROR_FIX_LINE = (
...
def check(agents: list[dict]) -> int:
    ...
    if failures or orphans:
        ...
        if orphans:
            ...
            for o in orphans:
                print(f"  {o}")
        print(MIRROR_FIX_LINE)
        return 1
    # A zero after fetch_agents is not a match (principles.md §3 and §7).
    # Do not print the regen Fix line: there is nothing to regenerate from.
    if len(agents) == 0:
        print(ZERO_ROWS_LINE)
        return 1
