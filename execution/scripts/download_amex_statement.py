#!/usr/bin/env python3
"""
Download American Express account statements using Playwright with 1Password browser extension.
...
4. Navigates to American Express statements section
...
Usage:
    python execution/scripts/download_amex_statement.py [--account-type <credit|savings>] [--period <YYYY-MM>]

Examples:
    python execution/scripts/download_amex_statement.py
    python execution/scripts/download_amex_statement.py --account-type credit --period 2025-12
"""
...
async def download_amex_statement(account_type: str = "credit", period: str = None):
    """
    Download American Express account statement using browser automation.

    Args:
        account_type: "credit" or "savings" (default: "credit")
    ...
    if not extension_path:
        ...
        print("3. Ensure 'American Express' item exists in 1Password")
        sys.exit(1)
    ...
    async with async_playwright() as p:
        ...
        auth_state_path = AUTH_STATE_DIR / "amex_auth_state.json"
        context_options = {
        ...
        # Navigate to American Express login
        print("\nNavigating to American Express...")
        try:
            await page.goto(
        ...
        # Wait for login
        print("\n" + "=" * 60)
        print("Please log in to American Express if prompted")
        ...
        for attempt in range(max_attempts):
            ...
            if "americanexpress.com" in current_url and (
        ...
        try:
            # American Express typically has statements under "Statements & Activity" or "Account Services"
            await page.goto(
                "https://www.americanexpress.com/us/account/login?inav=iNavLnkStmt",
                wait_until="domcontentloaded",
                timeout=30000,
            )
            await asyncio.sleep(3)

            # Alternative: Look for statements link in navigation
            try:
                await page.click('text="Statements"', timeout=10000)
                await asyncio.sleep(3)
            except Exception:
                pass
        except Exception as e:
            print(f"⚠️  Error navigating to statements: {e}")
            print("Please navigate manually to: Statements & Activity")
            await asyncio.sleep(10)
            ...
            if downloaded:
                ...
                if downloads:
                    ...
                    if period:
                        ...
                        filename = f"american_express_{account_type}_{year_month}.pdf"
                    else:
                        filename = f"american_express_{account_type}_{datetime.now().strftime('%Y_%m')}.pdf"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download American Express account statements"
    )
    parser.add_argument(
        "--account-type",
        type=str,
        choices=["credit", "savings"],
        default="credit",
        help="Account type: credit or savings (default: credit)",
    )
    parser.add_argument(
    ...
    asyncio.run(
        download_amex_statement(account_type=args.account_type, period=args.period)
    )
