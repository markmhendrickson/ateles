...
class Marker:
    ...
    profile_name: str
    # Session-scoped approval token (lib/approval/tokens.token_for(event_id,
    # session=date)) — the durable identity of this approval request. Reply
    # matching goes through this token (lib/approval.email_channel.read_replies
    # + tokens.parse_verdict), NOT thread position, so a stale reply sitting in
    # the inbox for a PRIOR session can never be mistaken for a reply to this
    # one (ateles 2026-07-22 incident; see tokens.token_for docstring).
    token: str = ""
    # The message id of our own notify email, captured via read_replies's
    # on_reply_message callback once the operator's reply is seen — needed so
    # the confirmation can go out via email_channel.reply_in_thread and land in
    # the SAME thread as the operator's approval.
