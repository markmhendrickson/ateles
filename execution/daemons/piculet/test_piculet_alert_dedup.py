...
def test_alert_refires_after_clear(monkeypatch):
    ...
    # Both channels share one gate keyed on the raw message, so clearing that
    # single key is what the daemon's own resolve path does (watch.py).
    watch._telegram_clear("Neotoma unavailable")
