...
the date **after at least one batch completes successfully**; same-day
re-invocations exit immediately once stamped. If every batch in a run fails
(missing `claude`, timeout, bad `MIMUS_RESULT`), the stamp is not written and
the daemon may retry later the same day.

### Phase 1 — scheduler (Python)

Thin scheduler mirroring Cotinga: acquire PID lock, enforce once-per-day
idempotency (only after success), read the backlog cursor, and spawn one or
more blocking `claude --print` extraction agents per run.

1. **Cursor.** Read `.mimus_cursor` (JSON: `{offset, drained, updated_at}`).
   `offset` is the number of oldest-first `conversation` entities already
   swept; `drained` is true once the full backlog has been processed. **First
   run sweeps the full backlog** (~5,701 conversations) in batches (operator
   decision, 2026-06-19); after drain, runs tail newly-updated conversations
   as the corpus grows.
2. **Batch loop.** For each batch (up to `MIMUS_MAX_BATCHES_PER_RUN`), spawn a
   blocking `claude --print` agent with the current `offset` and
   `MIMUS_BATCH_SIZE`. On failure, leave the cursor unmoved and stop (fail-open).

### Phase 2 — delegated extraction agent

The spawned agent performs all Neotoma I/O via MCP:

3. **Retrieve.** `retrieve_entities(entity_type="conversation", …)` ordered
   oldest-first (`created_at` ascending) with the batch's `limit`/`offset`.
   Prefer linked `analysis` entities when present; fall back to raw conversation
   content only when no analysis exists.
4. **Extract.** Identify candidate content ideas (title, summary, pillar,
   format, platforms, tags, confidence) using the content-marketing-ideas /
   memetic-ideas lens from the `analyze-neotoma-feedback` skill.
5. **Dedupe.** For each candidate, `retrieve_entities(entity_type="post_idea",
   search=<title/summary>)`; drop near-duplicates of existing ideas and of each
   other within the batch.
6. **Store.** Surviving candidates → `store` as `post_idea` with:
   - `status: "idea"`
...
7. **Notify.** One ranked Telegram digest (by `confidence`) to the content
   topic for ideas stored this batch.
8. **Advance cursor.** Print a trailing `MIMUS_RESULT {"retrieved": N, …}`
   sentinel. Python parses it on a clean run and writes `.mimus_cursor` with the
   new offset (and `drained` when a short or empty page signals end-of-corpus).
...
- **Idempotent** per day once at least one batch succeeds; the cursor prevents
  reprocessing swept conversations; all-fail runs skip the daily stamp.
- **Fail-open / read-mostly.** Any batch error → log, leave cursor unmoved, and
  exit without crashing; never partial-write an idea without its `REFERS_TO` +
  `PART_OF` links.
...
- **Backlog vs. forward-only** — *resolved 2026-06-19:* first run sweeps the
  full ~5,701-conversation backlog in batches (offset-based cursor, oldest-first),
  then tails newly-updated conversations once `drained` is true.
