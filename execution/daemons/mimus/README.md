...
| `ANTHROPIC_API_KEY` | Auth for the `claude --print` extraction subprocess |
...
| File | Purpose |
|------|---------|
| `.mimus_last_run` | Date stamp written after at least one batch succeeds — enforces once-per-day idempotency; omitted when every batch fails so the same day can retry |
...
- **Fail-open.** Any batch error logs and exits without advancing the cursor; a
  failed batch is retried on the next run (including later the same day if every
  batch in a run failed, since the once-per-day stamp is only written after at
