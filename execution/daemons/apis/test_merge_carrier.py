...
# The single reference instant for this file. Every fixture timestamp is built
# as an offset from it, and every call under test is handed it explicitly as
# `now=NOW` — so the arithmetic is closed over this constant and no assertion
# depends on the wall clock.
#
# This value is arbitrary and must stay arbitrary. It was originally the date
# the file was written, and the tests read the real clock while comparing
# against fixtures frozen here; the computed age therefore grew by one a day
# and `test_approved_clean_pr_is_found_with_age_and_state` went 12 -> 13 on
# 2026-09-01, failing every open PR in the repo. If you add a test here, pass
# NOW into the code under test rather than letting it reach for the clock.
