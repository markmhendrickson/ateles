...
AMBIGUOUS MATCH: FAIL CLOSED (ateles#1227)
-------------------------------------------
A ``matches`` predicate built on ``repo``/``issue_number`` identity should
resolve at most one row. If a single targeted combo's page contains MORE THAN
ONE row that satisfies ``matches`` — an anomaly the identity rule says should
never happen — this module refuses to guess which one is authoritative: it
logs the collision and returns ``None`` (not found), rather than adopting the
first re-verified hit. Silently picking one risks a caller CORRECTING the
wrong entity's fields, which is a worse failure than a lookup miss.
"""
...
async def _load_targeted(
    ...
    A combo whose page contains MORE THAN ONE re-verified match fails CLOSED
    (returns None, logged) rather than adopting the first — see the module
    docstring's "Ambiguous match" section (ateles#1227).
    """
    seen_presence = False
    for combo in filter_combos:
        ...
        for entity in entities:
            ...
            if matches(snap):
                verified.append((entity, snap))
        if len(verified) > 1:
            log.error(
                "[apis.entity_lookup] %s: %s targeted query (combo %s) "
                "returned %d re-verified matches (expected 0 or 1 under the "
                "caller's identity rule) — refusing to guess; treating as "
                "not found",
                ref, entity_type, sorted(combo.keys()), len(verified),
            )
            return None
        if verified:
            return verified[0]
...
async def _load_by_scan(
    ...
    A single page containing MORE THAN ONE re-verified match fails CLOSED
    (returns None immediately, logged) — same rule as ``_load_targeted``'s
    "Ambiguous match" behaviour (ateles#1227), so an ambiguity that the
    targeted query's own combos failed to resolve is not silently re-resolved
    by falling through to this scan.
    """
    cursor = ""
    for page in range(SCAN_MAX_PAGES):
        ...
        if len(verified) > 1:
            log.error(
                "[apis.entity_lookup] %s: %s recency-scan page %d returned "
                "%d re-verified matches (expected 0 or 1 under the caller's "
                "identity rule) — refusing to guess; treating as not found",
                ref, entity_type, page, len(verified),
            )
            return None
        if verified:
            return verified[0]
