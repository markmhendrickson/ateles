...
class TestDocsIndexMatchesInventory:
    ...
    def test_root_readme_start_here_footer_links_every_document(self) -> None:
        """The Start-here footer is checked as a section, not as part of the whole file.

        ``test_root_readme_links_every_document_and_no_ghost`` scans the entire README, so a document
        named anywhere in the prose satisfies it while the footer that a reader actually navigates from
        stays short. That is exactly how ``conformance_suite.md`` and ``planning_model.md`` went missing
        from the footer on PR #745 while every whole-file assertion stayed green.
        """
        import re

        text = (_REPO_ROOT / "README.md").read_text(encoding="utf-8")
        marker = "**The design of record:** [Foundation](docs/foundation/)"
        start = text.find(marker)
        assert start != -1, "the root README no longer carries the Start-here foundation footer"
        footer = text[start:].split("\n\n", 1)[0]
        linked = set(re.findall(r"\]\(docs/foundation/([\w.-]+\.md)\)", footer))
        inventory = self._inventory()
        assert inventory - linked == set(), sorted(inventory - linked)  # unlisted in the footer
        assert linked - inventory == set(), sorted(linked - inventory)  # footer link to a gone file

    def test_documentation_plan_states_the_current_inventory(self) -> None:
        """``docs/documentation_plan.md`` states the foundation's size; pin it to the directory.

        The plan is outside the two indexes the tests above cover, so it drifted independently — it read
        "19 documents ... 12 path-keyed" against a twenty-document tree. Only the authored ``*.md`` files
        count; ``docs/foundation/projection/`` is generated by ``render_reading_projection.py``
        (decision 66) and is deliberately excluded from the inventory.
        """
        import re

        text = (_REPO_ROOT / "docs" / "documentation_plan.md").read_text(encoding="utf-8")
