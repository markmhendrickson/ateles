...
def test_confirm_gates_clear_from_operator_calls_lanius(monkeypatch):
    ...
    The deterministic Neotoma persist+verify (_persist_gate_waive) is
    stubbed to simulate a successful waive — this test is about dispatch
    routing/prompt content/notifier, not the persistence mechanics (covered
    separately by the _persist_gate_waive-focused tests below).
    """
    calls = []
    ...
    async def fake_persist_gate_waive(self, repository, issue_number):
        return True

    monkeypatch.setattr(swarm_dispatch, "run_skill", fake_run_skill)
    monkeypatch.setattr(
        SwarmDispatcher, "_persist_gate_waive", fake_persist_gate_waive
    )
    notifier = _StubNotifier()
    dispatcher = SwarmDispatcher(notifier, _config())
...
# ── _persist_gate_waive — deterministic waive + verify-after-write ─────────────
#
# ateles task ent_425fa2ff1414064e609c7242: Lanius previously posted a GitHub
# "gates clear" comment while gate_status stayed pending in Neotoma (comment-
# only, false success). These tests cover the deterministic replacement:
# swarm_dispatch itself reads/initializes/writes/re-reads gate_status via the
# Neotoma REST /entities/query, GET /entities/:id, and /correct routes (the
# same shape IssueSpecStore already uses in issue_spec.py), independent of any
# spawned Lanius subprocess.


class _GateWaiveHttpClient:
    """Fake httpx.AsyncClient driving the three Neotoma REST calls
    _persist_gate_waive makes: POST /entities/query (resolve entity_id),
    GET /entities/<id> (read gate_status), POST /correct (write a field).

    `initial_gate_status=None` simulates a legacy issue with no gate_status
    field at all. `write_lands=False` simulates a write that returns 2xx but
    is not visible on the next read (the exact false-success shape being
    fixed) by never mutating `self.gate_status`/`self.current_owner`.
    """

    ENTITY_ID = "ent_test_gate_waive_0001"

    def __init__(
        self,
        repository: str,
        issue_number: int,
        initial_gate_status: dict | None,
        initial_current_owner: str | None = "pavo",
        write_lands: bool = True,
    ):
        self.repository = repository
        self.issue_number = issue_number
        self.gate_status = (
            dict(initial_gate_status) if initial_gate_status is not None else None
        )
        self.current_owner = initial_current_owner
    ...
    async def post(self, url, json=None, headers=None, **kwargs):
        ...
        if url.endswith("/correct"):
            self.correct_calls.append(json)
            if self.write_lands:
                if json.get("field") == "gate_status":
                    self.gate_status = json.get("value")
                elif json.get("field") == "current_owner":
                    self.current_owner = json.get("value")
            return _JsonResp(200, {"success": True})
        raise AssertionError(f"unexpected POST {url}")

    async def get(self, url, headers=None, **kwargs):
        self.get_calls.append(url)
        assert url.endswith(f"/entities/{self.ENTITY_ID}")
        snapshot = {"repository": self.repository}
        if self.gate_status is not None:
            # Return a COPY: the real Neotoma REST surface hands back a fresh
            # JSON payload per request. Returning the same dict object here
            # would let the daemon's in-memory mutation (before the /correct
            # write is even attempted) leak back as if it were already
            # persisted, defeating the write_lands=False test scenario.
            snapshot["gate_status"] = dict(self.gate_status)
        if self.current_owner is not None:
            snapshot["current_owner"] = self.current_owner
        return _JsonResp(200, {"entity_id": self.ENTITY_ID, "snapshot": snapshot})
...
def test_persist_gate_waive_legacy_issue_initializes_and_waives(monkeypatch):
    """A legacy issue with NO gate_status must be initialized then have every
    _WAIVABLE_PRE_IMPL_GATES gate waived, and the verify-read must pass."""
    client = _GateWaiveHttpClient(
        "owner/repo", 174, initial_gate_status=None, initial_current_owner=None
    )
    monkeypatch.setattr(swarm_dispatch.httpx, "AsyncClient", client)

    dispatcher = SwarmDispatcher(_StubNotifier(), DispatchConfig(neotoma_token="t"))
    ok = asyncio.run(dispatcher._persist_gate_waive("owner/repo", 174))

    assert ok is True
    assert client.gate_status is not None, "gate_status must have been initialized"
    for gate in PRE_IMPL_GATES:
        assert client.gate_status[gate] == "waived", client.gate_status
    # impl/pr_review/qa are untouched (stay pending) — only pre-impl gates waive.
    assert client.gate_status["impl"] == "pending"
    assert client.gate_status["qa"] == "pending"
    # current_owner advances past the pre-impl phase.
    assert client.current_owner == "pr_review"
    # At least one /correct call targeted gate_status.
    assert any(c.get("field") == "gate_status" for c in client.correct_calls)


def test_persist_gate_waive_write_succeeds_verify_passes(monkeypatch):
    """An issue with gate_status already initialized (pending pre-impl gates)
    gets waived and the write is confirmed on re-read."""
    client = _GateWaiveHttpClient(
        "owner/repo",
        80,
        initial_gate_status={
    ...
    dispatcher = SwarmDispatcher(_StubNotifier(), DispatchConfig(neotoma_token="t"))
    ok = asyncio.run(dispatcher._persist_gate_waive("owner/repo", 80))

    assert ok is True
    assert client.gate_status["pm"] == "waived"
    assert client.gate_status["arch"] == "waived"
    assert client.gate_status["ux"] == "waived"
    assert client.current_owner == "pr_review"


def test_persist_gate_waive_returns_false_when_write_does_not_land(monkeypatch):
    """If the /correct write reports success but the gate is STILL pending on
    re-read (Neotoma flake / silent no-op), _persist_gate_waive must return
    False — never claim success for a state change that didn't land."""
    client = _GateWaiveHttpClient(
        "owner/repo",
        80,
        initial_gate_status={
    ...
    assert ok is False
    # gate_status on the "server" is unchanged (write never landed).
    assert client.gate_status["pm"] == "pending"


def test_persist_gate_waive_idempotent_on_already_waived_gates(monkeypatch):
    """Re-running the waive on an issue whose pre-impl gates are already
    signed_off/waived must be a safe no-op that still verifies True."""
    client = _GateWaiveHttpClient(
        "owner/repo",
        80,
        initial_gate_status={
        ...
        initial_current_owner="pr_review",
    )
    monkeypatch.setattr(swarm_dispatch.httpx, "AsyncClient", client)
    ...
    assert ok is True
    # Already-signed gates are left exactly as they were (idempotent).
    assert client.gate_status["pm"] == "signed_off"
    assert client.gate_status["ux"] == "waived"
    assert client.gate_status["arch"] == "waived"
    # current_owner was already pr_review; no spurious re-write needed, but a
    # same-value correct is harmless if it happens.
    assert client.current_owner == "pr_review"


def test_persist_gate_waive_returns_false_when_entity_not_found(monkeypatch):
    """When the github_issue entity cannot be resolved at all (e.g. never
    synced to Neotoma), _persist_gate_waive must return False rather than
    ...
    class _EmptyQueryClient:
        ...
        async def post(self, url, json=None, headers=None, **kwargs):
            ...
            return _JsonResp(200, {"entities": []})

    monkeypatch.setattr(swarm_dispatch.httpx, "AsyncClient", _EmptyQueryClient())
    dispatcher = SwarmDispatcher(_StubNotifier(), DispatchConfig(neotoma_token="t"))
    ok = asyncio.run(dispatcher._persist_gate_waive("owner/repo", 999))
    assert ok is False


def test_lanius_waive_gates_skips_narration_when_persist_fails(monkeypatch):
    """_lanius_waive_gates must NOT invoke Lanius when the deterministic
    persist+verify fails — nothing true to narrate, and it must return
    False so the caller does not re-trigger the PR pipeline."""
    calls = []
    ...
    result = asyncio.run(dispatcher._lanius_waive_gates(_comment_trigger()))

    assert result is False
    assert calls == [], "Lanius must not be called when persistence failed"


def test_confirm_gates_clear_does_not_retrigger_pr_pipeline_when_waive_fails(
    monkeypatch,
):
    """_handle_confirm_gates_clear must NOT call _handle_pr when the gate
    waive did not persist (Part B guard) — re-triggering against pending
    gates would just waste a panel run, and it previously masked the bug."""
    handle_pr_calls = []
    ...
    async def fake_handle_pr(self, trigger):
        handle_pr_calls.append(trigger)
    ...
    asyncio.run(
        dispatcher._handle_issue_comment(
            _comment_trigger(comment_on_pr=True, body="Closes #50.")
        )
    )

    assert handle_pr_calls == [], (
        "PR pipeline must NOT be re-triggered when the gate waive did not persist"
    )
    assert any(
        "did not persist" in m or "NOT" in m for m in notifier.sent
    ), f"Operator must be notified the waive failed; got {notifier.sent}"
