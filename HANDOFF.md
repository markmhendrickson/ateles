...
## 1. Swap the Notion token in `.mcp.json`
...
## 2. Import Notion workout history into Neotoma
...
**After merging both PRs**: update your local `.env` files to ensure `NEOTOMA_BASE_URL` is explicitly set — the daemons no longer fall back to the hardcoded domain.
