...
- **Slack**: use `python3 execution/scripts/slack_cli.py` (wraps the Slack Web API like `gws` wraps Google). Read-only by design (issue #248): `search`, `history`, `channels`, `whoami`. There is no send/post capability — automated outbound alerts continue to use the watchdog webhook. Needs `SLACK_USER_TOKEN` (materialized from `ateles-private`). See `docs/slack_integration.md`.
