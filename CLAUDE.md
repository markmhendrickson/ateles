...
- **Extend the mechanism that already generalizes; do not build a parallel one.** **Enforcement: Partial**, and narrower than "every dispatched agent." Before building, look for the thing that already does this — and search the code, not your memory of it. Three times in one day work was commissioned that already existed: `execution/scripts/hallucination_filter.py` already implemented the output-side transcription screen, with six signals **and two more that were built, measured, and rejected**, documented so they are not rediscovered; `execution/daemons/apis/dispatch_role.py` already runs any role with a `SKILL.md` through the harness router with no role allowlist, so a non-code agent is already executable by name and only the decision to run it is missing; and the gate is already PR-independent — `evaluate_gate` takes confidence and `action_type` with no PR, `write_checkpoint_brief` keys on `task_entity_id`, and the approval loop closes over Telegram, so a second approval path for non-code work would have been wasted. The converse is equally costly: ateles#341 and #700 raced on the same constant because neither checked. This includes *types* — reuse the existing relationship or entity type rather than minting one (`REFERS_TO` for task→issue edges, chosen from precedent), and keep a name that is already accurate rather than renaming for tidiness. **Corrected 2026-09-18** (this rule itself was a stale-doc instance of the failure it describes): PR #686 was **closed, never merged** (`gh pr view 686`: `state: CLOSED`, `mergedAt: null`); its `SWARM_PRIOR_ART_CONTRACT` content actually landed via **PR #802** (merge `dc2fd979`, 2026-09-07) and has been live on `origin/main` for eleven days. But it is injected only when `include_github_contract=True` in `execution/daemons/apis/skill_runner.py::build_system_prompt` — set at the 13 GitHub-triggered call sites in `swarm_dispatch.py` and nowhere else. `apis.py` (Neotoma-task dispatch), `dispatch_role.py` (one-off dispatch), and `aquila.py` (periodic dispatch) all leave the flag at its `False` default, so prior-art checking is enforced for GitHub-triggered work only — manual everywhere else. `docs/foundation/status.md:130` carries the same stale "#686, open" claim as of its 2026-09-03 snapshot; that file is a dated, perishable measurement (not `--check`-verified against live state) and was left uncorrected here rather than hand-edited.
