...
- **Escalate choices by material operator outcome.** Ask the operator only when plausible options would materially change their goals, risk, cost, external commitments, data loss, or irreversible state and existing context does not select among them. Advance mechanical, representational, and technical choices with equivalent operator outcomes automatically or send them to the appropriate review lens. Reversible development and sandbox deployments are not operator-reserved when the deployment configuration is retrieved and gates are clear. Production or client-instance deployment remains operator-gated when it materially changes exposure, commitments, cost, or live data; credential rotation remains operator-only.
