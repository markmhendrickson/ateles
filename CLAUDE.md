...
- **Put every decision that needs Mark through the harness questions tool** (`AskUserQuestion`), not in trailing prose. One call carries every open decision, with enough context to decide: the options, what each implies, what is already settled, your recommendation, and what happens if he says nothing. Never re-raise one by name alone, and never write "unchanged" — restate the choice. Carry every open decision forward each turn until answered, not just the most recent. The content requirements here are unchanged and long-settled; what changed is where the asking happens. A decision list at the bottom of a message makes him answer positionally against items he has to scroll back for — on 2026-09-16 he answered a six-item prose list that way and issued this instruction nineteen minutes later. Prose may summarize what you asked; it must not be the place the asking happens. Zero open decisions: do not call the tool, and do not add a trailer or a line that says there are no decisions. If the tool cannot be called: print `[decisions-unposed]` and each decision's question text. Do not substitute a numbered list.
