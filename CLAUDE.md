...
- **Classify a blocker before surfacing it.** When a pull request or task is blocked, decide which kind it is. **Dispatch and report** — never ask — when the blocker is a verified, specced deliverable: a missing column an issue already names, a review whose finding is already discharged, a rebase, a regeneration, a stale reference. This extends to the swarm's own mechanics, not only foundation work. **Surface it** only when it needs a judgement that cannot be derived: an acceptance standard, an unruled design question, or a merge where a live blocking objection stands. Set after a six-item decision list was found to be half noise — three items were work already approved or mechanically obvious, and one had been approved two turns earlier.
...
- **Restart daemons as needed, without asking.** A merged fix reaches nothing until the checkout a daemon runs from is updated and the process restarts. Daemons run from `~/ateles-rc-src` and `~/neotoma-rc-src`, never the shared clone. Sequence: `git fetch && git merge --ff-only origin/main` in the deployment checkout, reload the plist, then VERIFY from the running process — a new pid, and the merged change present in the file the daemon loaded. A restart reported without reading the new process back is the same defect as a write reported without a read-back. Restart only the daemons a change touches, and say which. **Credential rotation and hosted client deploys remain the operator's.**
- **A plain comment does NOT re-dispatch a swarm review.** `github_gateway.py` routes every `issue_comment` event to the operator-override command handler, so a comment carrying no command reaches a handler that finds nothing and stops. `PR_ACTIONS = {"opened", "reopened", "synchronize"}` are the only events that re-run a PR review. Three re-review requests posted on 2026-09-11 were mechanically no-ops for this reason.
- **Re-request review yourself when monitoring shows it is warranted.** Warranted means every blocking finding is discharged in substance, checks are green, and the PR is otherwise mergeable — only a re-look is missing. A close-and-reopen is the mechanism, and it is reversible and a real pipeline entry rather than a skip. NOT warranted: a live finding nobody has answered, or an adjudication this session made that no lens has seen. Say on the PR why it was reopened so it is not mistaken for flapping.
