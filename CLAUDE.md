...
- **A plain comment does NOT re-dispatch a swarm review.** `github_gateway.py` routes every `issue_comment` event to the operator-override command handler, so a comment carrying no command reaches a handler that finds nothing and stops. `PR_ACTIONS = {"opened", "reopened", "synchronize"}` are the only events that re-run a PR review. Three re-review requests posted on 2026-09-11 were mechanically no-ops for this reason. Comment commands use three authorization layers (ateles#1132); only the mechanics row drives the pipeline from a comment, otherwise a push or a review event is what moves it.

  | Layer | Commands / events | Who |
  |---|---|---|
  | Mechanics | `/confirm-gates-clear`, `/swarm-run` | `_COMMAND_LOGINS` (`APIS_COMMAND_LOGINS`, comma-separated; operator always included; unset or empty = operator-only) |
  | H1 / merge-control | `/approve`, `/reject`, `/hold` | `_OPERATOR_LOGIN` only — the allowlist does not grant these |
  | Approval | `pr_review` `approved` → merge | `_OPERATOR_LOGIN` only |

  Enablement and the does-not-grant list live in `execution/daemons/apis/com.ateles.apis.plist`. Verify the running process env (`ps eww`), not the plist file alone. The mechanics-vs-judgement bound below is unchanged.
