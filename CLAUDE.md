...
- **Secrets management — SOPS+age pipeline live (Design B)** — 1Password Family stays the canonical secret store; values ride an age-encrypted git snapshot (`secrets/*.sops.env`) that daemons/CI/other machines decrypt **offline**, fixing the daemon `op read`-needs-live-session token-refresh bug. 1Password service accounts rejected (Business/Teams-only; Family suffices by storing just the age private key). Live on both machines 2026-06-22: dev machine + Mac Studio bootstrapped (age key at `~/.config/sops/age/keys.txt`, also in `op://Private/ateles-sops-age/key`); `cyphorhinus`/`piculet` restarted on the offline path; CI secret `SOPS_AGE_KEY` set; PR #142 merged. Scripts `execution/scripts/secrets_{lib,publish,materialize}.py`; runbook `docs/secrets_management.md`; registry `secrets/manifest.env-map.json` (no `env_var_mapping` entities exist). Two follow-ups tracked as tasks PART_OF the plan: shared-checkout isolation (`ent_52d2317ce6ad181a8676c004`, high) and daemon redeploy label gap (`ent_22ed087bb7ef1b906cd4ad64`, med).
