...
- **A mechanism that does not gate is not a control.** A linter, a review verdict, or a status field only constrains behaviour at the point where something is actually blocked — reporting is not enforcing. Before claiming a check protects anything, trace it to the thing it stops: a linter registered only in `scripts/lint.sh` that no workflow invokes never runs in CI; `continue-on-error: true` turns a wired linter into an advisory annotation; a gate verdict posted as an issue comment is not enforced by GitHub, which is how PRs merged seconds after a REQUEST_CHANGES and after an explicit "do not merge". Tests carry the same defect — a test asserting current behaviour pins a bug as expected, and a test asserting only the fields that survived a lossy write certifies the loss. Likewise a write returning `success: true` is not proof the fields landed. See the defect-class writeup, `analysis` entity (below).
- **Never assert a status a mechanism cannot back.** A stored flag claiming a process is alive is a claim about the present that nothing refreshes when the process dies. Liveness is *derived at read time* — from a lease with an expiry — never read from a stored status value.
- **Dispatch vocabulary: distinguish publication, claim, and execution.** "Dispatch" conflates three separate things, so prefer `created / claimed / running / released` over `undispatched / executing`. Under the pull model, publication *is* task creation — the task exists and is claimable; a claim is an agent taking it and holding a lease; execution is a process running. Reserve "dispatch" for the push exception (genuinely named ownership: operator-only tasks, a gate assigned to a specific reviewer).
- **Durable work goes into Neotoma as `task` entities, never harness task chips.** A chip (`spawn_task`) creates a local harness session, not an entity: no task entity, no claim, no lease, no observation history — so it is invisible to the swarm, unclaimable by any agent, and lost if the operator never clicks it. Under the pull model, work that is not in Neotoma is not claimable, so it does not exist. If a finding is worth acting on, it is a `task` entity an agent can claim; if it is not worth a task entity, it is not worth a chip either. This governs work worth doing, not every passing observation — the bar is unchanged, only the vehicle.
...
**Fail-closed on Neotoma — operator's stated principle, approval PENDING (not yet policy).** The operator's position is that Neotoma is the swarm's foundational level of truth, and that the swarm should halt rather than proceed when it is unreachable — the concern being not availability but *work with no record*, which across unattended daemons is worse than the work not happening. Design and open questions live on task `ent_670cacab2f46fd9547ced7ed`, still `awaiting_approval` (blocked as low-confidence/high-blast-radius), with the design detail on plan `ent_4222e5d52edd9bdba7b78cc1`. Do not implement or cite this as settled policy until that task is approved.
