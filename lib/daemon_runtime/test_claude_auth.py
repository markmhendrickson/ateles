...
def test_oauth_token_present_drops_api_key(monkeypatch):
    ...
    assert "ANTHROPIC_API_KEY" not in env, "API key must be removed when OAuth token is set"
...
def test_no_oauth_token_leaves_api_key(monkeypatch):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-api03-yyy")
    env = claude_subprocess_env()
    assert env["ANTHROPIC_API_KEY"] == "sk-ant-api03-yyy", "no token -> API key stays (graceful fallback)"
...
def test_blank_oauth_token_leaves_api_key(monkeypatch):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-api03-yyy")
    env = claude_subprocess_env()
    assert env["ANTHROPIC_API_KEY"] == "sk-ant-api03-yyy", "blank token must not count as present"
